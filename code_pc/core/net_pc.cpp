...
bool WinsockStartup()
{
	...
	return (WSAStartup((WORD)versionRequested, &versionData) == 0) ? true : false;
}
...
// --------------------------------------------------------------------------------
bool getinetaddr(const CKeyString &host, sockaddr_in *inAddr)
{
	// inet_addr converts a string containing an IP address
	// to network byteorder integer version of the IP.
	inAddr->sin_addr.s_addr = inet_addr(ntStr::GetString(host));
	...
	if(inAddr->sin_addr.s_addr == INADDR_NONE)
	{
		...
		// to an IP.
		hostent *hostEntry = gethostbyname(ntStr::GetString(host));
...
// --------------------------------------------------------------------------------
CKeyString netsocket::gethostname() const
{
	...
	{
		return CKeyString(inet_ntoa(hostAddress.sin_addr));
	}
	return CKeyString();
}
...
//    - already connected
// --------------------------------------------------------------------------------
bool netstream::connect(const CKeyString &hostName, int hostPort)
{
	if(isopen() == true) return false;
...
// --------------------------------------------------------------------------------
CKeyString netstream::getpeername() const
{
	sockaddr_in hostAddress;
	int         addressLength = sizeof(hostAddress);
...
// --------------------------------------------------------------------------------
CSimpleStream &netstream::operator <<(const CKeyString &data)
{
	send(ntStr::GetString(data), ntStr::GetLength(data));
...
// --------------------------------------------------------------------------------
netstream &netstream::operator >>(CKeyString &data)
{
	int iBufIndex = 0;
...
//   false otherwise
// --------------------------------------------------------------------------------
bool netserver::listen(int port, const CKeyString &adapter, int backlog)
{
	if(isopen() == true) return false;
...
bool netclient::connect(const int port, const CKeyString &targetIP, netstream &stream)
{
	if (isopen())
	{
		close();
	...
	if (getinetaddr(targetIP.GetString(), &inAddr) == false) return false;
	if (create() == false) return false;
	if (::connect(m_socket, (sockaddr*)&inAddr, sizeof(inAddr)) != 0) return false;
