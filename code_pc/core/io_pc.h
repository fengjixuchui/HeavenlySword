...
public:
	...
	virtual CSimpleStream &operator <<(const CKeyString &data) = 0;
...
public:
	...
	virtual CSimpleStream& operator <<(const CKeyString &data);
...
private:
	...
	static const uint32_t m_guiBufSize=1024;
	char		m_acBuf[ m_guiBufSize ];
	uint32_t 		m_uiCurrentPos;
	HANDLE		m_hFile;	// file descriptor
};
...
public:
	...
	uint32_t GetSize() {return m_pcEnd-m_pcStart;};
...
public:
	...
	// Constructor
	FileBuffer(const char * pcName, bool bBinary = true)
	{
		// open the file
		...
		m_pcBuffer[fileSize] = 0;

		m_iSize = (int)fileSize;
	}
