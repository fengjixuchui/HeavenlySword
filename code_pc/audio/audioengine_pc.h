...
#ifndef _AUDIOENGINE_PC_H
#define _AUDIOENGINE_PC_H
...
public:
	...
	bool Lock (void** pvBufferOut,u_long ulOffset,u_long ulSize);
	bool Unlock (void* pvBufferIn,u_long ulSize);

	bool SetPlayCursor (u_long ulPlayCursor);
	u_long GetPlayCursor ();
...
public:
	...
	void Play (u_int uiPlayCount);
	void Stop ();
	void Pause ();
	void Resume ();

	void SetVolume (float fVolume) { ntAssert(m_pobSoundBuffer); m_pobSoundBuffer->SetVolume(fVolume); }
	void SetPitch (float fPitch) { ntAssert(m_pobSoundBuffer); m_pobSoundBuffer->SetPitch(fPitch); }

	void SetPosition (const CPoint& obPosition);
	void SetMinDistance (float fMinDist);
    void SetMaxDistance (float fMaxDist);

	int	GetAllocationID () const { return m_iAllocationID; }
	
	float GetVolume () const { ntAssert(m_pobSoundBuffer); return m_pobSoundBuffer->GetVolume(); }
	float GetPitch () const { ntAssert(m_pobSoundBuffer); return m_pobSoundBuffer->GetPitch(); }
...
public:
	...
	void 			SetListenerVelocity (float fVelX,float fVelY,float fVelZ);
	void 			SetListenerOrientation (float fForwardX,float fForwardY,float fForwardZ,float fUpX,float fUpY,float fUpZ);

	// Run-time

	CAudioStream*	CreateStream (u_int uiWaveBankID,u_int uiWaveFileID,bool b3d);
	CAudioStream*	CreateStream (u_int uiWaveBankID,u_int uiWaveFileID,u_long ulChannelMask); // Prepare a sound which sends outputs to specific speakers
	CAudioStream*	CreateStream (const char* pcWaveBank,const char* pcWaveFile,bool b3d); // Create a sound instance

	bool			IsPlaying (int iAllocationID); // Check to see if a specific sound is still playing
