...
{
	u_short usFormatTag;
	WAVE_BANK_TYPE eType;
	u_long ulAlignment;
};

BUFFER_ALIGNMENT g_stBufferAlignment []=
{
	WAVE_FORMAT_PCM,			WAVE_BANK_RAM,			512,
	WAVE_FORMAT_PCM,			WAVE_BANK_HD,			512,
	WAVE_FORMAT_PCM,			WAVE_BANK_DVD,			2048,
//	WAVE_FORMAT_XBOX_ADPCM,		WAVE_BANK_RAM,			4608,
//	WAVE_FORMAT_XBOX_ADPCM,		WAVE_BANK_HD,			4608,
//	WAVE_FORMAT_XBOX_ADPCM,		WAVE_BANK_DVD,			18432,
	0,							WAVE_BANK_UNKNOWN,		0,
};

u_long CalculateBufferSize (WAVE_BANK_TYPE eType,const WAVEFORMATEX* pstFormat,int iMillisec)
{
	ntAssert(pstFormat);

	// Calculate the packetsize based on bitrate and format

	const u_long ulBitRate=pstFormat->nSamplesPerSec*(pstFormat->wBitsPerSample>>3)*pstFormat->nChannels; 	// Bitrate=Sampling rate x Bits/8 x Channels
	const u_long ulPacketSize=(ulBitRate*iMillisec)/1000;
	u_long ulFinalPacketSize=0;

	switch(pstFormat->wFormatTag)
	{
		case WAVE_FORMAT_PCM :
			ulFinalPacketSize=ulPacketSize;
			break;

//		case WAVE_FORMAT_XBOX_ADPCM :
//			ulFinalPacketSize=(ulPacketSize*36)/128; // ADPCM compresses 128 bytes to 36 bytes
//			break;
	}

	// Align our final packet size

	for(int iCount=0; g_stBufferAlignment[iCount].usFormatTag!=0; ++iCount)
	{
		if (pstFormat->wFormatTag==g_stBufferAlignment[iCount].usFormatTag &&
			eType==g_stBufferAlignment[iCount].eType)
		{
			ulFinalPacketSize=
				ulFinalPacketSize+g_stBufferAlignment[iCount].ulAlignment-(ulFinalPacketSize%g_stBufferAlignment[iCount].ulAlignment);
			break;
		}
	}

	return ulFinalPacketSize;
}
...
CWaveBank::~CWaveBank ()
{
	...
	uint8_t* pData = (uint8_t*)m_pvData;
	NT_DELETE_ARRAY( pData );
...
CWaveFile* CWaveBank::FindWaveFileP (const CKeyString& obWaveFile)
{
	for(ntstd::List<CWaveFile*>::iterator obWaveFileIt=m_obWaveFileList.begin(); obWaveFileIt!=m_obWaveFileList.end(); ++obWaveFileIt)
	{
...
CWaveFile::~CWaveFile ()
{
	uint8_t* pData = (uint8_t*)m_pvCache;
	NT_DELETE_ARRAY( pData );
...
CWaveFile* CAudioResourceManager::FindWaveFileP (const CKeyString& obWaveBank,const CKeyString& obWaveFile)
{
	for(ntstd::List<CWaveBank*>::iterator obWaveBankIt=m_obWaveBankList.begin(); obWaveBankIt!=m_obWaveBankList.end(); ++obWaveBankIt)
	{
