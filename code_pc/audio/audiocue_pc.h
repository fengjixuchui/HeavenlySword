...
#ifndef AUDIOCUE_PC_H
#define AUDIOCUE_PC_H
...
public:
	...
	CAudioCue* FindAudioCueP (u_int uiID); // Search for a wave description in this bank

	bool GetVolumeControl (const char* pcName,float fInput,float &fVolume);

	bool GetPitchControl (const char* pcName,float fInput,float &fPitch);

	bool GetParamEqControl (const char* pcName,float fInput,AUDIO_EFFECT_PARAMEQ* pstParamEq);
...
protected:
	...
	bool					ReadData (HANDLE hFileHandle);
	void					ClearData ();
