...
inline bool FwIsPow2(size_t value)
{
	return (((value & (value - 1)) == 0) && value);
}
...
//--------------------------------------------------------------------------------------------------
/**
	@brief			Returns whether a size_t value has the specified alignment
...
inline bool FwIsAligned(size_t value, size_t alignment)
{
	FW_ASSERT(FwIsPow2(alignment));
...
{
	return si_to_uint(si_clz(si_from_uint(value)));
}
...
{
	register qword v=si_from_uint(value);

	// Bit reverse each byte (4 bits lookups. we can do 5 bits actually..)
	const qword lookupHi 	= (qword) {0x00,0x08,0x04,0x0c,0x02,0x0a,0x06,0x0e,0x01,0x09,0x05,0x0d,0x03,0x0b,0x07,0x0f};
	const qword lookupLo	= (qword) {0x00,0x80,0x40,0xc0,0x20,0xa0,0x60,0xe0,0x10,0x90,0x50,0xd0,0x30,0xb0,0x70,0xf0};
	qword vBitReversed8		= si_or(si_shufb(lookupLo, lookupLo, si_andbi(v, 0x0f)), 
							  si_shufb(lookupHi, lookupHi, si_andbi(si_rotqmbii(v, 4), 0x0f)));

	// Now reverse as u32 (a classical byteswap)
	qword maskByteSwap				= (qword) {3,2,1,0,3,2,1,0,3,2,1,0,3,2,1,0};
	register qword vBitReversed32	= si_shufb(vBitReversed8, vBitReversed8, maskByteSwap);

	// Count leading zeros..
	return si_to_uchar(si_clz(vBitReversed32));
}
...
{
	return (qword)((vector unsigned int) {v1, v2, v3, v4} );
}
