...
/**
 * \addtogroup FIOSTime	Time and Deadlines
 */
/*@{*/

#define kDEADLINE_NOW      ((abstime_t)1)                      /**< The shortest possible deadline. Serviced as soon as possible. */
#define kDEADLINE_ASAP     ((abstime_t)2)                      /**< The second-shortest possible deadline. */
#define kDEADLINE_LATER    ((abstime_t)0x7FFFFFFFFFFFFFFFLL)   /**< An infinite deadline. Serviced whenever possible but without a strict deadline. */

#define kTIME_NULL         ((abstime_t)0)                      /**< The null time, which can be used as an invalid marker. */
#define kTIME_EARLIEST     ((abstime_t)1)                      /**< The earliest possible time. */
#define kTIME_LATEST       ((abstime_t)0x7FFFFFFFFFFFFFFFLL)   /**< The latest possible time. */

#define kDATE_NULL         ((datetime_t)0)                     /**< The null date, which can be used as an invalid marker. */
#define kDATE_EARLIEST     ((datetime_t)1)                     /**< The earliest possible date. */
#define kDATE_LATEST       ((datetime_t)0x7FFFFFFFFFFFFFFFULL) /**< The latest possible date. */
#define kDATE_HIGHBIT      ((datetime_t)0x8000000000000000ULL) /**< The high bit of the date, which is reserved for future expansion. */
...
*/
inline I64 FIOSAbstimeToNanoseconds(abstime_t abstime) { return platform::timeToNanoseconds(abstime); }
...
*/
inline abstime_t	FIOSNanosecondsToAbstime(U64 nanos) { return platform::nanosecondsToTime(nanos); }
...
*/
inline abstime_t FIOSRelativeNanoseconds(U64 nanos) { return FIOSGetCurrentTime() + FIOSNanosecondsToAbstime(nanos); }
...
*/
inline I64 FIOSAbstimeToMicroseconds(abstime_t abstime) { return ((FIOSAbstimeToNanoseconds(abstime) + 999) / 1000); }
...
/**
	...
	\return                 The corresponding value as an abstime_t.
*/
inline abstime_t FIOSMicrosecondsToAbstime(U64 mics) { return FIOSNanosecondsToAbstime((U64)((mics)*(U64)1000)); }
...
*/
inline abstime_t FIOSRelativeMicroseconds(U64 mics) { return (FIOSGetCurrentTime() + FIOSMicrosecondsToAbstime(mics)); }
...
*/
inline I64 FIOSAbstimeToMilliseconds(abstime_t abstime) { return ((FIOSAbstimeToNanoseconds(abstime) + 999999) / 1000000); }
...
/**
	...
	\return                 The corresponding value as an abstime_t.
*/
inline abstime_t FIOSMillisecondsToAbstime(U64 mills) { return FIOSNanosecondsToAbstime((U64)((mills)*(U64)1000000)); }
...
*/
inline abstime_t FIOSRelativeMilliseconds(U64 mills) { return (FIOSGetCurrentTime() + FIOSMillisecondsToAbstime(mills)); }
...
*/
inline I64 FIOSAbstimeToSeconds(abstime_t abstime) { return ((FIOSAbstimeToNanoseconds(abstime) + 999999999) / 1000000000); }
...
/**
	...
	\return                 The corresponding value as an abstime_t.
*/
inline abstime_t FIOSSecondsToAbstime(U64 secs) { return FIOSNanosecondsToAbstime((U64)((secs)*(U64)1000000000)); }
...
*/
inline abstime_t FIOSRelativeSeconds(U64 secs) { return (FIOSGetCurrentTime() + FIOSSecondsToAbstime(secs)); }
...
/**
	...
	The native date representation is defined in the appropriate fios_platform_imp_*.h file for the platform.
	...
	2006-04-13 00:00:00.000 by one platform, while another platform might represent
	the same value as 2006-04-13 00:00:01.000 -- one second off. However, these
	values will be essentially "correct" for most intents and purposes. If used on the
	same system, the values will be entirely consistent. If the values are sent to
	another system, there may be a small clock skew.
...
/** \brief Finds the difference between two dates in nanoseconds.
	...
	\return      The difference between the dates in nanoseconds, ie: dateA - dateB. If either date is invalid (eg, high bit is set) then zero is returned.
*/
inline I64 FIOSDateDifference(datetime_t dateB, datetime_t dateA) {
	return ((dateA | dateB) & kDATE_HIGHBIT) ? 0:(I64(dateA) - I64(dateB));
}

/** \brief Adds nanoseconds to a date.
	\param[in]   date         The starting date.
	\param[in]   nanos        Number of nanoseconds to add. */
inline datetime_t FIOSDateAdd(datetime_t date, I64 nanos) {
