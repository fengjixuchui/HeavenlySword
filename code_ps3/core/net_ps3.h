...
public:
	...
	bool   close();
	bool   isopen() const;
	CHashedString gethostname() const;
...
public:
	...
	CHashedString getpeername() const;
	...
	//CSimpleStream &operator <<(const CHashedString &data);
	//netstream &operator >>(      CHashedString &data);
...
public:
	...
	bool listen(int port, const char* adapter = NULL, int backlog = 5);
...
public:
	...
	bool connect(const int port, const char* targetIP, netstream &stream);
};
