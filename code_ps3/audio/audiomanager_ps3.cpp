...
void CAudioObject::SetPosition (const CPoint& obPosition)
{
	...
	{
		m_pobAudioStream->SetPosition(obPosition);
		m_pobTransform=0;
	}
}
...
*
***************************************************************************************************/
void CAudioObject::SetName (CHashedString obSoundCue, CHashedString obSoundBank)
{
	...
	m_uiMaximum3dSounds(48)
{
	...
	NinjaLua::LuaObject obAudioEventsTbl = NinjaLua::LuaObject::CreateTable(CLuaGlobal::Get().State());
	obGlobals.Set("AudioEvents", obAudioEventsTbl);
...
bool CAudioManager::Create (CAudioObjectID& obID,const char* pcString,SPATIALIZATION eSpatialType)
{
	...
	CHashedString obBank(acSoundBank);
	CHashedString obCue(acSoundCue);
...
bool CAudioManager::Create (CAudioObjectID& obID,const CHashedString& obBank,const CHashedString& obCue,SPATIALIZATION eSpatialType)
{
	if (!m_bEnabled) // Ignore if AudioManager is disabled
	{
		obID.Invalidate();
...
{
	CSoundBank* pobBank=FindSoundBankP(obBank.GetHash());

	if (pobBank)
	{
		if (pobBank->FindAudioCueP(obCue.GetHash()))
			return true;
	}
