...
#ifdef PLATFORM_PS3
extern void* LegacyMiscMalloc( size_t size );
extern void LegacyMiscFree( void* ptr );
#else
...
//! ATTN! this is DIRTY. xmlparse relies on realloc to copy
//! the contents of the old allocation to the new. our memory
//! allocators do not support realloc, and xmlparse does not track
//! the size of old allocations. hence we copy newsize, regardless
//! of whether is was ours or not! 
//! This can only work here as we've hacked the 'misc' pool to NEVER
//! sit adjacant to any OS memory, and thus never trip an invalid page access.
//! NEVER EVER DO THIS YOURSELF.
void* ReallocateAndCopy( void* pOldAlloc, size_t size )
{
#ifdef PLATFORM_PC
	return realloc(pOldAlloc,size);
#else
	// just free
	if (size==0)
	{
		LegacyMiscFree( pOldAlloc );
		return NULL;
	}

	// we donot free if old allocation failed
	void* pResult = LegacyMiscMalloc( size );
	if (pResult == NULL)
	{
		return NULL;
	}

	memcpy( pResult, pOldAlloc, size );
	
	// release old alloc, return new one
	LegacyMiscFree( pOldAlloc );
	return pResult;
#endif
}

void* LegacyCalloc( size_t num, size_t size )
{
#ifdef PLATFORM_PC
	return calloc(num,size);
#else
	size_t toAllocate = num * size;
	void* pResult = LegacyMiscMalloc( toAllocate );
	memset( pResult, 0, toAllocate );
	return pResult;
#endif
}
...
#define parseEnduint8_tIndex (((Parser *)parser)->m_parseEnduint8_tIndex)
...
{
  XML_Parser parser = LegacyMiscMalloc(sizeof(Parser));
  if (!parser)
    return parser;
  ...
  atts = LegacyMiscMalloc(attsSize * sizeof(ATTRIBUTE));
  nSpecifiedAtts = 0;
  dataBuf = LegacyMiscMalloc(INIT_DATA_BUF_SIZE * sizeof(XML_Char));
  groupSize = 0;
  groupConnector = 0;
...
#endif /* XML_DTD */
  dtdDestroy(&dtd);
  LegacyMiscFree((void *)atts);
  LegacyMiscFree(groupConnector);
  LegacyMiscFree(buffer);
  LegacyMiscFree(dataBuf);
  LegacyMiscFree(unknownEncodingMem);
  if (unknownEncodingRelease)
    unknownEncodingRelease(unknownEncodingData);
  LegacyMiscFree(parser);
}
...
int XML_Parse(XML_Parser parser, const char *s, int len, int isFinal)
{
  if (len == 0) {
	...
	buffer = buffer == 0 ? LegacyMiscMalloc(len * 2) : ReallocateAndCopy(buffer, len * 2);
...
void *XML_GetBuffer(XML_Parser parser, int len)
{
  if (len > bufferLim - bufferEnd) {
	...
	bufferSize *= 2;
      ...
      newBuf = LegacyMiscMalloc(bufferSize);
      if (newBuf == 0) {
	errorCode = XML_ERROR_NO_MEMORY;
	return 0;
      }
...
long XML_GetCurrentuint8_tIndex(XML_Parser parser)
{
  if (eventPtr)
    return parseEnduint8_tIndex - (parseEndPtr - eventPtr);
  return -1;
}

int XML_GetCurrentuint8_tCount(XML_Parser parser)
{
...
const XML_LChar *XML_ErrorString(int code)
{
  static const XML_LChar *message[] = {
    ...
    XML_T("ntError in processing external entity reference"),
  ...
  for (;;) {
	...
	*nextPtr = s;
	...
	if (freeTagList) {
	  ...
	  tag = LegacyMiscMalloc(sizeof(TAG));
	  if (!tag)
	    return XML_ERROR_NO_MEMORY;
	  tag->buf = LegacyMiscMalloc(INIT_TAG_BUF_SIZE);
	  if (!tag->buf)
	    return XML_ERROR_NO_MEMORY;
	...
	if (nextPtr) {
	  ...
	  if (tag->rawNameLength + (int)(sizeof(XML_Char) - 1) + (int)sizeof(XML_Char) > tag->bufEnd - tag->buf) {
	    ...
	    tag->buf = ReallocateAndCopy(tag->buf, bufSize);
	    if (!tag->buf)
	      return XML_ERROR_NO_MEMORY;
				...
				BINDING **bindingsPtr)
{
  ...
  if (n + nDefaultAtts > attsSize) {
    ...
    atts = ReallocateAndCopy((void *)atts, attsSize * sizeof(ATTRIBUTE));
    if (!atts)
      return XML_ERROR_NO_MEMORY;
    if (n > oldAttsSize)
  ...
  if (n > binding->uriAlloc) {
    TAG *p;
    XML_Char *uri = LegacyMiscMalloc((n + EXPAND_SPARE) * sizeof(XML_Char));
    if (!uri)
      return XML_ERROR_NO_MEMORY;
...
int addBinding(XML_Parser parser, PREFIX *prefix, const ATTRIBUTE_ID *attId, const XML_Char *uri, BINDING **bindingsPtr)
{
  ...
  if (freeBindingList) {
    b = freeBindingList;
    if (len > b->uriAlloc) {
      b->uri = ReallocateAndCopy(b->uri, sizeof(XML_Char) * (len + EXPAND_SPARE));
      if (!b->uri)
	return 0;
    ...
    b = LegacyMiscMalloc(sizeof(BINDING));
    if (!b)
      return 0;
    b->uri = LegacyMiscMalloc(sizeof(XML_Char) * (len + EXPAND_SPARE));
    if (!b->uri) {
      LegacyMiscFree(b);
      return 0;
    }
...
handleUnknownEncoding(XML_Parser parser, const XML_Char *encodingName)
{
  if (unknownEncodingHandler) {
    ...
    if (unknownEncodingHandler(unknownEncodingHandlerData, encodingName, &info)) {
      ENCODING *enc;
      unknownEncodingMem = LegacyMiscMalloc(XmlSizeOfUnknownEncoding());
	 ...
	 const char **nextPtr)
{
  ...
  for (;;) {
	...
	*eventEndPP = s;
	...
	{
		groupSize *= 2;
		groupConnector = ReallocateAndCopy(groupConnector, groupSize);
	}
	else
	  groupConnector = LegacyMiscMalloc(groupSize = 32);
	if (!groupConnector)
	  return XML_ERROR_NO_MEMORY;
      }
...
defineAttribute(ELEMENT_TYPE *type, ATTRIBUTE_ID *attId, int isCdata, int isId, const XML_Char *value)
{
  ...
  if (type->nDefaultAtts == type->allocDefaultAtts) {
    if (type->allocDefaultAtts == 0) {
      ...
      type->defaultAtts = LegacyMiscMalloc(type->allocDefaultAtts*sizeof(DEFAULT_ATTRIBUTE));
    }
    else {
      type->allocDefaultAtts *= 2;
      type->defaultAtts = ReallocateAndCopy(type->defaultAtts, type->allocDefaultAtts*sizeof(DEFAULT_ATTRIBUTE));
    }
    if (!type->defaultAtts)
      return 0;
  }
...
{
  DTD tem;
  NT_MEMCPY(&tem, p1, sizeof(DTD));
  NT_MEMCPY(p1, p2, sizeof(DTD));
  NT_MEMCPY(p2, &tem, sizeof(DTD));
}
...
The NT_NEW DTD has already been initialized. */
...
static int dtdCopy(DTD *newDtd, const DTD *oldDtd)
{
  ...
  for (;;) {
    ...
    if (!oldA)
      break;
    /* Remember to allocate the scratch uint8_t before the name. */
  ...
  for (;;) {
    ...
    if (oldE->nDefaultAtts) {
      newE->defaultAtts = (DEFAULT_ATTRIBUTE *)LegacyMiscMalloc(oldE->nDefaultAtts * sizeof(DEFAULT_ATTRIBUTE));
      if (!newE->defaultAtts)
	return 0;
    }
    if (oldE->idAtt)
...
NAMED *lookup(HASH_TABLE *table, KEY name, size_t createSize)
{
  ...
  if (table->size == 0) {
    if (!createSize)
      return 0;
    table->v = LegacyCalloc(INIT_SIZE, sizeof(NAMED *));
    if (!table->v)
      return 0;
    ...
    if (table->used == table->usedLim) {
      ...
      NAMED **newV = LegacyCalloc(newSize, sizeof(NAMED *));
      if (!newV)
	return 0;
  ...
  table->v[i] = LegacyCalloc(1, createSize);
  if (!table->v[i])
    return 0;
  table->v[i]->name = name;
  (table->used)++;
...
int poolGrow(STRING_POOL *pool)
{
  ...
  if (pool->blocks && pool->start == pool->blocks->s) {
    ...
    pool->blocks = ReallocateAndCopy(pool->blocks, offsetof(BLOCK, s) + blockSize * sizeof(XML_Char));
    if (!pool->blocks)
      return 0;
    ...
    if (blockSize < INIT_BLOCK_SIZE)
      ...
      blockSize *= 2;
    tem = LegacyMiscMalloc(offsetof(BLOCK, s) + blockSize * sizeof(XML_Char));
    if (!tem)
      return 0;
    tem->size = blockSize;
