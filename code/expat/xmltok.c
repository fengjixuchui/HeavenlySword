...
/* A 2 uint8_t UTF-8 representation splits the characters 11 bits
between the bottom 5 and 6 bits of the Bytes.
We need 8 bits to index into pages, 3 bits to add to that index and
5 bits to generate the mask. */
#define UTF8_GET_NAMING2(pages, uint8_t) \
    (namingBitmap[((pages)[(((uint8_t)[0]) >> 2) & 7] << 3) \
                      + ((((uint8_t)[0]) & 3) << 1) \
                      + ((((uint8_t)[1]) >> 5) & 1)] \
         & (1 << (((uint8_t)[1]) & 0x1F)))

/* A 3 uint8_t UTF-8 representation splits the characters 16 bits
between the bottom 4, 6 and 6 bits of the Bytes.
We need 8 bits to index into pages, 3 bits to add to that index and
5 bits to generate the mask. */
#define UTF8_GET_NAMING3(pages, uint8_t) \
  (namingBitmap[((pages)[((((uint8_t)[0]) & 0xF) << 4) \
                             + ((((uint8_t)[1]) >> 2) & 0xF)] \
		       << 3) \
                      + ((((uint8_t)[1]) & 3) << 1) \
                      + ((((uint8_t)[2]) >> 5) & 1)] \
...
#ifdef XML_MIN_SIZE
  int (*uint8_tType)(const ENCODING *, const char *);
  int (*isNameMin)(const ENCODING *, const char *);
  int (*isNmstrtMin)(const ENCODING *, const char *);
  int (*uint8_tToAscii)(const ENCODING *, const char *);
...
#define BYTE_TYPE(enc, p) \
 (((const struct normal_encoding *)(enc))->uint8_tType(enc, p))
#else
...
#define BYTE_TO_ASCII(enc, p) \
 (((const struct normal_encoding *)(enc))->uint8_tToAscii(enc, p))
static
...
enum {  /* UTF8_cvalN is value of masked first uint8_t of N uint8_t sequence */
  UTF8_cval1 = 0x00,
  UTF8_cval2 = 0xc0,
  UTF8_cval3 = 0xe0,
  UTF8_cval4 = 0xf0
...
static int unicode_uint8_t_type(char hi, char lo)
{
...
#define LITTLE2_uint8_t_TYPE(enc, p) \
 ((p)[1] == 0 \
  ? ((struct normal_encoding *)(enc))->type[(unsigned char)*(p)] \
  : unicode_uint8_t_type((p)[1], (p)[0]))
#define LITTLE2_uint8_t_TO_ASCII(enc, p) ((p)[1] == 0 ? (p)[0] : -1)
...
static
int little2_uint8_tType(const ENCODING *enc, const char *p)
{
  return LITTLE2_uint8_t_TYPE(enc, p);
}

static
int little2_uint8_tToAscii(const ENCODING *enc, const char *p)
{
  return LITTLE2_uint8_t_TO_ASCII(enc, p);
}
...
#define BYTE_TYPE(enc, p) LITTLE2_uint8_t_TYPE(enc, p)
#define BYTE_TO_ASCII(enc, p) LITTLE2_uint8_t_TO_ASCII(enc, p) 
...
#define BIG2_uint8_t_TYPE(enc, p) \
 ((p)[0] == 0 \
  ? ((struct normal_encoding *)(enc))->type[(unsigned char)(p)[1]] \
  : unicode_uint8_t_type((p)[0], (p)[1]))
#define BIG2_uint8_t_TO_ASCII(enc, p) ((p)[0] == 0 ? (p)[1] : -1)
...
static
int big2_uint8_tType(const ENCODING *enc, const char *p)
{
  return BIG2_uint8_t_TYPE(enc, p);
}

static
int big2_uint8_tToAscii(const ENCODING *enc, const char *p)
{
  return BIG2_uint8_t_TO_ASCII(enc, p);
}
...
#define MINBPC(enc) 2
/* CHAR_MATCHES is guaranteed to have MINBPC Bytes available. */
#define BYTE_TYPE(enc, p) BIG2_uint8_t_TYPE(enc, p)
#define BYTE_TO_ASCII(enc, p) BIG2_uint8_t_TO_ASCII(enc, p) 
...
{
  enum {
    /* minN is minimum legal resulting value for N uint8_t sequence */
    min2 = 0x80,
    min3 = 0x800,
    min4 = 0x10000
  };
	     ...
	     const char **nextTokPtr)
{
  ...
  encPtr = enc->encPtr;
  if (ptr + 1 == end) {
    /* only a single uint8_t available for auto-detection */
#ifndef XML_DTD /* FIXME */
    /* a well-formed document entity must have more than one uint8_t */
    ...
    case 0xFE:
    case 0xFF:
    case 0xEF: /* possibly first uint8_t of UTF-8 BOM */
	...
	*encPtr = encodingTable[UTF_16BE_ENC];
	...
	/* We could recover here in the case:
	   ...
	   presented just with a single uint8_t, we couldn't reliably determine
