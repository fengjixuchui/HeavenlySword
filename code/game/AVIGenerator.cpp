...
CAVIGenerator::CAVIGenerator(LPCTSTR sFileName, CView* pView, uint32_t dwRate)
...
CAVIGenerator::CAVIGenerator(LPCTSTR sFileName, LPBITMAPINFOHEADER lpbih, uint32_t dwRate)
: m_sFile(sFileName), m_dwRate(dwRate),
m_pAVIFile(NULL), m_pStream(NULL), m_pStreamCompressed(NULL)
{
		MakeExtAvi();
...
CAVIGenerator::~CAVIGenerator()
{
	// Just checking tha all allocated ressources have been released
	ntAssert(m_pStream==NULL);
	ntAssert(m_pStreamCompressed==NULL);
	ntAssert(m_pAVIFile==NULL);
}

void CAVIGenerator::SetBitmapHeader(LPBITMAPINFOHEADER lpbih)
{
	// checking that bitmap size are multiple of 4
	ntAssert(lpbih->biWidth%4==0);
	ntAssert(lpbih->biHeight%4==0);

	// copying bitmap info structure.
	// corrected thanks to Lori Gardi
	NT_MEMCPY(&m_bih,lpbih, sizeof(BITMAPINFOHEADER));
...
//	AVICOMPRESSOPTIONS FAR * aopts[1] = {&opts};
	...
	uint32_t wVer = HIWORD(VideoForWindowsVersion());
	if (wVer < 0x010a)
	{
	...
	if (hr != AVIERR_OK)
	{
		...
		case AVIERR_FILEREAD:
			m_sError+=_T("A disk ntError occurred while reading the file."); 
			break;
		case AVIERR_FILEOPEN:		
			m_sError+=_T("A disk ntError occurred while opening the file.");
			break;
...
//	}
	...
	if (hr != AVIERR_OK)
	{
		...
		case AVIERR_UNSUPPORTED:
			m_sError+=_T("Compression is not supported for this type of data. This ntError might be returned if you try to compress data that is not audio or video.");
			break;
		}
...
HRESULT CAVIGenerator::AddFrame(uint8_t *bmBits)
{
	HRESULT hr;
