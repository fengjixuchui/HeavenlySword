...
#define TRAVERSE_POLY( a, b, c, d )		if (traverse_polygon( (a), (b), (c), (d) ) == -666) {return -666;} 
     ...
     int v1)
{
  ...
  if (get_vertex_positions(v0, v1, &ip, &iq) == -1)
  {
	  return -1;
  }
  ...
  ntPrintf( "make_poly: mcur = %d, (v0, v1) = (%d, %d)\n", 
	  mcur, v0, v1);
  ntPrintf( "next posns = (p, q) = (%d, %d)\n", p, q);
#endif
     ...
     int n)
{
  ...
  if (tr[tr_start].u0 > 0)
  {
    TRAVERSE_POLY(0, tr_start, tr[tr_start].u0, TR_FROM_UP);
  }
  else if (tr[tr_start].d0 > 0)
  {
    TRAVERSE_POLY(0, tr_start, tr[tr_start].d0, TR_FROM_DN);
  }
	  ...
	  if (from == t->d1)
	    {
		  ...
		  if (mnew == -1)	{return -666;}
	      TRAVERSE_POLY(mcur, t->d1, trnum, TR_FROM_UP);
	      TRAVERSE_POLY(mnew, t->d0, trnum, TR_FROM_UP);	    
	    }
	  else
	    {
	      mnew = make_new_monotone_poly(mcur, v0, v1);
		  if (mnew == -1)	{return -666;}
	      TRAVERSE_POLY(mcur, t->d0, trnum, TR_FROM_UP);
	      TRAVERSE_POLY(mnew, t->d1, trnum, TR_FROM_UP);
	    }
	}
      else
	{
	  retval = SP_NOSPLIT;	/* Just traverse all neighbours */
	  TRAVERSE_POLY(mcur, t->u0, trnum, TR_FROM_DN);
	  TRAVERSE_POLY(mcur, t->u1, trnum, TR_FROM_DN);
	  TRAVERSE_POLY(mcur, t->d0, trnum, TR_FROM_UP);
	  TRAVERSE_POLY(mcur, t->d1, trnum, TR_FROM_UP);
	}
    }
	  ...
	  if (from == t->u1)
	    {
		  ...
		  if (mnew == -1)	{return -666;}
	      TRAVERSE_POLY(mcur, t->u1, trnum, TR_FROM_DN);
	      TRAVERSE_POLY(mnew, t->u0, trnum, TR_FROM_DN);	    
	    }
	  else
	    {
	      mnew = make_new_monotone_poly(mcur, v0, v1);
		  if (mnew == -1)	{return -666;}
	      TRAVERSE_POLY(mcur, t->u0, trnum, TR_FROM_DN);
	      TRAVERSE_POLY(mnew, t->u1, trnum, TR_FROM_DN);
	    }
	}
      else
	{
