...
static int newnode()
{
  if (q_idx < QSIZE)
    ...
    {
      ntPrintf( "newnode: Query-table overflow\n");
      return -1;
    }
}
...
static int newtrap()
{
  if (tr_idx < TRSIZE)
    ...
    {
      ntPrintf( "newtrap: Trapezoid-table overflow\n");
      return -1;
    }
}
...
static int init_query_structure( int segnum )
{
  ...
  tr[t4].hi.y = (double) (MAX_POS_FLOAT);
  tr[t4].hi.x = (double) (MAX_POS_FLOAT);
  tr[t3].lo.y = (double) -1* (MAX_POS_FLOAT);
  tr[t3].lo.x = (double) -1* (MAX_POS_FLOAT);
     ...
     int nseg)
{
  ...
  for (i = 1; i <= nseg; i++)
	...
	for (h = 1; h <= math_logstar_n(nseg); h++)
	{
		...
		{
			returnValue = add_segment(choose_segment());
			if (returnValue)
			{
				return returnValue;
			}
