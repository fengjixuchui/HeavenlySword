...
//!
//------------------------------------------------------
ArmyChap5_1::ArmyChap5_1( const ArmyBattlefield* pArmyArena ) :
	m_bDoneConstruct( false ),
	m_iLastExplodeCount(0)
{
	m_pArmyArena = pArmyArena;
	ntAssert( m_pArmyArena );
}

void ArmyChap5_1::CreateSmallSquare( const ArmyBattalion* pBattalionDef, const BattlefieldHeader* pBattlefieldHeader, Battalion* pBattalion, TempGruntVector& grunts )
{
...
void ArmyChap5_1::CreateMediumSquare( const ArmyBattalion* pBattalionDef, const BattlefieldHeader* pBattlefieldHeader, Battalion* pBattalion, TempGruntVector& grunts )
{
...
void ArmyChap5_1::CreateLargeSquare( const ArmyBattalion* pBattalionDef, const BattlefieldHeader* pBattlefieldHeader, Battalion* pBattalion, TempGruntVector& grunts )
{
...
void ArmyChap5_1::CreateSmallRectangle( const ArmyBattalion* pBattalionDef, const BattlefieldHeader* pBattlefieldHeader, Battalion* pBattalion, TempGruntVector& grunts )
{
...
void ArmyChap5_1::CreateMediumRectangle( const ArmyBattalion* pBattalionDef, const BattlefieldHeader* pBattlefieldHeader, Battalion* pBattalion, TempGruntVector& grunts )
{
...
void ArmyChap5_1::CreateLargeRectangle( const ArmyBattalion* pBattalionDef, const BattlefieldHeader* pBattlefieldHeader, Battalion* pBattalion, TempGruntVector& grunts )
{
...
void ArmyChap5_1::Create_bombermen( const ArmyBattalion* pBattalionDef, const BattlefieldHeader* pBattlefieldHeader, Battalion* pBattalion, TempGruntVector& grunts )
{
	...
	pBattalion->m_ShapeData.m_UnorderedSquare.m_NumGrunts = NUM_BOMBS;
	pBattalion->m_ShapeData.m_UnorderedSquare.m_UnitType[0] = NIBBLE_PACK( BUT_BOMBERMAN , 0 );
...
void ArmyChap5_1::SectionConstruct()
{
	// FROM THE SCRIPT 
		 ...
		 bdlIt != m_pArmyArena->m_ArmyBattalionDefs.end(); ++bdlIt, ++i )
	{
		...
		} else if( pBattalionDef->m_Command == CHashedString("bombermen") )
		{
			Create_bombermen( pBattalionDef, &battlefieldHeader, pBattalion, tmpGrunts );
		}
		else
		{
...
//!
//------------------------------------------------------
void ArmyChap5_1::Update( float fTimeStep )
{
	...
	// send a message every NUM_BOMBS explodey peeps exploding
	if( m_pBattlefield->m_pHeader->m_eaInfo->m_iExplodeCount >= (m_iLastExplodeCount+NUM_BOMBS) )
	{
		m_pArmyArena->m_pMessageHub->GetMessageHandler()->ProcessEvent( "OnDeath" );
		m_iLastExplodeCount = m_pBattlefield->m_pHeader->m_eaInfo->m_iExplodeCount;
	}
...
//!
//------------------------------------------------------
void ArmyChap5_1::ProcessGlobalEvent( const CHashedString obGlobalEvent )
{
