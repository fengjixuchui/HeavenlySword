...
//! 
//! \filename core\ntstd_vector.h
//! Complete copy of <vector> from the MS implementation of the C++ standard library
//! with one exception: "reserve" passes an argument by constant reference instead of
//! by value like the original MS implementation. Passing by value fails to compile with
//! aligned objects.
//! 
//-----------------------------------------------------------------------------------------

// Make sure we're on PC and compiling under an MS compiler.
#ifndef PLATFORM_PC
#	error aThis file is for PC only builds to get around MS STL not supporting aligned objects contained within std::vector.
#endif

// 1310 is VC7.1
#if ( MSC_VER >= 1310 )
#	error bThis file is for PC only builds to get around MS STL not supporting aligned objects contained within std::vector.
#endif

// vector standard header

// removed pragma once to stop incredibuild warnings
//#pragma once

#ifndef NTSTD_VECTOR_
#define NTSTD_VECTOR_
#include <memory>
#include <stdexcept>
