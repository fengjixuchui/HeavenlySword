//------------------------------------------------------------------------------------------
//!
//!	\file rotscreensprite.cpp
//! A rotated screen space quad sprite with a single texture and colour
//!
//------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------
//!
//!	RotScreenSprite::ctor
//!
//------------------------------------------------------------------------------------------
RotScreenSprite::RotScreenSprite( int iNumberOfSprites )
{
	...
	m_spriteData.GetGPUData().PushVertexElement( VD_STREAM_TYPE_HALF4,	VE_COLOUR,		"IN.colour" );
	m_spriteData.GetGPUData().PushVertexElement( VD_STREAM_TYPE_FLOAT2,	VE_ANGLE,		"IN.angle" );
...
#ifdef PLATFORM_PC	
	m_pVertexShader = DebugShaderCache::Get().LoadShaderFILE( SHADERTYPE_VERTEX, "fxshaders/rotscreensprite_vs.hlsl", "main", "vs_2_0" );
	m_pPixelShader = DebugShaderCache::Get().LoadShaderFILE( SHADERTYPE_PIXEL, "fxshaders/screensprite_ps.hlsl", "main", "ps_2_0" );
#elif defined(PLATFORM_PS3)
	m_pVertexShader = DebugShaderCache::Get().LoadShader( "rotscreensprite_vp.sho" );
...
//------------------------------------------------------------------------------------------
//!
//!	RotScreenSprite::SetTexture
//!
//------------------------------------------------------------------------------------------
void RotScreenSprite::SetTexture( const CKeyString& pName )
{
	SetTexture( TextureManager::Get().LoadTexture_Neutral( ntStr::GetString(pName) ) );
}

void RotScreenSprite::SetTexture( Texture::Ptr pTex )
{
	m_texture = pTex;
	m_bBilinear = GFXFormat::Is32F( m_texture->GetFormat() ) ? false : true;	
}

void RotScreenSprite::SetUV( float fTop, float fLeft, float fBottom, float fRight, int iItem )
{
...
//------------------------------------------------------------------------------------------
//!
//!	RotScreenSprite::SetSize
//! methods to set the size(s) of a sprite / sprites
//!
//------------------------------------------------------------------------------------------
void RotScreenSprite::SetSize( float fSize, int iItem )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex( iItem*4 );
...
void RotScreenSprite::SetWidth( float fWidth, int iItem )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex( iItem*4 );
...
void RotScreenSprite::SetHeight( float fHeight, int iItem )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex( iItem*4 );
...
void RotScreenSprite::SetSizeAll( float fSize )
{
...
//------------------------------------------------------------------------------------------
//!
//!	RotScreenSprite::SetColour
//! methods to set the colour(s) of a sprite / sprites
//!
//------------------------------------------------------------------------------------------
void RotScreenSprite::SetColour( uint32_t iCol, int iItem )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex( iItem*4 );
...
void RotScreenSprite::SetColourAll( uint32_t iCol )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex(0);
	for (u_int i = 0; i < m_spriteData.GetGPUData().GetMaxVertices(); i++, pSprite++)
		pSprite->SetCol(iCol);
...
void RotScreenSprite::SetColour( const CVector& col, int iItem )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex( iItem*4 );
...
void RotScreenSprite::SetColourAll( const CVector& col )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex(0);
	for (u_int i = 0; i < m_spriteData.GetGPUData().GetMaxVertices(); i++, pSprite++)
		pSprite->SetCol(col);
...
//!
//------------------------------------------------------------------------------------------
void RotScreenSprite::SetPosition( const CPoint& pos, int iItem )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex( iItem*4 );
...
void RotScreenSprite::SetAngle( const float fAngle, int iItem )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex( iItem*4 );

	// ensure angle sits between -PI and +PI
	float a = fAngle / (2*PI);
	a = (a - floorf(a)) * 2*PI;

	pSprite[0].SetAngle(a);
	pSprite[1].SetAngle(a);
	pSprite[2].SetAngle(a);
	pSprite[3].SetAngle(a);
}

//------------------------------------------------------------------------------------------
//!
//!	RotScreenSprite::Render
...
//------------------------------------------------------------------------------------------
//!
//!	RotScreenSprite::GetColourAll
//! 
//! Returns uint32_t colour values stored in the first vertex.
//!
//------------------------------------------------------------------------------------------
void RotScreenSprite::GetColourAll( uint32_t& iCol )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex(0);
	pSprite->GetCol( iCol );
}
...
//!
//------------------------------------------------------------------------------------------
void RotScreenSprite::GetColourAll( CVector& obColour )
{
	Vertex* pSprite = (Vertex*)m_spriteData.GetGPUData().GetVertex(0);
