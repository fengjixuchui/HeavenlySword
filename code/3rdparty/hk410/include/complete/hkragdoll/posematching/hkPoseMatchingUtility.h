...
class hkPoseMatchingUtility
{

public:

	HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_ANIMATION, hkPoseMatchingUtility);
	...
	hkPoseMatchingUtility( hkInt16 rootBoneIdx, hkInt16 otherBoneIdx, hkInt16 anotherBoneIdx, const hkVector4& upAxisMS );
	...
	int addCandidatePose(const hkQsTransform* poseModelSpace, const hkAnimationBinding* binding, hkReal time);
	...
	{	
		...
		const hkAnimationBinding* m_binding;	// A copy of the binding for the original animation
