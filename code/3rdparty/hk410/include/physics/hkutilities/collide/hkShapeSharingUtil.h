...
class hkShapeSharingUtil
{
		...
		*/
		...
		struct Options
		{
			...
			float m_equalityThreshold;
				...
				/// If true (default), the utility will check for permutations of vertices/triangles/planes when comparing hkSimpleMeshShape or
				/// hkConvexVerticesShape objects. This will possibly detect more instanced shapes but due to the increased amount of comparisons
		...
		static hkResult HK_CALL shareShapes (hkArray<hkRigidBody*>& rigidBodies, const Options& options, Results& resultsOut);

			/// Given a rigid body, it will try to change its shape hierarchy so cloned shapes (identical shapes) are
			/// shared (instanced) rather than duplicated. 
		static hkResult HK_CALL shareShapes (hkRigidBody* rigidBody, const Options& options, Results& resultsOut);
	...
	private:
		...
		static const hkShape* replaceShapesRecursively (const hkShape* shape, class _ShapeReplacementData& shapeReplacementData);
};
