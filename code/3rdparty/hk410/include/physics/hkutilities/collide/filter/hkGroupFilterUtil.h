...
class hkGroupFilterUtil
{
	public:
			/// This utility class helps setting up hkCollidable::m_collisionFilterInfo for the group filter.
			...
			///       calling hkGroupFilter::getNewSystemGroup(). This system group will by applied to the m_collisionFilterInfo
			///       of all involved bodies. If the bodies already have a system group assigned to, this function will assert.
		static void HK_CALL disableCollisionsBetweenConstraintBodies( const class hkConstraintInstance*const* constraints, int numConstraints, int groupFilterSystemGroup );
};
