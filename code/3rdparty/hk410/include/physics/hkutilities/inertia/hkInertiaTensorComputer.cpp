...
static void HK_CALL computeRecursiveShapeVolumeMassProperties(const hkShape* shape, const hkTransform& transform, hkMassProperties& massPropertiesOut);
...
static void HK_CALL scaleMassProperties(hkReal scale, hkMassProperties& massProperties);
...
hkResult HK_CALL hkInertiaTensorComputer::computeBoxSurfaceMassProperties(const hkVector4& halfExtents, hkReal mass, hkReal surfaceThickness, hkMassProperties& result)
{
...
hkResult HK_CALL hkInertiaTensorComputer::computeTriangleSurfaceMassProperties(const hkVector4& v0, const hkVector4& v1, const hkVector4& v2, hkReal mass, hkReal surfaceThickness, hkMassProperties& result)
{
...
hkResult HK_CALL hkInertiaTensorComputer::computeBoxVolumeMassProperties(const hkVector4& halfExtents, hkReal mass, hkMassProperties &result)
{
...
hkResult HK_CALL hkInertiaTensorComputer::computeVertexHullVolumeMassProperties(const hkReal* vertexIn, int striding, int numVertices, hkReal mass, hkMassProperties &result)
{
	...
	hkGeometryUtility::createConvexGeometry( stridedVerts, g, planeEquations);

	hkInertiaTensorComputer::computeGeometryVolumeMassProperties(&g, mass, result);
	result.m_mass = mass;
...
hkResult HK_CALL hkInertiaTensorComputer::computeCapsuleVolumeMassProperties(const hkVector4& startAxis, const hkVector4& endAxis, hkReal radius, hkReal mass, hkMassProperties& result)
{
	...
	HK_ASSERT2(0x18f8cf41,  radius > 0.0f, "Cannot calculate capsule mass properties with zero radius or less. You may edit hkInertiaTensorComputer::computeCapsuleVolumeProperties() to bypass this assert if you are sure this is the behaviour you desire." );
	if(radius <= 0.0f)
	{
		return HK_FAILURE;
	}
...
hkResult HK_CALL hkInertiaTensorComputer::computeCylinderVolumeMassProperties(const hkVector4& startAxis, const hkVector4& endAxis, hkReal radius, hkReal mass, hkMassProperties& result)
{
	...
	HK_ASSERT2(0x18f8cf41,  radius > 0.0f, "Cannot calculate cylinder mass properties with zero radius or less. You may edit hkInertiaTensorComputer::computeCylinderVolumeProperties() to bypass this assert if you are sure this is the behaviour you desire." );
	if(radius <= 0.0f)
	{
		return HK_FAILURE;
	}
	...
	HK_ASSERT2(0x15938fa4, height > 0.0f, "Cannot calculate cylinder mass properties with zero height. You may edit hkInertiaTensorComputer::computeCylinderVolumeProperties() to bypass this assert if you are sure this is the behaviour you desire." );
...
hkResult HK_CALL hkInertiaTensorComputer::computeGeometrySurfaceMassProperties(const hkGeometry* geom, hkReal surfaceThickness, hkBool distributeUniformly, hkReal mass, hkMassProperties &result)
{
	...
	for(i = 0; i < geom->m_triangles.getSize(); i++)
	{
		...
		hkInertiaTensorComputer::computeTriangleSurfaceMassProperties(v0, v1, v2, triMass, surfaceThickness, triMassProperties);
...
void HK_CALL hkInertiaTensorComputer::setShapeVolumeMassProperties(const hkShape* shape, hkReal mass, hkRigidBodyCinfo& bodyInfo)
{
...
void HK_CALL hkInertiaTensorComputer::clipInertia( hkReal maxInertiaRatio, hkRigidBodyCinfo& bodyInfo)
{
...
void HK_CALL hkInertiaTensorComputer::computeShapeVolumeMassProperties(const hkShape* shape, hkReal mass, hkMassProperties &result)
{
...
static void HK_CALL computeRecursiveShapeVolumeMassProperties(const hkShape* shape, const hkTransform& transform, hkMassProperties& massPropertiesOut)
{
	hkMassProperties newMassProperties;
		...
		// switch statement), otherwise we recurse ( eg. hkTransformShape, hkListShape etc.)
	...
	switch (shape->getType())
	{
		...
		case HK_SHAPE_SPHERE:
		{
			...
			hkInertiaTensorComputer::computeSphereVolumeMassProperties(sphereShape->getRadius(), 1.0f, newMassProperties);
			break;
		}
		...
		case HK_SHAPE_BOX:
		{
			...
			hkInertiaTensorComputer::computeBoxVolumeMassProperties(halfExtents, 1.0f, newMassProperties);
			break;
		}
		...
		case HK_SHAPE_CONVEX_VERTICES:
		{
			const hkConvexVerticesShape* convexVerticesShape = static_cast<const hkConvexVerticesShape*>(shape);
				...
				// To get vertices, we must get spheres as there is no immediate interface to the vertices.
			hkSphereRepShape::hkCollisionSpheresInfo info;
			convexVerticesShape->getCollisionSpheresInfo( info );

			hkLocalBuffer<hkSphere> buffer( info.m_numSpheres );
			
			const hkSphere* spheres = convexVerticesShape->getCollisionSpheres(buffer.begin());

			HK_ON_DEBUG(hkResult result =) hkInertiaTensorComputer::computeVertexHullVolumeMassProperties(reinterpret_cast<const hkReal*>(spheres), sizeof(hkSphere), info.m_numSpheres, 1.0f, newMassProperties);
		...
		case HK_SHAPE_TRIANGLE:
		{
			...
			hkInertiaTensorComputer::computeTriangleSurfaceMassProperties(v0, v1, v2, 1.0f, triangleShape->getRadius(), newMassProperties);
		...
		case HK_SHAPE_CONVEX_TRANSLATE:
		{
			...
			const hkShape* childShape = ts->getChildShape();
		...
		{
			const hkMultiSphereShape* multiSphere = static_cast<const hkMultiSphereShape*>(shape);	
		...
		case HK_SHAPE_MOPP:
		{
				// All of these work on a "hkShapeCollection".
				// The LIST, MULTI_SPHERE and COLLECTION are hkShapeCollections
				// The BV_TREE "owns" a hkShapeCollection.

			const hkShapeContainer* shapeContainer;			
			if(shape->getType() == HK_SHAPE_BV_TREE || shape->getType() == HK_SHAPE_MOPP)
			{
				const hkBvTreeShape* bvTreeshape = static_cast<const hkBvTreeShape*>(shape);	
				shapeContainer = bvTreeshape->getShapeCollection()->getContainer();	
			}
			else
			{
			...
			//
			{
				...
				for (	hkShapeKey key = shapeContainer->getFirstKey();
						key != HK_INVALID_SHAPE_KEY;
						key = shapeContainer->getNextKey( key ))
				{
					const hkShape* childShape = shapeContainer->getChildShape( key, buffer );
		...
		{
			const hkCapsuleShape* capsuleShape = static_cast<const hkCapsuleShape*>(shape);
			hkInertiaTensorComputer::computeCapsuleVolumeMassProperties(capsuleShape->getVertex(0), capsuleShape->getVertex(1), capsuleShape->getRadius(), 1.0f, newMassProperties);
			break;
		}

		case HK_SHAPE_CYLINDER:
		{
			const hkCylinderShape* cylinderShape = static_cast<const hkCylinderShape*>(shape);
			hkInertiaTensorComputer::computeCylinderVolumeMassProperties(cylinderShape->getVertex(0), cylinderShape->getVertex(1), cylinderShape->getCylinderRadius(), 1.0f, newMassProperties);
			break;
		}
		
		default:
		{
			HK_WARN(0x7d6a631a, "Unknown shape type to compute mass propeties of. Have you implemented a user shape but not yet implemented mass property calculation in hkInertiaTensorComputer::computeRecursiveShapeVolumeMassProperties()?\n" );
			return;
		}
	...
	{
		hkArray<hkMassElement> allElements;
		...
		hkMassElement currentMassElement;
		currentMassElement.m_properties = massPropertiesOut;
		allElements.pushBack(currentMassElement);
		

		hkMassElement newMassElement;
		newMassElement.m_properties = newMassProperties;
		newMassElement.m_transform = transform;
		allElements.pushBack(newMassElement);

		hkInertiaTensorComputer::combineMassProperties(allElements, massPropertiesOut);
	}	

}

void HK_CALL hkInertiaTensorComputer::shiftInertiaToCom(hkVector4& shift, hkReal mass, hkMatrix3& inertia)
{
...
void HK_CALL hkInertiaTensorComputer::shiftInertiaFromCom(hkVector4& shift, hkReal mass, hkMatrix3& inertia)
{
	inertia(0,0) += mass * (shift(1)*shift(1) + shift(2)*shift(2));
	inertia(1,1) += mass * (shift(2)*shift(2) + shift(0)*shift(0));
	inertia(2,2) += mass * (shift(0)*shift(0) + shift(1)*shift(1));
...
struct InternalInertiaTensorComputer
{
	...
	static hkVector4 HK_CALL compShift(const hkGeometry* geom);
...
// This computes a shift which will ensure that the geometry is "roughly" centered
...
hkVector4 HK_CALL InternalInertiaTensorComputer::compShift(const hkGeometry* geom)
{

	hkVector4 avg;
	avg.setZero4();

	int numv = geom->m_vertices.getSize();
	hkVector4 min;
	min.setAll(HK_REAL_MAX);
	hkVector4 max;
	max.setAll(-HK_REAL_MAX);
	for(int i = 0; i< numv; i++)
	{
	    hkVector4 vert = geom->m_vertices[i];
		min.setMin4(vert, min);
		max.setMax4(vert, max);
	}
...
hkResult HK_CALL hkInertiaTensorComputer::computeGeometryVolumeMassProperties(const hkGeometry* geom, hkReal mass, hkMassProperties &result)
{
	...
	// First calculate a shift to add to the vertices, to "recenter" them
	InternalInertiaTensorComputer computer;

	hkVector4 shift = computer.compShift(geom); shift(3) = 0;
	...
	computer.computeInertialTensorInternal(mass, density, r, J);

	// Must subtract shift (since we forced readPolyhedron to construct genetry data with
	// shift added to each vertex, hence VOL, IT are the same, but C.O.M for the "unshifted"
	// geometry is equal to C.O.M(shifted) - "shift".
	// This has NOTHING to do with any body/local/world transforms.
	r.setSub4(r, shift);

	// When we decide to perform this calculation in body space, we'll be able to use the lines below
	// to map to local.
	result.m_centerOfMass = r;
	result.m_inertiaTensor = J;
	result.m_mass = mass;
...
/*
// WARNING: This function assumes the geometry is properly closed.
// <todo> verify warning ???? taken from hkResult HK_CALL hkInertiaTensorComputer::computeGeometryVolumeMassProperties(const hkGeometry* geom, hkReal mass, hkMassProperties &result)
void HK_CALL hkInertiaTensorComputer::internalComputeVolume(const hkGeometry* geometry, hkReal& volumeOut, hkVector4& shiftOut)
{
	// First calculate a shift to add to the vertices, to "recenter" them
	InternalInertiaTensorComputer computer;
	shiftOut = computer.compShift(geometry); 
	shiftOut(3) = 0;
	computer.compVolumeIntegrals(geometry, shiftOut);
	volumeOut = computer.T0;
}
*/

/*
// Warning: don't use this function if withouth executing internalComputeVolume() just before it.
// It is based on some static variables set by that function.
void HK_CALL hkInertiaTensorComputer::internalContinueComputeMassProperties(hkReal volume, const hkVector4& shift, hkReal mass, hkMassProperties &result)
{
	HK_ASSERT2(0x7df8b01c,  mass > 0.0f, "Cannot calculate mass properties with zero mass or less." );
...
static hkReal HK_CALL hkInertiaTensorComputer_optimizeInertiasOfConstraintTreeInt(
			hkArray<const hkConstraintInstance*>& constraints, 
			hkRigidBody* body, hkReal inertiaFactorHint )
{

	// find all children
	hkInplaceArray<const hkConstraintInstance*,16> children;
	{
		for (int i = constraints.getSize()-1; i>=0;i-- )
		{
			const hkConstraintInstance* ci = constraints[i];
			hkRigidBody* otherBody = HK_NULL;
	...
	hkReal maxChild = 0.0f;
	{
		...
		{
			const hkConstraintInstance* ci = children[i];
			hkRigidBody* otherBody = (  ci->getRigidBodyA() == body ) ? ci->getRigidBodyB() : ci->getRigidBodyA();
...
void hkInertiaTensorComputer::optimizeInertiasOfConstraintTree( hkConstraintInstance*const* constraints, int numConstraints, hkRigidBody* rootBody, hkReal inertiaFactorHint  )
{
	hkLocalArray<const hkConstraintInstance*> constr(numConstraints);
