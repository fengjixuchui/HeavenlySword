...
class hkSerializedDisplayRbTransforms : public hkReferencedObject
{
public:
	...
	inline hkSerializedDisplayRbTransforms();
	...
	inline int findBodyIndex(const hkRigidBody* b) const;

		/// Make sure that pi is with range.
	inline hkTransform& getTransform(int pi);

		/// Make sure that pi is with range.
	inline hkRigidBody* getBody(int pi);

		/// Will reference count rb
	inline void addPair( hkRigidBody* rb, const hkTransform& t);
...
public:
	...
	hkSerializedDisplayRbTransforms(hkFinishLoadedObjectFlag flag) : m_transforms(flag) { }
};

#include <hkutilities/serialize/display/hkSerializedDisplayRbTransforms.inl>
