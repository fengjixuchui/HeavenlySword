...
HK_REFLECTION_DEFINE_VIRTUAL(hkPoweredChainMapper);

namespace
{
	struct hkMapperTargetInfo
	{
		hkPoweredChainMapper::Target m_target;
		int m_linkIndex;
	};
}

hkPoweredChainMapper* hkPoweredChainMapper::buildChainMapper(const Config config, const hkArray<hkConstraintInstance*>& allConstraints, const hkArray<ChainEndpoints>& pairs, hkArray<hkConstraintInstance*>* unusedConstraints)
{
	hkPoweredChainMapper* mapper = new hkPoweredChainMapper();
	hkArray<hkMapperTargetInfo> targetInfos;
	hkArray<int> linkIdxToNumTargets;
	{
		linkIdxToNumTargets.setSize(allConstraints.getSize());
		hkString::memSet( linkIdxToNumTargets.begin(), 0, linkIdxToNumTargets.getSize() * sizeof(int) );
	}

	hkPointerMap<hkConstraintInstance*, int> constraintToLink;
	...
	hkString::memSet( mapper->m_links.begin(), 0, mapper->m_links.getSize() * sizeof(LinkInfo) );
	...
	{
		hkArray<hkEntity*> linkedEntities;
		hkArray<hkConstraintInstance*> linkedConstraints;

		// attempt to find a chain
		if ( HK_FAILURE == hkConstraintChainUtil::findConstraintLinkBetweenEntities(allConstraints, pairs[p].m_start, pairs[p].m_end, linkedEntities, linkedConstraints) )
		{
		...
		hkConstraintChainInstance* chainInstance = hkConstraintChainUtil::buildPoweredChain(linkedConstraints, config.m_cloneMotors);
		...
		{
			hkPointerMap<hkConstraintInstance*, int>::Iterator it = constraintToLink.findKey(linkedConstraints[c]);
			...
			if (chainInstance->getData()->getType() != hkConstraintData::CONSTRAINT_TYPE_POWERED_CHAIN)
			{
			...
			target.m_chain = static_cast<hkPoweredChainData*>( chainInstance->getData() );
			target.m_infoIndex = c;

			if (config.m_createLimitConstraints && (! link.m_limitConstraint))
			{
				link.m_limitConstraint = hkConstraintUtils::convertToLimits(linkedConstraints[c]);
			}
		}
	}
...
void hkPoweredChainMapper::setForceLimits(int linkIndex, int coordinageIndex, hkReal minForce, hkReal maxForce)
{
	hkArray<hkConstraintMotor*> motors;
	...
	if (motors.getSize())
	{
		...
		{
			HK_ON_DEBUG( hkConstraintMotor::MotorType type = motors[m]->getType());
			HK_ASSERT2(0xad67d9a1, type == hkConstraintMotor::TYPE_POSITION
				                || type == hkConstraintMotor::TYPE_VELOCITY
								|| type == hkConstraintMotor::TYPE_SPRING_DAMPER, "Unsupported motor type. If your custom motor derives from the hkLimitedForceConstraintMotor class, you can safely ignore this assert.");

			hkLimitedForceConstraintMotor* limited = static_cast<hkLimitedForceConstraintMotor*>(motors[m]);
...
void hkPoweredChainMapper::getMotors(int linkIndex, int coordinateIndex, hkArray<hkConstraintMotor*>& motorsOut)
{
	...
	for (int t = 0; t < link.m_numTargets; t++)
	{
		...
		HK_ASSERT2(0x79c0f6a2, target.m_chain->getType() == hkConstraintData::CONSTRAINT_TYPE_POWERED_CHAIN, "Illegal chain type in the mapper. Attempting to extract motors from a non-powered chain.");

		hkPoweredChainData* powered = static_cast<hkPoweredChainData*>(target.m_chain);
		hkConstraintMotor* motor = powered->m_infos[target.m_infoIndex].m_motors[coordinateIndex];
		if (motor)
		{
...
void hkPoweredChainMapper::setMotors(int linkIndex, int coordinateIndex, hkConstraintMotor* newMotor)
{
	LinkInfo& link = m_links[linkIndex];
	...
	for (int t = 0; t < link.m_numTargets; t++)
	{
		...
		HK_ASSERT2(0x4e035480, target.m_chain->getType() == hkConstraintData::CONSTRAINT_TYPE_POWERED_CHAIN, "Illegal chain type in the mapper. Attempting to extract motors from a non-powered chain.");
...
void hkPoweredChainMapper::setTargetOrientation(int linkIndex, const hkQuaternion& newTarget)
{
	LinkInfo& link = m_links[linkIndex];
	...
	for (int t = 0; t < link.m_numTargets; t++)
	{
		//Target& target = link.m_targets[t];
		Target& target = m_targets[link.m_firstTargetIdx + t];
		HK_ASSERT2(0x7ccc1cad, target.m_chain->getType() == hkConstraintData::CONSTRAINT_TYPE_POWERED_CHAIN, "Illegal chain type in the mapper. Attempting to extract motors from a non-powered chain.");
		hkPoweredChainData* powered = static_cast<hkPoweredChainData*>(target.m_chain);
		powered->m_infos[target.m_infoIndex].m_aTc = newTarget;
	}
