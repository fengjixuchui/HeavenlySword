...
class hkPoweredChainMapper : public hkReferencedObject
{
	public:
	...
	public:
		...
		static hkPoweredChainMapper* HK_CALL buildChainMapper(const Config config, const hkArray<hkConstraintInstance*>& allConstraints, const hkArray<ChainEndpoints>& pairs, hkArray<hkConstraintInstance*>* unusedConstraints = HK_NULL);
		...
		void getMotors(int linkIndex, int coordinateIndex, hkArray<hkConstraintMotor*>& motorsOut);
		...
		void setMotors(int linkIndex, int coordinateIndex, hkConstraintMotor* newMotor);
		...
		struct LinkInfo
		{
			...
			hkConstraintInstance* m_limitConstraint;
			...
			/// Matches 1-1 the hkConstraintInstance array passed to buildChainMapper()
		...
		hkArray<class hkConstraintChainInstance*> m_chains;

			/// Finish up ctor.
		hkPoweredChainMapper( hkFinishLoadedObjectFlag f )  : m_links(f), m_targets(f), m_chains(f) {}
