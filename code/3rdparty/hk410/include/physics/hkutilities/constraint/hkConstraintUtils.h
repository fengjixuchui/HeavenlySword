...
public:
	...
	static hkConstraintInstance* HK_CALL convertToPowered (const hkConstraintInstance* originalConstraint, hkConstraintMotor* constraintMotor);
	...
	static hkConstraintInstance* HK_CALL convertToLimits (hkConstraintInstance* originalConstraint);
	...
	static hkBool HK_CALL checkAndFixConstraint (const hkConstraintInstance* constraint, hkReal maxAllowedError = 0.05f, hkReal relativeDisplacementTowardsParent = 0.8f);


		/// Extracts pivot vectors from the constraint.
	static hkResult HK_CALL getConstraintPivots(const hkConstraintInstance* constraint, hkVector4& pivotA, hkVector4& pivotB);

		/// Extracts motors from the constraints.
	static hkResult HK_CALL getConstraintMotors(const hkConstraintData* constraintData, hkConstraintMotor*& motor0, hkConstraintMotor*& motor1, hkConstraintMotor*& motor2 );
