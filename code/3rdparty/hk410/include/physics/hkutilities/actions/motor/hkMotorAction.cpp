...
HK_REFLECTION_DEFINE_VIRTUAL(hkMotorAction);

hkMotorAction::hkMotorAction(hkRigidBody* body, const hkVector4& axis, hkReal spinRate, hkReal gain) :
	hkUnaryAction(body), m_axis(axis), m_spinRate(spinRate), m_gain(gain), m_active(true)
{
	m_axis.normalize4();
}


void hkMotorAction::applyAction( const hkStepInfo& stepInfo )
{
...
hkAction* hkMotorAction::clone( const hkArray<hkEntity*>& newEntities, const hkArray<hkPhantom*>& newPhantoms ) const
{
	...
	hkMotorAction* ma = new hkMotorAction( (hkRigidBody*)newEntities[0], m_axis, m_spinRate, m_gain);
