...
HK_REFLECTION_DEFINE_VIRTUAL(hkDashpotAction);

hkDashpotAction::hkDashpotAction(hkRigidBody* entityA, hkRigidBody* entityB , hkUint32 userData ) 
...
const hkVector4 & hkDashpotAction::getImpulse()
{
	return m_impulse;
}
...
void hkDashpotAction::applyAction(const hkStepInfo& stepInfo)
{
	...
	//m_impulse = (dt * m_strength) * ( pa - pb ) +  * ( va - vb ) Below
    {
	  ...
	  m_impulse.setMul4(dt * m_strength, pab);
	  vab.mul4(dt * m_damping);
	  m_impulse.add4(vab);
	}
    ...
    */
	...
	{ 
		...
		negImpulse.setMul4(-1,m_impulse);
...
hkAction* hkDashpotAction::clone( const hkArray<hkEntity*>& newEntities, const hkArray<hkPhantom*>& newPhantoms ) const
{
	...
	hkDashpotAction* sa = new hkDashpotAction( (hkRigidBody*)newEntities[0], (hkRigidBody*)newEntities[1], m_userData );
