...
/// It is similar to a point-to-point constraint, although it is an hkAction rather
/// an hkConstraintInstance.
...
class hkDashpotAction : public hkBinaryAction
{
	public:
		...
		hkDashpotAction(hkRigidBody* entityA = HK_NULL, hkRigidBody* entityB = HK_NULL, hkUint32 userData = 0);

 			/// Apply the hkDashpotAction to the two bodies.
		...
		inline void setPointB(const hkVector4& pb);
			...
			/// Return positions in local space on bodies A and B to which the hkDashpotAction is attached.
			/// \param pa The hkDashpotAction application point on bodyA, in the local space of bodyA, are copied into pa.
			/// \param pb The hkDashpotAction application point on bodyB, in the local space of bodyB, are copied into pb.
		inline void getPoints(hkVector4& pa, hkVector4& pb);

			/// Dynamically change points (in local space) on bodies A and B to which the hkDashpotAction is attached.
		...
		inline void setPoints(const hkVector4& pa, const hkVector4& pb);
			...
			/// Returns the strength of the hkDashpotAction.
			/// This value is the strength of the restoring force between the two points. An appropriate strength for 
			/// a hkDashpotAction is the weight of the heavier of the two rigid bodies.
		inline hkReal getStrength() const;

			/// Dynamically sets the strength of the hkDashpotAction.
			/// This value is the strength of the restoring force between the two points. An appropriate strength for a 
			/// hkDashpotAction is the weight of the heavier of the two rigid bodies
			/// \param s The new strength.
		inline void setStrength(hkReal s);

			/// Returns the damping coefficient of the hkDashpotAction.
			/// The damping parameter specifies the rate of decay of the oscillations the hkDashpotAction produces.
			/// Values higher than 1/10 of the hkDashpotAction strength can cause instability.
		inline hkReal getDamping() const;

			/// Dynamically sets the damping coefficient of the hkDashpotAction.
			/// The damping parameter specifies the rate of decay of the oscillations the hkDashpotAction produces.
			/// In general the damping should be 10% of the strength of the hkDashpotAction.
			/// \param d The damping coefficient of the hkDashpotAction.
	...
	public:
		...
		hkDashpotAction( class hkFinishLoadedObjectFlag flag ) : hkBinaryAction(flag) {}

};

#include <hkutilities/actions/dashpot/hkDashpotAction.inl>
