...
HK_REFLECTION_DEFINE_VIRTUAL(hkSpringAction);

hkSpringAction::hkSpringAction( hkRigidBody* entityA , hkRigidBody* entityB , hkUint32 userData )
...
void hkSpringAction::setPositionsInWorldSpace( const hkVector4& pivotA, const hkVector4& pivotB )
{
...
void hkSpringAction::setPositionsInBodySpace( const hkVector4& pivotA, const hkVector4& pivotB )
{
	hkRigidBody* rbA = static_cast<hkRigidBody*>( m_entityA );
	hkRigidBody* rbB = static_cast<hkRigidBody*>( m_entityB );
	HK_ASSERT2(0xf568efca, rbA && rbB, "Bodies not set in spring.");
...
void hkSpringAction::applyAction( const hkStepInfo& stepInfo )
{
...
hkAction* hkSpringAction::clone( const hkArray<hkEntity*>& newEntities, const hkArray<hkPhantom*>& newPhantoms ) const
{
	...
	hkSpringAction* sa = new hkSpringAction( (hkRigidBody*)newEntities[0], (hkRigidBody*)newEntities[1], m_userData );
