...
int hkContactPointViewer::m_tag = 0;

void HK_CALL hkContactPointViewer::registerViewer()
{
	m_tag = hkProcessFactory::getInstance().registerProcess( getName(), create );
}

hkProcess* HK_CALL hkContactPointViewer::create(const hkArray<hkProcessContext*>& contexts)
{
	return new hkContactPointViewer(contexts);
}

hkContactPointViewer::hkContactPointViewer(const hkArray<hkProcessContext*>& contexts)
: hkWorldViewerBase( contexts)
{
	
}

void hkContactPointViewer::init()
{
	if (m_context)
	{
...
hkContactPointViewer::~hkContactPointViewer()
{
	if (m_context)
	{
		for (int i=0; i < m_context->getNumWorlds(); ++i)
...
void hkContactPointViewer::worldAddedCallback( hkWorld* w)
{
	w->markForWrite();
...
void hkContactPointViewer::worldRemovedCallback( hkWorld* w)
{
	w->markForWrite();
...
void hkContactPointViewer::contactPointConfirmedCallback(       hkContactPointConfirmedEvent& event)
{
}

void hkContactPointViewer::contactPointAddedCallback( hkContactPointAddedEvent& event)
{
/*
	...
	for (int i=0; i < 20; i++ )
	{
		...
		m_displayHandler->displayLine(p, pdir, hkColor::RED);
	}
*/
...
void hkContactPointViewer::contactPointRemovedCallback( hkContactPointRemovedEvent& event)
{
...
void hkContactPointViewer::contactProcessCallback( hkContactProcessEvent& event)
{
