...
int hkRigidBodyInertiaViewer::m_tag = 0;

hkProcess* HK_CALL hkRigidBodyInertiaViewer::create(const hkArray<hkProcessContext*>& contexts)
{
	return new hkRigidBodyInertiaViewer(contexts);
}

void HK_CALL hkRigidBodyInertiaViewer::registerViewer()
{
	m_tag = hkProcessFactory::getInstance().registerProcess( getName(), create );
}

hkRigidBodyInertiaViewer::hkRigidBodyInertiaViewer(const hkArray<hkProcessContext*>& contexts)
: hkWorldViewerBase(contexts)
{

}

void hkRigidBodyInertiaViewer::init()
{
...
hkRigidBodyInertiaViewer::~hkRigidBodyInertiaViewer()
{
	if (m_context) // have a physics context
	{
		int nw = m_context->getNumWorlds();
...
void hkRigidBodyInertiaViewer::worldRemovedCallback( hkWorld* world )
{
	removeWorld(world);
}

void hkRigidBodyInertiaViewer::worldAddedCallback( hkWorld* world )
{
	addWorld(world);
}

void hkRigidBodyInertiaViewer::removeWorld(hkWorld* world)
{
	world->markForWrite();
...
void hkRigidBodyInertiaViewer::addWorld(hkWorld *world)
{
	world->markForWrite();
...
void hkRigidBodyInertiaViewer::entityAddedCallback( hkEntity* entity )
{
...
void hkRigidBodyInertiaViewer::entityRemovedCallback( hkEntity* entity )
{
//	if( entity->getType() == HK_RIGID_BODY )
//	{
		hkRigidBody* rigidBody = static_cast<hkRigidBody*>(entity);
	...
	//	HK_ASSERT2(0x312dbfd7, index != -1, "Trying to remove body which hkRigidBodyInertiaViewer does not think has been added!");
		if(index >= 0)
		{
...
void hkRigidBodyInertiaViewer::postSimulationCallback( hkWorld* world )
{
	HK_TIMER_BEGIN("hkRigidBodyInertiaViewer", this);
	...
	for(int i = 0; i < m_entitiesCreated.getSize(); i++)
	{
		...
		if(mass != 0.0f)
		{
			...
			*/
			...
			//hkCollideDebugUtil::drawObbLines(t, halfExtents, HK_DEFAULT_INERTIA_TENSOR_COLOR, m_displayHandler);
		}
	}
