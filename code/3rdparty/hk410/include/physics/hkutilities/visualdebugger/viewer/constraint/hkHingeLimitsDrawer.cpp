...
// Nearly exact copy of the code from hkLimitedHingeDrawer. (With exception of the pivot vectors being zeroed.)
void hkHingeLimitsDrawer::drawConstraint(hkConstraintInstance* constraint, hkDebugDisplayHandler* displayHandler, int tag)
{
	hkHingeLimitsData* lhinge = static_cast<hkHingeLimitsData*>(constraint->getData());
	...
	axisInWorld.setRotatedDir(m_RB, baseB[hkHingeLimitsData::Atoms::AXIS_AXLE]);

	hkVector4 axisPerpInWorld;
	axisPerpInWorld.setRotatedDir(m_RB, baseB[hkHingeLimitsData::Atoms::AXIS_PERP_TO_AXLE_2]);
	...
	// is with respect to.
	{
		...
		hkVector4 axisPerpA;
		axisPerpA.setRotatedDir(m_RA, baseA[hkHingeLimitsData::Atoms::AXIS_PERP_TO_AXLE_2]); 
