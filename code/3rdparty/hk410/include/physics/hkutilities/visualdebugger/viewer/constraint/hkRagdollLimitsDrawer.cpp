...
// Nearly exact copy of the code from hkRagdollDrawer. (With exception of the pivot vectors being zeroed.)
void hkRagdollLimitsDrawer::drawConstraint(hkConstraintInstance* constraint, hkDebugDisplayHandler* displayHandler, int tag)
{

	hkRagdollLimitsData* ragdoll = static_cast<hkRagdollLimitsData*>(constraint->getData());
	...
	refTwistAxisWorld.setRotatedDir(m_RB, baseB[hkRagdollLimitsData::Atoms::AXIS_TWIST]);

	hkVector4 refCrossedWorld;
	refCrossedWorld.setCross( baseB[hkRagdollLimitsData::Atoms::AXIS_TWIST], baseB[hkRagdollLimitsData::Atoms::AXIS_PLANES] );
	refCrossedWorld.normalize3();
	refCrossedWorld.setRotatedDir(m_RB, refCrossedWorld);

	hkVector4 refPlaneNormalWorld;
	refPlaneNormalWorld.setRotatedDir(m_RB, baseB[hkRagdollLimitsData::Atoms::AXIS_PLANES]);

	hkVector4 cross;
	cross.setCross(baseA[hkRagdollLimitsData::Atoms::AXIS_PLANES], baseA[hkRagdollLimitsData::Atoms::AXIS_TWIST]);
	cross.normalize3();
	hkVector4 attCrossedWorld;
	attCrossedWorld.setRotatedDir(m_RA, cross);

	hkVector4 attTwistAxisWorld;
	attTwistAxisWorld.setRotatedDir(m_RA, baseA[hkRagdollLimitsData::Atoms::AXIS_TWIST]);
