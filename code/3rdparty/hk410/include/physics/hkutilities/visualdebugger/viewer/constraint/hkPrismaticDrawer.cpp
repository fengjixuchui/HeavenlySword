...
void hkPrismaticDrawer::drawConstraint(hkConstraintInstance* constraint, hkDebugDisplayHandler* displayHandler, int tag)
{
	hkPrismaticConstraintData* prismatic = static_cast<hkPrismaticConstraintData*>(constraint->getData());
	...
	axisInWorld.setRotatedDir(m_RB, prismatic->m_atoms.m_transforms.m_transformB.getColumn(hkPrismaticConstraintData::Atoms::AXIS_SHAFT));
	hkVector4 axisPerpInWorld;
	axisPerpInWorld.setRotatedDir(m_RB, prismatic->m_atoms.m_transforms.m_transformB.getColumn(hkPrismaticConstraintData::Atoms::AXIS_PERP_TO_SHAFT));
