...
void hkPointToPathDrawer::drawConstraint(hkConstraintInstance* constraint, hkDebugDisplayHandler* displayHandler, int tag)
{

	hkPointToPathConstraintData* pToPConstraint = static_cast<hkPointToPathConstraintData*>(constraint->getData());
	...
	if(pToPConstraint->getPath() != HK_NULL)
	{
		...
		hkParametricCurve* curve = pToPConstraint->getPath(); 
