...
hkCharacterProxy::hkCharacterProxy(const hkCharacterProxyCinfo& info)
	: m_shapePhantom(HK_NULL)
{
	updateFromCinfo( info );
}

void hkCharacterProxy::getCinfo(hkCharacterProxyCinfo& info) const
{
...
void hkCharacterProxy::updateFromCinfo( const hkCharacterProxyCinfo& info )
{
...
/*
#include <stdio.h>
static void printManifold(hkArray<hkRootCdPoint>& manifold)
{
	printf("\nManifold: \n");
...
void hkCharacterProxy::convertFractionToDistance( const hkRootCdPoint* cHit, int numHits, const hkVector4& displacement ) const
{
...
void hkCharacterProxy::integrate( const hkStepInfo& stepInfo, const hkVector4& worldGravity )
{
...
void hkCharacterProxy::integrateWithCollectors(const hkStepInfo& stepInfo, const hkVector4& worldGravity, hkAllCdPointCollector& castCollector, hkAllCdPointCollector& startPointCollector)
{
	...
	for ( int iter = 0; (remainingTime > HK_REAL_EPSILON) && (iter < m_maxCastIterations) ; iter++ )
	{
		...
		{
			bool simplexResultOk = m_oldDisplacement.equals3(output.m_position);
			...
			if (!simplexResultOk)
			{
				...
				if ( castCollector.hasHit() )
				{
					...
					if (castCollector.getHits().getSize() > 0)
					{
						...
						{
							hkRigidBody* body =  hkGetRigidBody(surface.m_rootCollidableB);
							...
							hkPhantom* phantom = hkGetPhantom( surface.m_rootCollidableB );
...
hkReal hkCharacterProxy::moveToLinearCastHitPosition(const hkSimplexSolverOutput& output, 
...
const hkArray<hkRootCdPoint>& hkCharacterProxy::getManifold() const
{
	return m_manifold;
}

// Extract velocity information from the contact point.
void hkCharacterProxy::extractSurfaceVelocity(const hkRootCdPoint& hit, hkVector4& velocityOut) const
{
...
void hkCharacterProxy::refreshManifold( hkAllCdPointCollector& startPointCollector )
{
...
void hkCharacterProxy::checkSupport(const hkVector4& direction, hkSurfaceInfo& ground)
{
	hkAllCdPointCollector startPointCollector;
	checkSupportWithCollector(direction, ground, startPointCollector);
}

void hkCharacterProxy::checkSupportWithCollector(const hkVector4& direction, hkSurfaceInfo& ground, hkAllCdPointCollector& startPointCollector)
{
...
void hkCharacterProxy::checkSupportDeprecated(const hkVector4& direction, hkSurfaceInfoDeprecated& ground) const
{
	HK_ASSERT2(0x79d57ec9,  hkMath::equal(direction.length3(), 1.0f), "checkSupport Direction should be normalized");
	HK_TIMER_BEGIN("checkSupport", HK_NULL);
...
void hkCharacterProxy::extractSurfaceConstraintInfo(const hkRootCdPoint& hit, hkSurfaceConstraintInfo& surface, hkReal timeTravelled) const
{
...
//	  - Velocity
inline hkReal hkCharacterProxy::surfaceDistance(const hkRootCdPoint& p1, const hkRootCdPoint& p2) const
{
...
int hkCharacterProxy::findSurface(const hkRootCdPoint& info) const
{
	int bestIndex = -1;
...
void hkCharacterProxy::updateManifold(const hkAllCdPointCollector& startPointCollector, const hkAllCdPointCollector& castCollector )
{
	...
	//
	{
		for (int h=0; h < startPointHits.getSize(); h++)
		{
			...
			if ( (surface.m_contact.getDistance() ) == minimumPenetration )
			{
				...
				if (index >= 0)
				{
					hkRootCdPoint& current = m_manifold[index];
					if (surface.m_rootCollidableB != current.m_rootCollidableB)
					{
						fireContactRemoved(current);
						fireContactAdded(surface);
...
void hkCharacterProxy::applySurfaceInteractions( const hkStepInfo& stepInfo, const hkVector4& worldGravity )
{
	...
	for (int s=0; s < m_manifold.getSize(); s++)
	{
		...
		if (object->hasProperty(HK_PROPERTY_CHARACTER_PROXY))
		{
			...
			hkCharacterProxy* otherChar = reinterpret_cast<hkCharacterProxy*>( val.getPtr()) ;
		...
		if ( ( body != HK_NULL ) && ( !body->isFixedOrKeyframed() ) )
		{
			...
			// Add gravity
			{
				hkVector4 charVelDown; charVelDown.setMul4(stepInfo.m_deltaTime, worldGravity);
...
void hkCharacterProxy::setPosition(const hkVector4& position)
{
...
const hkVector4&	hkCharacterProxy::getLinearVelocity() const
{
	return m_velocity ;
}

void hkCharacterProxy::setLinearVelocity( const hkVector4& vel )
{
	m_velocity = vel;
}
...
const hkShapePhantom* hkCharacterProxy::getShapePhantom() const
{
	return m_shapePhantom;
}

void hkCharacterProxy::addCharacterProxyListener( hkCharacterProxyListener* cpl)
{
...
void hkCharacterProxy::removeCharacterProxyListener( hkCharacterProxyListener* cpl)
{
...
void hkCharacterProxy::fireConstraintsProcessed( const hkArray<hkRootCdPoint>& manifold, hkSimplexSolverInput& input ) const
{
	for ( int i = m_listeners.getSize()-1; i >= 0; i-- )
	{
...
void hkCharacterProxy::fireContactAdded(const hkRootCdPoint& point) const
{
	for ( int i = m_listeners.getSize()-1; i >= 0; i-- )
	{
...
void hkCharacterProxy::fireContactRemoved(const hkRootCdPoint& point) const
{
	for ( int i = m_listeners.getSize()-1; i >= 0; i-- )
	{
...
void hkCharacterProxy::entityRemovedCallback( hkEntity* entity )
{
	...
	{
		hkRigidBody* body =  hkGetRigidBody(m_manifold[p1].m_rootCollidableB);
		if (body == entity)
		{
			m_manifold.removeAt(p1);
		}
	...
	hkRigidBody* oldBody = static_cast<hkRigidBody*>(entity);
...
void hkCharacterProxy::phantomRemovedCallback( hkPhantom* phantom )
{
	...
	for (int p1 = size-1; p1 >= 0; p1--)
	{
		hkPhantom* manifoldPhantom =  hkGetPhantom(m_manifold[p1].m_rootCollidableB);
	...
	hkPhantom* oldPhantom = static_cast<hkPhantom*>(phantom);
...
void hkCharacterProxy::calcStatistics( hkStatisticsCollector* collector ) const
{
