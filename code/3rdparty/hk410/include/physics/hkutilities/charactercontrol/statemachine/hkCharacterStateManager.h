...
/// the hkCharacterInput class or use its user data member to point to per character info.
class hkCharacterStateManager : public hkReferencedObject
{
	public:
		...
		~hkCharacterStateManager();
			...
			/// is removed.
		void registerState(hkCharacterState* m_state, hkCharacterStateType state);

			/// returns the state registered for the given type
			/// If no state has been registered this returns HK_NULL
		hkCharacterState* getState(hkCharacterStateType initialState) const;
			
	private:

		hkCharacterState* m_registeredState[HK_CHARACTER_MAX_STATE_ID];
};
