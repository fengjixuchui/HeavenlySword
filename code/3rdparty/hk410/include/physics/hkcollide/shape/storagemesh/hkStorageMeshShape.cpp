...
HK_REFLECTION_DEFINE_VIRTUAL(hkStorageMeshShape);
HK_REFLECTION_DEFINE_SCOPED_VIRTUAL(hkStorageMeshShape,SubpartStorage);

hkStorageMeshShape::SubpartStorage::SubpartStorage( hkFinishLoadedObjectFlag flag )
	: m_vertices(flag), m_indices16(flag), m_indices32(flag),
	m_materialIndices(flag), m_materials(flag), m_materialIndices16(flag)
{
}

hkStorageMeshShape::hkStorageMeshShape( hkFinishLoadedObjectFlag flag )
	: hkMeshShape(flag), m_storage(flag)
{
	if( flag.m_finishing )
	{
		for( int i = 0; i < m_subparts.getSize(); ++i )
		{
			...
			part.m_materialBase = reinterpret_cast<hkMeshMaterial*>(store.m_materials.begin());
		}
	}	
}

hkStorageMeshShape::hkStorageMeshShape( hkReal radius, int numbits )
: hkMeshShape(radius, numbits)
{
}

hkStorageMeshShape::~hkStorageMeshShape()
{
...
hkStorageMeshShape::hkStorageMeshShape( const hkMeshShape* mesh )
: hkMeshShape( mesh->getRadius(), mesh->getNumBitsForSubpartIndex() )
{
	m_scaling = mesh->m_scaling;
	
	// [HVK-2295] missing mesh atributes
	m_userData = mesh->m_userData; // hkShape
	m_disableWelding = mesh->m_disableWelding; // hkShapeCollection
...
#ifdef HK_DEBUG
	hkShapeKey korig = mesh->getFirstKey();
	hkShapeKey kthis = this->getFirstKey();
	while(1)
	{
		HK_ASSERT(0, korig == kthis);
		if( korig == HK_INVALID_SHAPE_KEY ) break;

		hkShapeCollection::ShapeBuffer borig;
		hkShapeCollection::ShapeBuffer bthis;

		const hkTriangleShape* torig = static_cast<const hkTriangleShape*>(mesh->getChildShape(korig, borig));
		const hkTriangleShape* tthis = static_cast<const hkTriangleShape*>(this->getChildShape(kthis, bthis));

		for( int j = 0; j < 3; ++j )
		{
			HK_ASSERT(0, torig->getVertex(j).equals3( tthis->getVertex(j), 0 ) );
		}
...
void hkStorageMeshShape::addSubpart( const Subpart& partIn )
{
...

	{
		...
		if( partIn.m_stridingType == INDICES_INT16 )
		{
			...
			int nindices = numIndices(strideIn, partIn.m_numTriangles);
	...
	if( partIn.m_materialIndexBase )
	{
		...
		if (part.m_materialIndexStriding != 0)
		{
			...
			default                    : HK_ASSERT2(0XAD45F3F3,0, "hkMeshShape::m_materialIndexType not properly specified."); break;
			}
		}
	...
	{
		HK_ASSERT2(0xad87fb7b, partIn.m_materialBase, "Materials not defined for a hkMeshShape::SubPart.");
		...
		part.m_materialBase = reinterpret_cast<const hkMeshMaterial*>(store.m_materials.begin());
	}
	else
	{
		part.m_numMaterials = 1;
		part.m_materialBase = reinterpret_cast<const hkMeshMaterial*>( &hkVector4::getZero() );
		part.m_materialStriding = 0;
