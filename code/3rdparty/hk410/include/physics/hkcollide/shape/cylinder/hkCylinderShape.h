...
extern const hkClass hkCylinderShapeClass;
	...
	/// 
	/// hkCylinderShape is not modeled as an implicit geometry (hkSphereShape and hkCapsuleShape are implicitly modeled).
	/// hkCylinderShape is represented by vertices generated on the cylinder's surface. Those vertices 
	...
	/// hkCylinderShape uses a specialized collision agent, hkPredGskCylinderAgent3.
	...
	/// and this reduction may result in objects tunnelling through each other. In a test where a 1cm-radius cylinder collided against
	...
	///
	/// hkPredGskCylinderAgent3 is not used when an hkCylinderShape is enclosed in an hkShapeCollection
	/// hierarchy e.g. when an hkCylinderShape is part of an hkListShape.
	///
	/// A problem + work-around for such cases:
	///
	/// hkCylinderShape is tessellated, not implicit like hkSphereShape or hkCapsuleShape. This leads 
	...
	/// shape consisting of an hkCylinderShape and an hkCapsuleShape. The hkCylinderShape ensures 
	/// that the list shape can stand on its ends. The hkCapsuleShape ensures that the list 
	...
	/// 
	/// You can vary the tessellation of cylinders by calling hkCylinderShape::setNumberOfSideSegments().
...
class hkCylinderShape : public hkConvexShape
{
	public:

		HK_DECLARE_REFLECTION();

			/// Creates a new hkCylinderShape using the specified points and radius
		hkCylinderShape(	const hkVector4& vertexA,	
		...
		virtual hkBool castRay( const hkShapeRayCastInput& input, hkShapeRayCastOutput& results ) const;
 		...
 		void getAabb( const hkTransform& localToWorld, hkReal tolerance, hkAabb& out  ) const;
			...
			/// Gets this hkShape's type. For hkCylinderShapes, this is HK_SHAPE_CYLINDER.
	...
	private:
		HK_FORCE_INLINE void decodeVertexId(hkVertexId code, hkVector4& result) const;

	protected:
		// The cylinder's radius (as opposed to the bounding volume radius associated with hkConvexShape
		hkReal    m_cylRadius; 
	...
	public:
		...
		hkCylinderShape( hkFinishLoadedObjectFlag flag );
...
#include <hkcollide/shape/cylinder/hkCylinderShape.inl>
