...
class hkMultiSphereTriangleAgent : public hkIterativeLinearCastAgent
{
    public:
	...
	protected:
		...
		HK_FORCE_INLINE hkMultiSphereTriangleAgent( const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* contactMgr );

		/// Agent creation function used by the hkCollisionDispatcher. 
        static hkCollisionAgent* HK_CALL createMultiSphereTriangleAgent( const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* contactMgr);
        
		/// Agent creation function used by the hkCollisionDispatcher. 
		static hkCollisionAgent* HK_CALL createTriangleMultiSphereAgent( const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* contactMgr);

	protected:
		hkCollideTriangleUtil::ClosestPointTriangleCache m_closestPointTriangleCache;
		hkContactPointId m_contactPointId[hkMultiSphereShape::MAX_SPHERES];
};
