...
class hkBoxBoxManifold
{
	...
	public:
		...
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_CDINFO, hkBoxBoxManifold );

		hkBoxBoxManifold();

		// for 2.0 bridge
		int addPoint( const hkCdBody& bodyA, const hkCdBody& bodyB, hkFeatureContactPoint& fcp );

		void removePoint( int index );

		inline int getNumPoints();

		inline hkFeatureContactPoint& operator[]( int index );

		inline hkBool findInManifold( const hkFeatureContactPoint& fcp );
	...
	public:
		hkFeatureContactPoint m_contactPoints[HK_BOXBOX_MANIFOLD_MAX_POINTS];
		...
		hkVector4 m_manifoldNormalB;

};


#include <hkcollide/agent/boxbox/hkBoxBoxManifold.inl>
