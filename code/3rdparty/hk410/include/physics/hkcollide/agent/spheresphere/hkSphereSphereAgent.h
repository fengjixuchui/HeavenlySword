...
class hkCollisionDispatcher;
	...
	/// This agent handles collisions between two hkSphereShape s
class hkSphereSphereAgent : public hkIterativeLinearCastAgent
{
    public:
	...
	private:
		...
		inline hkSphereSphereAgent(hkContactMgr* mgr);

		static HK_FORCE_INLINE hkBool getClosestPoint( const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactPoint& contact );

	protected:
        /// Agent creation function used by the hkCollisionDispatcher. 
		static hkCollisionAgent* HK_CALL createSphereSphereAgent(	const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* mgr);
