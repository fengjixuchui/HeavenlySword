...
class hkCollisionDispatcher;
	...
	/// This agent handles collisions between hkSphereShape and hkCapsuleShape
class hkSphereCapsuleAgent : public hkIterativeLinearCastAgent
{
    public:
	...
	protected:
		...
		HK_FORCE_INLINE hkSphereCapsuleAgent( const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* mgr );

        /// Agent creation function used by the hkCollisionDispatcher. 
		static hkCollisionAgent* HK_CALL createSphereCapsuleAgent(const 	hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* mgr);
        
		/// Agent creation function used by the hkCollisionDispatcher. 
		static hkCollisionAgent* HK_CALL createCapsuleSphereAgent(	const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* mgr);
	...
	private:
		static HK_FORCE_INLINE ClosestPointResult getClosestPointInl( const  hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactPoint& cpoint);

	protected:
		static ClosestPointResult HK_CALL getClosestPoint( const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactPoint& cpoint);
