...
struct hkCollisionQualityInfo
{
		...
		/// should be identical to hkCollisionInput.m_collisionTolerance
	...
	//
		...
		/// The negative "total" maximum penetration depth. This parameter is further scaled by hkCollidable::m_allowedPenetrationDepth.
	hkReal m_minSeparation;			// total

		/// The negative maximum penetration depth relative to the current distance. This parameter is further scaled by hkCollidable::m_allowedPenetrationDepth.
	...
	hkReal m_minSafeDeltaTime;
		...
		/// When the distance travelled by a body in that time approaches the body's size, 
	...
	hkReal m_minToiDeltaTime;
		...
		/// else it is not used:	hkConstraintInstance::ConstraintPriority
