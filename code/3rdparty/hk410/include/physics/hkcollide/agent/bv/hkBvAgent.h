...
class hkBvAgent : public hkCollisionAgent
{
	public:			
	...
	protected:
		...
		hkBvAgent(const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* mgr);

		///Destructor, called by cleanup().
		~hkBvAgent(){}

		hkCollisionAgent* m_boundingVolumeAgent;
		hkCollisionAgent* m_childAgent;

			/// Agent creation function used by the hkCollisionDispatcher. 
		static hkCollisionAgent* HK_CALL createBvShapeAgent(	const hkCdBody& bodyA, const hkCdBody& bodyB, const hkCollisionInput& input, hkContactMgr* mgr );
