...
/// This class implements the hkRayHitCollector, and stores all the points the ray hits.
/// It never causes an early out for the ray cast.
class hkAllRayHitCollector : public hkRayHitCollector
{
	public:

			/// Constructor calls reset.
		inline hkAllRayHitCollector();
		...
		inline virtual ~hkAllRayHitCollector();

			/// Get all the hits
		inline const hkArray<hkWorldRayCastOutput>& getHits() const;
	...
	protected:
		hkInplaceArray<hkWorldRayCastOutput,8> m_hits;
};

#include <hkcollide/collector/raycollector/hkAllRayHitCollector.inl>
