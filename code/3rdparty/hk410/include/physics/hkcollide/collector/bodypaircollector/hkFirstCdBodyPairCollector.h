...
#include <hkcollide/collector/bodypaircollector/hkRootCdBodyPair.h>
	...
	/// hkFirstCdBodyPairCollector  collects the first body pair (e.g. from a hkCollisionAgent::getPenetrations() call)
	...
	/// the rood hkCollidable and the (leave) hkShapeKey. If you have only one hkShapeCollection in your shape hierarchy,
...
class hkFirstCdBodyPairCollector : public hkCdBodyPairCollector
{
	public:
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_AGENT, hkFirstCdBodyPairCollector);

			/// Constructor calls reset().
		inline hkFirstCdBodyPairCollector();

		inline virtual ~hkFirstCdBodyPairCollector();
		...
		inline const hkRootCdBodyPair& getHit() const;
	...
	protected:
		...
		hkRootCdBodyPair	m_cdBodyPair;
};

#include <hkcollide/collector/bodypaircollector/hkFirstCdBodyPairCollector.inl>
