...
/// the hkConstraintMotor::motor() callback the difference between the current
...
class hkRagdollConstraintData : public hkConstraintData
{
	public:
		...
		//
			...
			/// Given the relative attached (A) to reference (B) frame, create
			/// the target constraint basis to achieve this relative orientation.
		void setTargetFromRelativeFrame( const hkRotation& bRa );

			/// Get the target frame
		void getTarget(hkMatrix3& targetFrameAinB );

			/// Return the motor used to drive the twist axis angle.
		hkConstraintMotor* getTwistMotor() const;
		...
		void setTwistMotor( hkConstraintMotor* motor );

			/// Return the motor used to drive the cone axis angle.
		hkConstraintMotor* getConeMotor() const;
		...
		void setConeMotor( hkConstraintMotor* motor );

			/// Return the motor used to drive the plane axis angle.
		hkConstraintMotor* getPlaneMotor() const;
			...
			/// Increments reference of new motor, decrements counter of replaced
			/// motor (if any, can be null).
		void setPlaneMotor( hkConstraintMotor* motor );
		...
		void setMotorsActive( hkConstraintInstance* instance, hkBool toBeEnabled );
		...
		virtual int getType() const;
			...
			/// WARNING:  this function will be deprecated, use setTargetFromRelativeFrame instead.
			/// A matrix holding the target constraint space of the attached
			/// body (A) in the reference bodies (B) space. The motors will
			/// work to make the actual constraint space of A line up with this
			/// target. Twist should line up with col0 and plane with col1, col2
			/// should be twist cross plane.
		void setTarget( const hkMatrix3& relativeBasisAinB );
		...
		struct Runtime
		{
			HK_ALIGN16( class hkSolverResults m_solverResults[12/*VC6 doesn't like the scoping for SOLVER_RESULT_MAX*/] );
		...
		struct Atoms
		{
			...
			struct hkRagdollMotorConstraintAtom         m_ragdollMotors;
			struct hkAngFrictionConstraintAtom			m_angFriction;
		...
		/// Increments reference of new motor, decrements counter of
		/// replaced motor (if any, can be null).
		void setMotor(MotorIndex index, hkConstraintMotor* newMotor);
	...
	public:
		...
		hkRagdollConstraintData(hkFinishLoadedObjectFlag f) : hkConstraintData(f), m_atoms(f) {}
};


#include <hkinternal/dynamics/constraint/bilateral/ragdoll/hkRagdollConstraintData.inl>
