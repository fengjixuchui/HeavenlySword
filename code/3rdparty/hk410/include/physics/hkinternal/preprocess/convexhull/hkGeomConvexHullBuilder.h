...
class hkGeomConvexHullBuilder
{
	public:
		...
		static void HK_CALL draw( hkGeomHull& hull, int color, hkTextDisplay* textDisplay );
...
//						 (m_rightEdge->m_vertex == other.m_rightEdge->m_vertex) );
	...
	//protected:
		...
		public: 
			...
			/// Finds the supporting vertices from the input set in 26 directions.
		static void HK_CALL approximateHullFromVerticesWithKDop(const hkVector4 *verticesin, int numverts, hkArray<hkVector4>& planeEquations, hkArray<hkVector4>& usedVertices);
		...
		static void HK_CALL addWrappingLines( const hkGeomConvexHullConfig& config, WeightedLine* sourceLine, hkArray<WeightedNeighbour>& leftN, hkArray<WeightedNeighbour>& rightN, hkArray<WeightedLine>& wrappingLinesOut );
		...
		static void HK_CALL drawPlane( const hkVector4& planeEqn, const hkVector4& centrePoint, int color );

		static hkReal HK_CALL getPseudoAngle( hkReal cosTheta, hkReal sinTheta );
