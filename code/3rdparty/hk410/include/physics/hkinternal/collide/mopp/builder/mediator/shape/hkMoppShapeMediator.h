...
class hkMoppShapeMediator: public hkMoppMediator 
{
		HK_DECLARE_CLASS_ALLOCATOR( HK_MEMORY_CLASS_MOPP );

	public:
		// Mediator constructor 
		hkMoppShapeMediator(const hkShapeContainer* shape);		
		// Shape destructor
		~hkMoppShapeMediator();												
	...
	private:
		HK_FORCE_INLINE void projectPrimitive( const hkMoppCompilerPrimitive &primtiveIn, const hkVector4 &direction, int directionIndex, hkReal* minimum, hkReal* maximum );

	protected:
		const hkShapeContainer* m_shape;
