...
HK_REFLECTION_DEFINE_VIRTUAL(hkVehicleRaycastWheelCollide);
HK_REFLECTION_DEFINE_VIRTUAL(hkRejectRayChassisListener);

hkVehicleRaycastWheelCollide::hkVehicleRaycastWheelCollide()
{
...
void hkVehicleRaycastWheelCollide::init( const hkVehicleInstance* vehicle )
{
	...
	m_phantom = new hkAabbPhantom(pci, m_wheelCollisionFilterInfo);
...
hkVehicleWheelCollide* hkVehicleRaycastWheelCollide::clone( const hkArray<hkPhantom*>& newPhantoms ) const
{
	hkVehicleRaycastWheelCollide* newC = new hkVehicleRaycastWheelCollide();
	newC->m_phantom = (hkAabbPhantom*) newPhantoms[0];
	newC->m_phantom->addReference();

	newC->m_phantom->removePhantomOverlapListener(const_cast<hkRejectRayChassisListener*>(&this->m_rejectRayChassisListener));
	newC->m_phantom->addPhantomOverlapListener(&newC->m_rejectRayChassisListener);
	
	return newC;
}

void hkVehicleRaycastWheelCollide::getPhantoms( hkArray<hkPhantom*>& phantomsOut )
{
...
void hkVehicleRaycastWheelCollide::calcWheelsAABB( const hkVehicleInstance* vehicle, hkAabb& aabbOut )
{
	...
	{
		const hkVehicleInstance::WheelInfo &wheel_info = vehicle->m_wheelsInfo[w_it];// note is non-const &
...
void hkVehicleRaycastWheelCollide::castSingleWheel(const hkVehicleInstance::WheelInfo& wheelInfo, hkVehicleInstance* vehicle, hkWorldRayCastOutput& output)
{
...
void hkVehicleRaycastWheelCollide::calcSingleWheelGroundFriction( hkVehicleInstance* vehicle, hkInt8 wheelInfoNum, const hkWorldRayCastOutput& worldRayCastOutput, hkReal& frictionOut ) const
{
...
void hkVehicleRaycastWheelCollide::updatePhantom( hkVehicleInstance* vehicle )
{
	hkAabb aabb;
	calcWheelsAABB( vehicle, aabb );
...
void hkVehicleRaycastWheelCollide::collideWheels(const hkReal deltaTime, hkVehicleInstance* vehicle, CollisionDetectionWheelOutput* cdInfoOut )
{
	// Check if the phantom has been added to the world.
	HK_ASSERT2(0x676876e3, m_phantom->getWorld() != HK_NULL, "The phantom for the wheelCollide component must be added to the world before using a hkVehicleInstance.");
	...
	for (hkInt8 w_it=0; w_it< vehicle->m_data->m_numWheels; w_it ++)
	{
		...
		const hkVehicleInstance::WheelInfo& wheel_info = vehicle->m_wheelsInfo[w_it];
		//
		// RAY CAST
		//
		...
		if (output.hasHit())
		{
			...
			for( int i = 0; i < hkShapeRayCastOutput::MAX_HIERARCHY_DEPTH && output.m_shapeKeys[i] != HK_INVALID_SHAPE_KEY; ++i )
			{
				cd_wheelInfo.m_contactShapeKey = output.m_shapeKeys[i];
			}

			hkRigidBody* groundRigidBody = hkGetRigidBody(output.m_rootCollidable);
			HK_ASSERT2(0x1f3b75d4,  groundRigidBody, 
				...
				\nTo do this, change hkVehicleRaycastWheelCollide::m_wheelCollisionFilterInfo or hkCollisionFilter::isCollisionEnabled( const hkCollidable& a, const hkCollidable& b )");
...
hkRejectRayChassisListener::hkRejectRayChassisListener()
{
	m_chassis = HK_NULL;
}
...
void hkRejectRayChassisListener::collidableAddedCallback(  const hkCollidableAddedEvent& event )
{
...
void hkRejectRayChassisListener::collidableRemovedCallback( const hkCollidableRemovedEvent& event )
{
}
