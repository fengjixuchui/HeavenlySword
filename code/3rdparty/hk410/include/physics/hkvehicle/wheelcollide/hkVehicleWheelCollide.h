...
/// ground.
class hkVehicleWheelCollide : public hkReferencedObject
{
	public:
		...
		struct CollisionDetectionWheelOutput
		{
			...
			hkReal m_suspensionRelativeVelocity;
				...
				/// Clipping is affected by hkVehicleData::m_normalClippingAngle.
		...
		virtual void collideWheels(const hkReal deltaTime, hkVehicleInstance* vehicle, CollisionDetectionWheelOutput* cdInfoOut) = 0;

		virtual void init( const hkVehicleInstance* vehicle ) = 0;

		virtual void getPhantoms( hkArray<hkPhantom*>& phantomsOut ) = 0;

			/// As all other parts of the vehicle can usually be shared, except for the wheel collide.
		virtual hkVehicleWheelCollide* clone( const hkArray<hkPhantom*>& newPhantoms ) const = 0;
