...
HK_REFLECTION_DEFINE_VIRTUAL(hkVehicleInstance);

hkVehicleInstance::hkVehicleInstance( hkRigidBody* chassis ) :
	hkUnaryAction(chassis)
{
	m_data = HK_NULL;
...
void hkVehicleInstance::setChassis ( hkRigidBody* chassis )
{
	setEntity(chassis);
}

void hkVehicleInstance::WheelInfo::init()
{
	...
	m_contactShapeKey = hkShapeKey(-1);
	...
	m_spinAxisCs.set(1.0f, 0.0f, 0.0f); //@@@PS3: temp-workaround for race condition when starting vehicle demos
...
hkVehicleInstance::~hkVehicleInstance()
{
...
void hkVehicleInstance::init()
{
	...
	{  // set diagonal of rot inertia tensor for normal stuff
		...
		HK_ASSERT2(0x5c819b4e, getChassis()->getMotionType() == hkMotion::MOTION_BOX_INERTIA, "Vehicle chassis MUST be of type hkRigidBodyCinfo::MOTION_BOX_INERTIA!");
	}
...
void hkVehicleInstance::handleFixedGroundAccum( hkRigidBody* ground, hkVelocityAccumulator& accum )
{
	accum.setFixed();
}

void hkVehicleInstance::updateWheelsHardPoints(const hkTransform &car_transform)
{
...
// Apply action method
void hkVehicleInstance::applyAction(const hkStepInfo& stepInfo)
{
	...
	hkVehicleWheelCollide::CollisionDetectionWheelOutput* cdInfo = hkAllocateStack<hkVehicleWheelCollide::CollisionDetectionWheelOutput>( m_data->m_numWheels);
	{
	...
	HK_INTERNAL_TIMER_SPLIT_LIST("components");
	    ...
	    // Driver Input
	    //
	hkVehicleDriverInput::FilteredDriverInputOutput filteredDriverInputInfo;
	{
		{
		...
		//	Steering
		//
	hkVehicleSteering::SteeringAnglesOutput steeringInfo;
	{
		...
		//	Transmission
		//
	hkVehicleTransmission::TransmissionOutput transmissionInfo;
	{
		...
		//	Engine
		//
	hkVehicleEngine::EngineOutput engineInfo;
	{
		...
		//
	hkVehicleBrake::WheelBreakingOutput wheelBreakingInfo;
	{
	...
	hkVehicleAerodynamics::AerodynamicsDragOutput aerodynamicsDragInfo;
	{
	...
	
	{
		...
		// ================================================================================================================
		{
			...
			hkMotion* chassis = getChassis()->getRigidMotion();
		...
		// Per Wheel Behaviors
		// ================================================================================================================
		hkVehicleFrictionSolverParams frictionParams;
		for (int i = 0; i < 2; i++)
		{
		...
		hkRigidBody *groundBody[2] = { HK_NULL, HK_NULL };
		...
		for (int w_it = 0 ; w_it < m_data->m_numWheels ; w_it++)
		{
			...
			const hkReal suspension_force = suspensionForces[w_it];
			{
				...
				hkRigidBody* ground = cdInfo[w_it].m_contactBody;
				if (ground != HK_NULL)
				{
			...
			hkVehicleFrictionSolverAxleParams &axle_params = frictionParams.m_axleParams[axle];
		...
		// =========================================================
		hkVelocityAccumulator groundAccum[2];
		hkVelocityAccumulator groundAccumAtIntegration[2];
		{
			...
			{
				hkVehicleFrictionSolverAxleParams &axle_params = frictionParams.m_axleParams[ax_it];
	...
	#endif
				hkRigidBody* ground = groundBody[ax_it];
				...
				{
					hkMotion* motion = ground->getRigidMotion();
		...
		// =========================================================
		{
			hkRigidBody* chassis = getChassis();
			hkMotion* motion = chassis->getRigidMotion();
		...
		// =================================================================================
		{
			...
			for (int ax_it=0; ax_it<m_data->m_numWheelsPerAxle.getSize(); ax_it++)
			{
				...
				if ( groundBody[ax_it] && !groundBody[ax_it]->isFixed() )
				{
					...
					{
						hkMotion* motion = groundBody[ax_it]->getRigidMotion();
		...
		// =================================================================================
		{
			for (int w_it=0; w_it< m_data->m_numWheels; w_it++)
			{
				...
				hkVehicleFrictionStatus::AxisStatus& astat = m_frictionStatus.m_axis[axle];
...
void hkVehicleInstance::calcCurrentPositionAndRotation( const hkRigidBody* chassis, const hkVehicleSuspension* suspension, int wheelNo, hkVector4& posOut, hkQuaternion& rotOut )
{
...
hkReal hkVehicleInstance::calcRPM() const
{
	return m_rpm;
}
...
hkReal hkVehicleInstance::calcKMPH() const
{
...
hkReal hkVehicleInstance::calcMPH() const
{
	const hkReal vel_ms = getChassis()->getLinearVelocity().length3();
...
void hkVehicleInstance::getPhantoms( hkArray<hkPhantom*>& phantomsOut )
{
	m_wheelCollide->getPhantoms( phantomsOut );
}

hkAction* hkVehicleInstance::clone( const hkArray<hkEntity*>& newEntities, const hkArray<hkPhantom*>& newPhantoms ) const
{ 
	hkRigidBody* newChassis = (hkRigidBody*) newEntities[0];
	hkVehicleInstance* newV = new hkVehicleInstance( newChassis );
	*newV = *this;
...
void hkVehicleInstance::calcStatistics( hkStatisticsCollector* collector ) const
{
