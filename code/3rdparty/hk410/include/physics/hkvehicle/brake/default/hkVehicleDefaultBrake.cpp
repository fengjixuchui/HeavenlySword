...
HK_REFLECTION_DEFINE_VIRTUAL(hkVehicleDefaultBrake);

hkVehicleDefaultBrake::hkVehicleDefaultBrake()
{
...
void hkVehicleDefaultBrake::calcBreakingInfo( const hkReal deltaTime, const hkVehicleInstance* vehicle, const hkVehicleDriverInput::FilteredDriverInputOutput& filteredDriverInfo, WheelBreakingOutput& breakingInfo )
{
	...
	hkBool wheelsShouldBlock = false;
	{
		for (int w_it=0; w_it < vehicle->m_data->m_numWheels; w_it++)
		{
			...
			// NORMAL (PEDAL) BRAKE
			{
				...
				HK_ASSERT2(0xad45dee3, deltaTime > HK_REAL_EPSILON, "Non-zero deltaTime required for hkVehicleDefaultBrake::calcBreakingInfo.");
