...
HK_REFLECTION_DEFINE_VIRTUAL(hkTyremarksWheel);
HK_REFLECTION_DEFINE_VIRTUAL(hkTyremarksInfo);

hkTyremarksWheel::hkTyremarksWheel()
{
	m_numPoints = 0;
	m_currentPosition = 0;
...
void hkTyremarksWheel::setNumPoints( int num_points )
{
...
const hkTyremarkPoint& hkTyremarksWheel::getTyremarkPoint( int point) const
{
...
hkReal hkTyremarkPoint::getTyremarkStrength() const
{ 
	 return m_pointRight(3);
}

void hkTyremarksWheel::addTyremarkPoint( hkTyremarkPoint& point)
{
...
hkTyremarksInfo::hkTyremarksInfo(const hkVehicleData& data, int num_points)
{
	...
	for (int i = 0; i < m_tyremarksWheel.getSize() ; i++ )
	{
		hkTyremarksWheel* tyremarksWheel = new hkTyremarksWheel();
		tyremarksWheel->setNumPoints(num_points);
		m_tyremarksWheel[i] = tyremarksWheel;
	}
}

hkTyremarksInfo::~hkTyremarksInfo()
{
...
void hkTyremarksInfo::updateTyremarksInfo( hkReal timestep, const hkVehicleInstance* vehicle )
{
...
void hkTyremarksInfo::getWheelTyremarksStrips( const hkVehicleInstance* vehicle, int wheel, hkVector4 *strips_out) const
{

	const hkTyremarksWheel &tyremarks_wheel = *m_tyremarksWheel[wheel];
	const int num_points = tyremarks_wheel.m_numPoints;

	for (int p_it=0; p_it< num_points; p_it++)
	{
		const hkTyremarkPoint &tyre_point = tyremarks_wheel.getTyremarkPoint(p_it);
