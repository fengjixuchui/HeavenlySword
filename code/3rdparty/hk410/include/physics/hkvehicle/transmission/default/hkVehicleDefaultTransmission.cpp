...
HK_REFLECTION_DEFINE_VIRTUAL(hkVehicleDefaultTransmission);

hkVehicleDefaultTransmission::hkVehicleDefaultTransmission()
{
	m_downshiftRPM = 0;
	m_upshiftRPM = 0;
...
hkReal hkVehicleDefaultTransmission::getCurrentRatio( const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut ) const
{
...
hkReal HK_CALL hkVehicleDefaultTransmission::calculatePrimaryTransmissionRatio( const hkReal vehicleTopSpeed,
																			   const hkReal wheelRadius,
																			   const hkReal maxEngineRpm,
																			   const hkReal topGearRatio )
{
	HK_ASSERT2(0x2a408c39, vehicleTopSpeed	> 0.0f, "Invalid vehicleTopSpeed passed to hkVehicleDefaultTransmission::calculatePrimaryTransmissionRatio");
	HK_ASSERT2(0x413737ff, wheelRadius		> 0.0f, "Invalid wheelRadius passed to hkVehicleDefaultTransmission::calculatePrimaryTransmissionRatio");
	HK_ASSERT2(0x5fdc2f33, maxEngineRpm		> 0.0f, "Invalid maxEngineRpm passed to hkVehicleDefaultTransmission::calculatePrimaryTransmissionRatio");
	HK_ASSERT2(0x79faa2d6, topGearRatio		> 0.0f, "Invalid topGearRatio passed to hkVehicleDefaultTransmission::calculatePrimaryTransmissionRatio");
...
void hkVehicleDefaultTransmission::calcTransmission( const hkReal deltaTime, const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut )
{
#if defined HK_DEBUG
...
hkReal hkVehicleDefaultTransmission::calcMainTransmittedTorque( const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut ) const 
{
...
hkReal hkVehicleDefaultTransmission::calcTransmissionRPM( const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut ) const
{
#if defined HK_DEBUG
	checkTotalWheelTorqueRatio( vehicle );
#endif // #if defined HK_DEBUG
	...
	for (w_it=0; w_it< vehicle->m_data->m_numWheels; w_it++)
	{
		...
		const hkReal wheel_rpm = wheel_info.m_spinVelocity * sec_min * rev_rad;
		average_wheel_rpm += wheel_rpm * m_wheelsTorqueRatio[w_it];
	}

	const hkReal cur_ratio = getCurrentRatio( vehicle, transmissionOut );
	const hkReal current_rpm = average_wheel_rpm * cur_ratio;

	return current_rpm;
}

void hkVehicleDefaultTransmission::updateCurrentGear( const hkReal deltaTime, const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut )
{
...
hkBool hkVehicleDefaultTransmission::calcIsReversing( const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut ) const
{
...
#if defined HK_DEBUG
void hkVehicleDefaultTransmission::checkTotalWheelTorqueRatio( const hkVehicleInstance* vehicle ) const
{
