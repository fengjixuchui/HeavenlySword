...
/// The hkVehicleDefaultTransmission class implements an automatic transmission, with n
/// gears and a reverse gear, different torque ratios for each wheel (so FWD - RWD -
/// AWD can be specified) and clutch delay.
class hkVehicleDefaultTransmission : public hkVehicleTransmission
{
	public:
		...
		virtual void calcTransmission(const hkReal deltaTime, const hkVehicleInstance* vehicle, TransmissionOutput& TransmissionOutputOut);
	...
	public:
		...
		virtual hkReal calcMainTransmittedTorque(const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut) const;
		
			/// Calculates the transmission RPM.
		virtual hkReal calcTransmissionRPM(const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut) const;
		
			/// Calculates if the vehicle is currently reversing.
		virtual hkBool calcIsReversing(const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut) const;
		
			/// Calculates the current gear.
		virtual void updateCurrentGear( const hkReal deltaTime, const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut );
		
			/// Returns the gear ratio of the current gear.
		virtual hkReal getCurrentRatio(const hkVehicleInstance* vehicle, TransmissionOutput& transmissionOut) const;
	...
	protected:
		...
		void checkTotalWheelTorqueRatio( const hkVehicleInstance* vehicle ) const;
	#endif // #if defined HK_DEBUG

	public:
		hkVehicleDefaultTransmission(hkFinishLoadedObjectFlag f) : m_gearsRatio(f), m_wheelsTorqueRatio(f) { }
};
