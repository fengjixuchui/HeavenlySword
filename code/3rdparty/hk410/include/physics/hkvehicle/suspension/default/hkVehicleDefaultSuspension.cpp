...
HK_REFLECTION_DEFINE_VIRTUAL(hkVehicleDefaultSuspension);

void hkVehicleDefaultSuspension::calcSuspension( const hkReal deltaTime, const hkVehicleInstance* vehicle, const hkVehicleWheelCollide::CollisionDetectionWheelOutput* cdInfo, hkReal* suspensionForceOut )
{
	...
	for (int w_it=0; w_it< vehicle->m_data->m_numWheels; w_it++)
	{
		...
		if ( cdInfo[w_it].m_contactBody )
		{
			...
			//
			// From hkVehicleWheelCollide::CollisionDetectionWheelOutput
			//
