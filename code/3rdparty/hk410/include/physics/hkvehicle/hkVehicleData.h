...
/// Contains data about a vehicle that does not change after initialisation.
/// This means that a hkVehicleData class can be shared between multiple
/// vehicles.
class hkVehicleData : public hkReferencedObject
{
	public:
		...
		hkVehicleData();
			...
			/// The init function must be called after all the components of the vehicle have been initialised.
		virtual void init( const hkArray<struct hkVehicleSuspension::SuspensionWheelParameters>& suspensionWheelParams, hkRigidBody* chassis);
	...
	public:
		...
		hkReal m_normalClippingAngle;
			...
			/// Contains information that affects the behaviour of each wheel.
		...
		struct hkVehicleFrictionDescription m_frictionDescription;
	...
	public:
		...
		hkVehicleData( hkFinishLoadedObjectFlag f ) : m_wheelParams(f), m_numWheelsPerAxle(f) { }
