...
public:
	...
	hkEnum<hkAccumulatorType,hkUint8> m_type;	

		/// set to true if coreFromWorldMatrix is the identity matrix
	hkBool m_matrixIsIdentity;	

		/// An internal counter, counting down for every frame the object does not move
	hkUint32 m_deactivationCounter;

		/// An index into hkSolverInfo::m_deactivationInfo which is taken from
		/// hkRigidBodyCinfo::m_solverDeactivation
	...
	hkVector4 m_angularVel;
		...
		/// + the w component is the inverted mass
...
public:
	/// Set this velocity accumulater to the one of a fixed object
	HK_FORCE_INLINE void setFixed()
	{
		m_type = hkVelocityAccumulator::HK_KEYFRAMED_RIGID_BODY;
