/* 
 * 
 * Confidential Information of Telekinesys Research Limited (t/a Havok).  Not for disclosure or distribution without Havok's
 * prior written consent.This software contains code, techniques and know-how which is confidential and proprietary to Havok.
 * Level 2 and Level 3 source code contains trade secrets of Havok. Havok Software (C) Copyright 1999-2006 Telekinesys Research Limited t/a Havok. All Rights Reserved. Use of this software is subject to the terms of an end user license agreement.
 * 
 */

#ifndef HK_DYNAMICS2_ACTION_H
#define HK_DYNAMICS2_ACTION_H


#include <hkmath/hkMath.h>
#include <hkdynamics/entity/hkEntityListener.h>

class hkEntity;
class hkPhantom;
class hkSimulationIsland;
class hkStepInfo;
class hkWorld;

extern const hkClass hkActionClass;

/// This is the base class from which user actions (or controllers) are derived. Actions 
/// are the interface between user controllable behavior of the physical simulation and the Havok core. 
class hkAction : public hkReferencedObject
{
	public:

		HK_DECLARE_REFLECTION();

		HK_DECLARE_CLASS_ALLOCATOR(HK_MEMORY_CLASS_ACTION);

			/// Default constructor
		inline hkAction(hkUint32 userData);
		
			/// Virtual Destructor.
		virtual ~hkAction() { }

			/// The applyAction() method does the actual work of the action, and is called at every simulation step.
		virtual void applyAction( const hkStepInfo& stepInfo ) = 0;


			/// This method is used to maintain simulation islands.
			/// An action that requires that the entities it operates on are simulated together
			/// must return these entities in this function call. See the hkBinaryAction 
			/// implementation for an example.
		virtual void getEntities( hkArray<hkEntity*>& entitiesOut ) = 0;

			/// Get referenced phantoms. This is used in order to generically clone hkAction objects
			/// that reference phantoms. This has a default implementation that returns no phantoms.
		virtual void getPhantoms( hkArray<hkPhantom*>& phantomsOut ) {}

			/// Returns the hkWorld that owns this action.
		inline hkWorld* getWorld();

			// hkEntityListener-like interface implementation
		virtual void entityRemovedCallback(hkEntity* entity) = 0;

			/// Get the user data for the action
		inline hkUint32 getUserData() const;

			/// Set the user data of the action.
		inline void setUserData( hkUint32 data );

			/// Get the name of this action.
		inline const char* getName() const;

			/// Set the name of this action.
			/// IMPORTANT: This data will not be cleaned up by the hkAction destructor. You are required to track it yourself.
		inline void setName( const char* name );

			/// In order to clone an action, with a varying number of dependant rigid bodies
			/// that you would like to repoint the clone at (and null the world ptr etc)
			/// we provide a list of entities, in the same order as given by getEntities()
			/// that are the new cloned bodies to use in the process.
			/// We also provide a list of phantoms. Actions do not generally reference phantoms,
			/// but there are exceptions, as in the case of hkVehicleInstance.
		virtual hkAction* clone( const hkArray<hkEntity*>& newEntities, const hkArray<hkPhantom*>& newPhantoms ) const = 0;

	private:

		hkWorld* m_world; //+nosave
		hkSimulationIsland* m_island; //+nosave

	protected:
		hkUint32 m_userData;

		const char* m_name;

		friend class hkWorld;
		friend class hkWorldOperationUtil;
		friend class hkSimulationIsland;

		//
		// Internal methods
		//
	public:

			// Gets the simulation island to which the action belongs.
		inline hkSimulationIsland* getSimulationIsland();

	private:
	
			// Sets the simulation island.
		inline void setSimulationIsland(hkSimulationIsland* island);

			// Sets the hkWorld that owns this action.
		inline void setWorld( hkWorld* owner );

	public:
		hkAction( class hkFinishLoadedObjectFlag flag ) {}
};

#include <hkdynamics/action/hkAction.inl>

#endif // HK_DYNAMICS2_ACTION_H

/*
* Havok SDK - PUBLIC RELEASE, BUILD(#20060902)
*
* Confidential Information of Havok.  (C) Copyright 1999-2006 
* Telekinesys Research Limited t/a Havok. All Rights Reserved. The Havok
* Logo, and the Havok buzzsaw logo are trademarks of Havok.  Title, ownership
* rights, and intellectual property rights in the Havok software remain in
* Havok and/or its suppliers.
*
* Use of this software for evaluation purposes is subject to and indicates 
* acceptance of the End User licence Agreement for this product. A copy of 
* the license is included with this software and is also available from salesteam@havok.com.
*
*/
