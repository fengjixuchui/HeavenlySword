...
extern "C"
{
		...
		/// Apart from that it is also allowed to pass in a pointer to an array of any other class, e.g. hkEntity. In this case you have to manually
		/// supply the motion's offset within this different class (e.g. motionOffset = HK_OFFSET_OF(hkEntity, m_motion) )
	...
	int hkRigidMotionUtilApplyForcesAndStep( const struct hkSolverInfo& solverInfo, const hkStepInfo& info, const hkVector4& deltaVel, hkMotion*const* motions, int numMotions, int motionOffset );
		...
		///
		/// This function is capable of handling batches: see hkRigidMotionUtilStep() for details.
	hkVelocityAccumulator* hkRigidMotionUtilApplyForcesAndBuildAccumulators(const hkStepInfo&				info,
																			hkMotion*const*					motions,
																			int								numMotions,
																			int								motionOffset,
																			hkVelocityAccumulator*			accumulatorsOut);
	...
	int hkRigidMotionUtilApplyAccumulators( const struct hkSolverInfo& solverInfo,
											const hkStepInfo&				info,
											const hkVelocityAccumulator*	accumulators,
											      hkMotion*const*			motions,
												  int						numMotions,
												  int						motionOffset,
												  hkBool processDeactivation = true);
