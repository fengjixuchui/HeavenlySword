...
extern const hkClass hkMotionClass;
	...
	/// An object's hkMotion stores all the information relating to
	/// how it moves, including its mass, position, and velocity.
	/// A hkMotion is owned by exactly one object, usually a hkEntity
	/// or another hkMotion.
	/// Each hkRigidBody has an hkMotion that stores transform, velocity, and mass information
	/// for the object.
	/// Do not access this class directly - use hkRigidBody functions instead.
...
public:
	...
	/// A list of all motion types. The motion type of a hkRigidBody determines what
	...
	enum MotionType
	{
		...
		/// positions. The hkKeyFrameUtility class can be used to simply apply keyframes
...
public:
	...
	hkMotion(const hkVector4& position, const hkQuaternion& rotation, bool wantDeactivation = false);
	...
	//
	/// get the deactivation class as defined in hkSolverInfo::DeactivationClass
	inline int getDeactivationClass();

	/// Gets the deactivation counter. If the counter is 0, the solver decided that the object is deactivated
	inline int getDeactivationCounter();

	/// set the deactivation class as defined in hkSolverInfo::DeactivationClass
	void setDeactivationClass(int deactivationClass);

	/// sets the deactivation counter
	inline void setDeactivationCounter( int newCounter );
	...
	inline void enableDeactivation( bool value, int randomNumber = 0);
...
public:
	...
	virtual void getMotionStateAndVelocitiesAndDeactivationType(hkMotion* motionOut);
public:
...
public:
	...
	hkUint8		m_deactivationNumInactiveFrames[2];
	...
	hkVector4 m_deactivationRefPosition[ 2 ];
		...
		// the packed m_deactivationRefOrientation (stored in the motionState motion (so that sizeof(hkEntity) < 512))
...
public:
	...
	hkMotion( class hkFinishLoadedObjectFlag flag ) { }
};

class hkRigidMotion: public hkMotion
{
	hkRigidMotion( class hkFinishLoadedObjectFlag flag ): hkMotion(flag) { }
};

#include <hkdynamics/motion/hkMotion.inl>
