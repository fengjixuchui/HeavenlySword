...
hkShapePhantom::hkShapePhantom( const hkShape* shape, const hkTransform& transform )
	: hkPhantom( shape )
{
	HK_ASSERT2(0xad65bbd5, m_collidable.getShape(), "hkShapePhantoms must be created with non-null shapes.");
	
	hkCollidable* collidable = getCollidableRw();
...
hkMotionState* hkShapePhantom::getMotionState()
{
...
void hkShapePhantom::setTransform( const hkTransform& transform )
{
	HK_ACCESS_CHECK_WITH_PARENT( m_world, HK_ACCESS_RW, this, HK_ACCESS_RW );
	// re: HVK-2304 check for normalised Tr in hkShapePhantom::setTransform
	HK_ASSERT2(0xad6bd8ad, transform.getRotation().isOrthonormal(), "The rotation of the transform passed to hkShapePhantom::setTransform() is not orthonormal.");
	m_motionState.getTransform() = transform;
...
void hkShapePhantom::setPosition( const hkVector4& position, hkReal extraTolerance )
{
	HK_ACCESS_CHECK_WITH_PARENT( m_world, HK_ACCESS_RW, this, HK_ACCESS_RW );
...
void hkShapePhantom::calcAabb( hkAabb& aabb )
{
	HK_ACCESS_CHECK_WITH_PARENT( m_world, HK_ACCESS_IGNORE, this, HK_ACCESS_RO );
	HK_ASSERT2(0xad65bbd4, m_collidable.getShape(), "hkShapePhantoms must be created with non-null shapes.");
	HK_ASSERT2(0x525a9acd,  m_world, "You can only call calcAabb, after the phantom has been added to the world");
	hkReal halfTolerance = 0.5f * m_world->getCollisionInput()->getTolerance();
...
hkWorldOperation::Result hkShapePhantom::setShape( hkShape* shape )
{
	...
	if (m_world)
	{
		...
		hkWorldOperationUtil::removePhantomBP(m_world, this);
	}
	...
	if (m_world)
	{
		hkWorldCallbackUtil::firePhantomShapeSet( m_world, this );
	}
	this->firePhantomShapeSet();

	if (m_world)
	{
		hkWorldOperationUtil::addPhantomBP(m_world, this);
