...
class hkRigidBodyCinfo
{
	public:
		...
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_ENTITY, hkRigidBodyCinfo);
	...
	public:
		...
		hkUint32 m_collisionFilterInfo;
			...
			/// This defaults to HK_NULL, and must be set before constructing a hkRigidBody.
		const hkShape* m_shape;

			/// The collision response. See hkMaterial::hkResponseType for hkWorld default
			/// implementations.
			/// This defaults to hkMaterial::RESPONSE_SIMPLE_CONTACT.
		hkEnum<enum hkMaterial::ResponseType, hkInt8> m_collisionResponse;
		...
		hkVector4 m_angularVelocity;
			...
			/// The inertia tensor of the rigid body. Use the hkInertiaTensorComputer class to
		...
		hkReal m_allowedPenetrationDepth;
			...
			/// This defaults to hkMotion::MOTION_DYNAMIC
		hkEnum<enum hkMotion::MotionType, hkInt8> m_motionType;
		
			/// The initial deactivator type of the body.
			/// This defaults to hkRigidBodyDeactivator::DEACTIVATOR_SPATIAL.
		hkEnum<enum hkRigidBodyDeactivator::DeactivatorType, hkInt8> m_rigidBodyDeactivatorType;
		...
		hkEnum<enum SolverDeactivation, hkInt8> m_solverDeactivation;
		
			/// The quality type, used to specify when to use continuous physics
			/// This defaults to HK_COLLIDABLE_QUALITY_INVALID			
	 		/// If you add a hkRigidBody to the hkWorld, this type automatically gets converted to either
		    /// HK_COLLIDABLE_QUALITY_FIXED, HK_COLLIDABLE_QUALITY_KEYFRAMED or HK_COLLIDABLE_QUALITY_DEBRIS
		hkEnum<enum hkCollidableQualityType, hkInt8> m_qualityType;
