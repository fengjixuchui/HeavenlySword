...
class hkSpuDmaWriter
{
	public:
		...
		HK_FORCE_INLINE void init(HK_CPU_PTR(void*) dstInMainMemoryVirtualBase, HK_CPU_PTR(void*) dstInMainMemory, int size, void* bufferOnSpu, int baseBufferSize, int overflowBufferSize, int dmaGroup0, int dmaGroup1);
		...
		HK_FORCE_INLINE void* getVirtualBase();
	...
	protected:
		...
		hkPadSpuLong<HK_CPU_PTR(void*)> m_dstVirtualBase;
		
			// Current position in the destination buffer in main memory
		hkPadSpuLong<HK_CPU_PTR(void*)> m_currentDstPosition;
		...
		hkPadSpuLong<HK_CPU_PTR(void*)> m_dstEnd;
...
#include <hkdynamics/world/simulation/multithreaded/spu/utilities/hkSpuDmaWriter.inl>
