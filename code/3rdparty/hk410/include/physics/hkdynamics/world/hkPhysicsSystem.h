...
class hkPhysicsSystem : public hkReferencedObject
{
	public:
		...
		hkPhysicsSystem* clone() const;
		
			/// Shallow copy a system. Just copies arrays of ptrs.
		void copy(const hkPhysicsSystem& toCopy);

			/// Add a rigidbody to the list
		void addRigidBody( hkRigidBody* rb );

			/// Add a phantom to the list
		void addPhantom(  hkPhantom* p );

			/// Add a constraint to the list
		void addConstraint( hkConstraintInstance* c );

			/// Add an action to the list
		void addAction( hkAction* a );
		...
		inline const hkArray<hkRigidBody*>& getRigidBodies() const;

			/// Get the held array of phantoms
		inline const hkArray<hkPhantom*>& getPhantoms() const;

			/// Get the held array of constraints
		inline const hkArray<hkConstraintInstance*>& getConstraints() const;
	...
	protected:
		...
		hkArray<class hkRigidBody*> m_rigidBodies;

		hkArray<class hkConstraintInstance*> m_constraints;

		hkArray<class hkAction*> m_actions;

		hkArray<class hkPhantom*> m_phantoms;
	...
	public:
		...
		hkPhysicsSystem( hkFinishLoadedObjectFlag f ) : m_rigidBodies(f), m_constraints(f), m_actions(f), m_phantoms(f) { } 
};

#include <hkdynamics/world/hkPhysicsSystem.inl>
