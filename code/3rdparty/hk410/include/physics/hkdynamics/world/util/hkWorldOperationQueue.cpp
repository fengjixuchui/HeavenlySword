...
hkWorldOperationQueue::hkWorldOperationQueue(hkWorld* world) : m_queueOperationCriticalSection(4000)
{ 
	m_world = world; 
...
void hkWorldOperationQueue::queueOperation(const hkWorldOperation::BaseOperation& operation )
{
	...
	if (    m_world && m_world->m_simulationType == hkWorldCinfo::SIMULATION_TYPE_MULTITHREADED
		&& (m_world->m_multiThreadLock.m_threadId == hkMultiThreadLock::LOCKED_RO ||
			m_world->m_multiThreadLock.m_threadId == hkMultiThreadLock::UNLOCKED   )         )
	{
	...
	switch(op.m_type)
	{
		...
		case hkWorldOperation::ENTITY_BATCH_ADD:
		{
			...
			cop.m_entities = hkAllocateChunk<hkEntity*>(cop.m_numEntities, HK_MEMORY_CLASS_DYNAMICS);
			const hkWorldOperation::AddEntityBatch& coperation = static_cast<const hkWorldOperation::AddEntityBatch&>(operation);
			hkString::memCpy(cop.m_entities, coperation.m_entities, sizeof(hkEntity*) * coperation.m_numEntities);
			hkEntity** e = cop.m_entities;
			hkEntity** eEnd = e + cop.m_numEntities;
			while (e < eEnd)
			{
				addReferenceTo( (e++)[0] );
			}
		...
		case hkWorldOperation::PHANTOM_BATCH_ADD:
		{
			...
			cop.m_phantoms = hkAllocateChunk<hkPhantom*>(cop.m_numPhantoms, HK_MEMORY_CLASS_DYNAMICS);
			const hkWorldOperation::AddPhantomBatch& coperation = static_cast<const hkWorldOperation::AddPhantomBatch&>(operation);
			hkString::memCpy(cop.m_phantoms, coperation.m_phantoms, sizeof(hkPhantom*) * coperation.m_numPhantoms);
			hkPhantom** p = cop.m_phantoms;
			hkPhantom** pEnd = p + cop.m_numPhantoms;
			while (p < pEnd)
			{
				addReferenceTo( (p++)[0] );
			}
		...
		case hkWorldOperation::WORLD_OBJECT_REMOVE_REFERENCE:
		{
			...
			HK_ASSERT2(0xad67ddbb, cop.m_worldObject->getReferenceCount() > 1, "Error. Deleting a hkWorldObject which is still in hkWorld.");
...
void hkWorldOperationQueue::executeAllPending()
{
	...
	HK_ASSERT2(0xad005002, !m_world->areCriticalOperationsLocked() && !m_world->m_blockExecutingPendingOperations, "Attempting to execute pending operations while the world is still hkWorld::areCriticalOperationsLocked() or blocked by hkWorld::m_blockExecutingPendingOperations.");

	{
		...
		if (m_islandMerges.getSize())
		{
			...
			m_pending.insertAt(0, m_islandMerges);
			m_islandMerges.clear();
		}
	}

	hkInplaceArray<hkWorldOperation::BiggestOperation,16> stolenPending;
	stolenPending.swap( m_pending );
	HK_ON_DEBUG( hkDebugInfoOnPendingOperationQueues::addNextPendingOperationQueue(m_world, &stolenPending); );
	m_world->m_pendingOperationQueueCount++;

	for (int i = 0; i < stolenPending.getSize(); i++)
	{
		HK_ON_DEBUG( hkDebugInfoOnPendingOperationQueues::updateNextPendingOperationIndex(m_world, i+1); );
...
#				endif
			...
			case hkWorldOperation::ENTITY_BATCH_ADD:
			{
				...
				hkDeallocateChunk<hkEntity*>(cop.m_entities, cop.m_numEntities, HK_MEMORY_CLASS_DYNAMICS);
				break;
			}
			case hkWorldOperation::ENTITY_BATCH_REMOVE:
			{
...
#				ifndef HK_FORGIVE_FAULTY_OPERATIONS
					HK_ASSERT2(0xf0ff00a6, cop.m_constraint->getOwner() && static_cast<hkSimulationIsland*>(cop.m_constraint->getOwner())->getWorld() == m_world, "Problems with adding a constraint: Probable cause: or attempting to add the same constraint to two worlds immediately.");
#				endif // !HK_ENABLE_EXTENSIVE_WORLD_CHECKING
...
#				ifndef HK_FORGIVE_FAULTY_OPERATIONS
					HK_ASSERT2(0xf0ff00a6, cop.m_action->getOwner() && static_cast<hkSimulationIsland*>(cop.m_action->getOwner())->getWorld() == m_world, "Problems with adding a action: Probable cause: or attempting to add the same action to two worlds immediately. Or phantom ");
#				endif // !HK_ENABLE_EXTENSIVE_WORLD_CHECKING
			...
			case hkWorldOperation::ISLAND_MERGE:
			{
				...
				if ( cop.m_entities[0]->getWorld() == m_world && cop.m_entities[1]->getWorld() == m_world)
				{
					...
					hkWorldOperationUtil::mergeIslandsIfNeeded(cop.m_entities[0], cop.m_entities[1]);
				}
...
#				endif
			...
			case hkWorldOperation::PHANTOM_BATCH_ADD:
			{
				...
				hkDeallocateChunk<hkPhantom*>(cop.m_phantoms, cop.m_numPhantoms, HK_MEMORY_CLASS_DYNAMICS);
				break;
			}
			case hkWorldOperation::PHANTOM_BATCH_REMOVE:
			{
			...
			case hkWorldOperation::ENTITY_UPDATE_BROAD_PHASE:
			{
				...
				if (cop.m_entity->getWorld() == m_world)
				{
					hkWorldOperationUtil::updateEntityBP(m_world, cop.m_entity);
				}
				else
				{
			...
			{
				hkWorldOperation::UpdateMovedBodyInfo& cop = static_cast<hkWorldOperation::UpdateMovedBodyInfo&>(op);
				hkRigidBody::updateBroadphaseAndResetCollisionInformationOfWarpedBody(cop.m_entity) ;
				removeReferenceFrom( cop.m_entity );
				break;
			}
			...
			case hkWorldOperation::ENTITY_BATCH_REINTEGRATE_AND_RECOLLIDE:
			{
				// warning: no check is done whether the entities have already been added to the world
				hkWorldOperation::ReintegrateAndRecollideEntityBatch& cop = static_cast<hkWorldOperation::ReintegrateAndRecollideEntityBatch&>(op);
				m_world->m_simulation->reintegrateAndRecollideEntities(cop.m_entities, cop.m_numEntities, m_world);
		...
		HK_ASSERT2(0xad005001, !m_world->areCriticalOperationsLocked() && !m_world->m_blockExecutingPendingOperations, "An operation executed from the pending list did not properly set hkWorld::areCriticalOperationsLocked() or hkWorld::m_blockExecutingPendingOperations state to its initial value");
	...
	HK_ON_DEBUG( hkDebugInfoOnPendingOperationQueues::removeLastPendingOperationQueue(m_world, &stolenPending); );
	stolenPending.swap( m_pending );
	m_pending.clear();
	m_world->m_pendingOperationQueueCount--;
}
...
void HK_CALL hkDebugInfoOnPendingOperationQueues::init(hkWorld* world)
{
	HK_ASSERT2(0, world->m_pendingOperations != HK_NULL, "Internal: Just checking");
	world->m_pendingOperationQueues = new hkDebugInfoOnPendingOperationQueues(&world->m_pendingOperations->m_pending);
}

void HK_CALL hkDebugInfoOnPendingOperationQueues::cleanup(hkWorld* world)
{
...
void HK_CALL hkDebugInfoOnPendingOperationQueues::updateNextPendingOperationIndex(hkWorld* world, int index)
{
	hkDebugInfoOnPendingOperationQueues* queue = world->m_pendingOperationQueues;
	queue = hkDebugInfoOnPendingOperationQueues::getLastQueue(queue);
...
void HK_CALL hkDebugInfoOnPendingOperationQueues::addNextPendingOperationQueue(hkWorld* world, hkArray<hkWorldOperation::BiggestOperation>* tmpOldPending)
{
	hkDebugInfoOnPendingOperationQueues* queue = world->m_pendingOperationQueues;
	queue = hkDebugInfoOnPendingOperationQueues::getLastQueue(queue);
	...
	queue->m_nextQueue = new hkDebugInfoOnPendingOperationQueues(&world->m_pendingOperations->m_pending);
	queue->m_pending = tmpOldPending;
	queue->m_nextQueue->m_prevQueue = queue;
}

	// tmpOldPending -- is passed for assert only
void HK_CALL hkDebugInfoOnPendingOperationQueues::removeLastPendingOperationQueue(hkWorld* world, hkArray<hkWorldOperation::BiggestOperation>* tmpOldPending)
{
	hkDebugInfoOnPendingOperationQueues* queue = world->m_pendingOperationQueues;
	queue = hkDebugInfoOnPendingOperationQueues::getLastQueue(queue);
...
int HK_CALL hkDebugInfoOnPendingOperationQueues::getNumPendingOperationQueues(hkWorld* world)
{
	hkDebugInfoOnPendingOperationQueues* queue = world->m_pendingOperationQueues;
	queue = hkDebugInfoOnPendingOperationQueues::getLastQueue(queue);
...
const hkWorldOperation::BaseOperation* hkDebugInfoOnPendingOperationQueues::findFirstPendingIslandMerge(hkWorld* world, hkSimulationIsland* isleA, hkSimulationIsland* isleB)
{
	...
	while(queue != HK_NULL)
	{
		for (int i = queue->m_nextPendingOperationIndex; i < queue->m_pending->getSize(); i++)
		{
			...
			{
				hkWorldOperation::MergeIslands& cop = static_cast<hkWorldOperation::MergeIslands&>(op);
				hkSimulationIsland* refA = cop.m_entities[0]->getSimulationIsland();
				hkSimulationIsland* refB = cop.m_entities[1]->getSimulationIsland();
...
const hkBool hkDebugInfoOnPendingOperationQueues::areEmpty(hkWorld* world)
{
	hkDebugInfoOnPendingOperationQueues* queue = world->m_pendingOperationQueues;
	queue = hkDebugInfoOnPendingOperationQueues::getLastQueue(queue);
