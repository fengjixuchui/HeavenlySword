...
HK_REFLECTION_DEFINE_VIRTUAL(hkWorldCinfo);
...
hkWorldCinfo::hkWorldCinfo()
{
	...
	m_wantOldStyleDeactivation = false;
...
void hkWorldCinfo::setBroadPhaseWorldSize(hkReal sideLength)
{
	m_broadPhaseWorldAabb.m_min.setAll( -0.5f * sideLength );
	m_broadPhaseWorldAabb.m_max.setAll(  0.5f * sideLength );
}

void hkWorldCinfo::setupSolverInfo( enum hkWorldCinfo::SolverType st)
{
	switch ( st )
	{
