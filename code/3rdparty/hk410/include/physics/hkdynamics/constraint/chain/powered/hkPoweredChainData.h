...
#include <hkdynamics/action/hkArrayAction.h>
	...
	/// hkPowereChainData connects a series of bodies by 6d constraints.
	///
	/// The powered chain is essentially composed of a hkBallSocketChainData with angular motors added on top of it. 
	///
	/// hkPoweredChainData is suited to the accurate simulation of character limbs.
	/// This constraints also allows for motors with a torque limit to model real human muscles.
class hkPoweredChainData : public hkConstraintChainData
{
	public:
		...
		void addConstraintInfoInBodySpace(const hkVector4& pivotInA, const hkVector4& pivotInB, const hkQuaternion& aTc,
											class hkConstraintMotor* xMotor, class hkConstraintMotor* yMotor, class hkConstraintMotor* zMotor);
		...
		static void HK_CALL enableMotor(class hkConstraintChainInstance* instance, int constraintIndex, int motorIndex);

			/// This is re-enables processing of a motor in a chain. Initially all motors are enabled.
		static void HK_CALL disableMotor(class hkConstraintChainInstance* instance, int constraintIndex, int motorIndex);
	...
	public:
		...
		struct Runtime
		{
			// Stores hkSolverResults                [6 * m_infos.getSize()]
			//        hk3dAngularMotorSolverInfo::Status [m_infos.getSize()] // flags for angular constraints
		...
		inline hkSolverResults* getSolverResults(hkConstraintRuntime* runtime) { return reinterpret_cast<hkSolverResults*>( runtime ); }

			// assuming we always want solver results
			// -- other wise swap the order of solverResults and externalRuntimeData 
		inline hk3dAngularMotorSolverInfo::Status* getConstraintFlags(hkConstraintRuntime* runtime)
		{
			return reinterpret_cast<hk3dAngularMotorSolverInfo::Status*>( hkAddByteOffset(runtime, m_infos.getSize() * 6 * sizeof(hkSolverResults) ));
		}
		inline hkReal* getMotorRuntimeQuaternions( hkConstraintRuntime* runtime)
		{
			return reinterpret_cast<hkReal*>( hkAddByteOffset( getConstraintFlags(runtime), HK_NEXT_MULTIPLE_OF(4, m_infos.getSize() * sizeof(hk3dAngularMotorSolverInfo::Status)) ) );
		}
	...
	public:
			...
			/// Serialization ctor.
		hkPoweredChainData(hkFinishLoadedObjectFlag f);
	...
	public:
		...
		struct ConstraintInfo
		{
			...
			class hkConstraintMotor* m_motors[3];
		...
		hkArray<struct ConstraintInfo> m_infos;
			...
			/// Solver tau, this overrides the global value from hkSolverInfo when processing the ball-socket part of the constraint.
			/// The angular parts use the tau/dumping values from the motors.
		hkReal m_tau;

			/// Solver damping, this overrides the global value from hkSolverInfo.
			/// The angular parts use the tau/dumping values from the motors.
		hkReal m_damping;
