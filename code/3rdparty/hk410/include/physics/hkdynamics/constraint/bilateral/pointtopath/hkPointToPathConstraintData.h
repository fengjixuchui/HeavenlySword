...
class hkPointToPathConstraintData : public hkConstraintData
{
	public:  
		...
		hkPointToPathConstraintData();
			...
			/// Destructor.
		virtual ~hkPointToPathConstraintData();
		...
		void setInWorldSpace( const hkTransform& slidingBodyTransform, const hkTransform& referenceBodyTransform, const hkVector4& pivotWs, hkParametricCurve* path, const hkTransform& pathToRefBodyTransform = hkTransform::getIdentity() );

			/// Sets the pivot and path with body space information.
		void setInBodySpace( const hkVector4& pivotA, const hkVector4& pivotB, hkParametricCurve* path);
		...
		virtual hkBool isValid() const;
			...
			/// Set the path.
		void setPath(hkParametricCurve* path);

			/// Get the path.
		inline hkParametricCurve* getPath();
		...
		struct Runtime
		{
			class hkSolverResults m_solverResults[7/*VC6 doesn't like the scoping for SOLVER_RESULT_MAX*/];
	...
	public:
		...
		hkPointToPathConstraintData(hkFinishLoadedObjectFlag f) : hkConstraintData(f), m_atoms(f) {}

};

#include <hkdynamics/constraint/bilateral/pointtopath/hkPointToPathConstraintData.inl>
