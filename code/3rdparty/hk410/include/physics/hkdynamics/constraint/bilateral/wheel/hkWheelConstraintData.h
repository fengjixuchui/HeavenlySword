...
class hkWheelConstraintData : public hkConstraintData
{
	...
	public:
		...
		struct Atoms
		{
			...
			struct hkSetLocalTransformsConstraintAtom	m_suspensionBase;
			struct hkLinLimitConstraintAtom				m_lin0Limit;
			struct hkLinSoftConstraintAtom				m_lin0Soft;
			struct hkLinConstraintAtom					m_lin1;
			struct hkLinConstraintAtom					m_lin2;

			struct hkSetLocalRotationsConstraintAtom	m_steeringBase;
			struct hk2dAngConstraintAtom                m_2dAng;
			...
			const hkConstraintAtom* getAtoms() const { return &m_suspensionBase; }
		...
		struct Runtime
		{
			class hkSolverResults m_solverResults[6/*VC6 doesn't like the scoping for SOLVER_RESULT_MAX*/];
		};
	...
	public:
		...
		hkWheelConstraintData(hkFinishLoadedObjectFlag f) : hkConstraintData(f), m_atoms(f) {}
};

void hkWheelConstraintData::setSuspensionMaxLimit(hkReal maxLimit)
{
	m_atoms.m_lin0Limit.m_max = maxLimit;
}

void hkWheelConstraintData::setSuspensionMinLimit(hkReal minLimit)
{
	m_atoms.m_lin0Limit.m_min = minLimit;
}

void hkWheelConstraintData::setSuspensionStrength(hkReal tau)
{
	m_atoms.m_lin0Soft.m_tau = tau;
}

void hkWheelConstraintData::setSuspensionDamping(hkReal damping)
{
