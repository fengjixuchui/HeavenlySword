...
class hkLimitedHingeConstraintData : public hkConstraintData
{
	public:
		...
		inline void setAngularLimitsTauFactor( hkReal mag );
 		...
 		inline hkReal getAngularLimitsTauFactor(  );
		...
		void setMotorActive( hkConstraintInstance* instance, hkBool toBeEnabled );
			...
			/// The angle passed to the hkConstraintMotor::motor() callback will be the relative angle  
		...
		inline hkReal getMotorTargetAngle();
		...
		struct Runtime
		{
			...
			HK_ALIGN16( class hkSolverResults m_solverResults[8/*VC6 doesn't like the scoping for SOLVER_RESULT_MAX*/] );
	...
	public:
		...
		struct Atoms
		{
			...
			struct hkAngMotorConstraintAtom           m_angMotor;
			struct hkAngFrictionConstraintAtom        m_angFriction;
			struct hkAngLimitConstraintAtom           m_angLimit; 
	...
	public:
		...
		hkLimitedHingeConstraintData(hkFinishLoadedObjectFlag f) : hkConstraintData(f), m_atoms(f) {}

};

#include <hkdynamics/constraint/bilateral/limitedhinge/hkLimitedHingeConstraintData.inl>
