...
class hkMotionState
{
	...
	public:
		...
		hkReal	m_angularDamping;
			...
			// Note: the next variables actually belong to hkMotion, however they can cheaply put here (padding!)
			// The deactivation class of the motion state as used internally
		hkUint16	m_deactivationClass;

			// Deactivation data used internally
		hkUint16    m_deactivationCounter;

			// the packed m_deactivationRefOrientation. See hkWorldCinfo::m_deactivationReferenceDistance for details
		hkUint32 m_deactivationRefOrientation[2];
};

#include <hkmath/basetypes/hkMotionState.inl>
