...
/*		HK_ASSERT(0x777504aa,  x > 0 && x < 256 );
		...
		HK_ASSERT(0x3443045a,  temp >= 0 && temp < 256 );
*/
		hkUint32 temp = getRand32() % x;
		return temp;
	}
	...
	hkReal getRandRange( hkReal from, hkReal to )
	{
		return getRandReal01() * ( to - from ) + from;
	}
	...
	void getRandomRotation( hkQuaternion& rotOut )
	{
		...
		/// sets xyz randomly between -1 and 1
	void getRandomVector11( hkVector4& vecOut )
	{
		vecOut.set( getRandReal11(), getRandReal11(), getRandReal11() );
	}

		// Get random real in range [a,b] using getRand01()
	hkReal getRandRealRange(hkReal a, hkReal b)
	{	
		return (b - a) * getRandReal01() + a;
	}
