											 ...
											 hkMotionState& motionState )
{
#ifdef HK_DEBUG
	{
		if ( motionState.getSweptTransform().getInvDeltaTime() != 0.0f)
		{
			...
			HK_ASSERT(0xf0ff0083, hkMath::equal(info.m_startTime, motionEndTime ) ) ;
		}
	}
#endif
	...
	motionState.getSweptTransform().m_centerOfMass1.addMul4(info.m_deltaTime, linearVelocity);
	...
	hkReal angle;
	{
		...
		const hkReal maxAnglePerStep = hkMath::min2( 0.9f, info.m_deltaTime * motionState.m_maxAngularVelocity );
	...
	/*
	{
		...
		const hkReal rel2SinHalfAngle = 1.0f - sa * angle2 + sb * angle3;
			...
			// create an extra angular velocity to avoid numerical accuracy accumulation in the tims
		const hkReal extraAngularVelocity = 0.001f;	
		const hkReal collisionToleranceEps = extraAngularVelocity * info.m_deltaTime * motionState.m_objectRadius;
		motionState.m_maxAngularError = collisionToleranceEps + motionState.m_objectRadius * rel2SinHalfAngle * angle;
	}
	*/
...
void HK_CALL hkSweptTransformUtil::calcAngularTimInfo( const hkMotionState& ms0, const hkMotionState& ms1, hkReal deltaTime, hkVector4& deltaAngleOut0, hkVector4& deltaAngleOut1)
{
	const hkSweptTransform& st0 = ms0.getSweptTransform();
	const hkSweptTransform& st1 = ms1.getSweptTransform();
	...
	deltaAngleOut0.setMul4( f0, ms0.m_deltaAngle );
	deltaAngleOut1.setMul4( f1, ms1.m_deltaAngle );
}
