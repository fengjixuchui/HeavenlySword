...
class hkVector4
{
	public:
		...
		HK_FORCE_INLINE void storeUncached( void* dest);
		...
		/* equality */
			...
			/// Compares the x, y, z elements of the vectors and returns true if equivalent.
		HK_FORCE_INLINE hkBool equals3(const hkVector4 &a, hkReal epsilon = 1e-3f ) const;

			/// Compares all elements of the vectors, including the w component, and returns true if equivalent.
		HK_FORCE_INLINE hkBool equals4(const hkVector4 &a, hkReal epsilon = 1e-3f ) const;

			/// Elementwise compare.  This function returns the equality status in a hkVector4CompareMask.
			/// This mask contains a bit for every element in the vectors.  See the declaration of hkVector4CompareMask for
			/// the exact values and bit order.
		HK_FORCE_INLINE int compareLessThan4(const hkVector4 &a) const;

			/// Elementwise compare.  This function returns the less than or equal status in a hkVector4CompareMask.
			/// This mask contains a bit for every element in the vectors.  See the declaration of hkVector4CompareMask 
			/// for the exact values and bit order.
		HK_FORCE_INLINE int compareLessThanEqual4(const hkVector4 &a) const;

			/// Elementwise compare.  This function returns the vector's less than zero status in a hkVector4CompareMask.
			/// This mask contains a bit for every element in the vector.  See the declaration of hkVector4CompareMask 
			/// for the exact values and bit order.
		HK_FORCE_INLINE int compareLessThanZero4() const;
		...
		HK_FORCE_INLINE hkSimdReal length3() const;
			...
			/// This avoidins the square root calculation.
		...
		HK_FORCE_INLINE void setXYZ0(const hkVector4& xyz);
			...
			/// Sets each element to its reciprocal.
