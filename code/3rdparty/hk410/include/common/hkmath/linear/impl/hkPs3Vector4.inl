...
inline void hkVector4::storeUncached( void* dest)
{
...
inline void hkVector4::setInterpolate4( const hkVector4& x, const hkVector4& y, hkSimdRealParameter t)
{
	...
	hkQuadReal bs = vec_sub( hkQuadReal(1.0f), bt );
...
{
	m_quad = vec_sub( hkQuadReal(0), x.m_quad);
}
...
inline int hkVector4::compareLessThan4(const hkVector4& a) const
{	
#if defined(__PPU__)
	vector bool int compare = vec_cmplt(m_quad, a.m_quad);
	vector signed int spread = vec_and( compare, (vector signed int){8,4,2,1} );
	union { vector signed int vi; int i[4]; } u;
	u.vi = vec_sums( spread, (vector signed int)(0));
	return u.i[3];
#elif defined(__SPU__)
	vector unsigned int compare = spu_cmpgt(a.m_quad, m_quad);
	vector unsigned int mask = spu_gather(compare);
	return si_to_int((qword)mask);
#endif
}

inline int hkVector4::compareLessThanEqual4(const hkVector4 &a) const
{
#if defined(__PPU__)
	vector bool int compare = vec_cmple(m_quad, a.m_quad);
	vector signed int spread = vec_and( compare, (vector signed int){8,4,2,1} );
	union { vector signed int vi; int i[4]; } u;
	u.vi = vec_sums( spread, (vector signed int)(0));
	return u.i[3];
#elif defined(__SPU__)
	vector unsigned int compare = spu_cmpgt(m_quad, a.m_quad);
	vector unsigned int mask = spu_gather( spu_nor(compare, compare) );
	return si_to_int((qword)mask);
#endif
}

inline int hkVector4::compareLessThanZero4() const
{
#if defined(__PPU__)
	vector bool int compare = vec_cmplt(m_quad, hkQuadReal(0));
	vector signed int spread = vec_and( compare, (vector signed int){8,4,2,1} );
	union { vector signed int vi; int i[4]; } u;
	u.vi = vec_sums( spread, (vector signed int)(0));
	return u.i[3];
#elif defined(__SPU__)
	vector unsigned int compare = spu_cmpgt(hkQuadReal(0), m_quad);
	vector unsigned int mask = spu_gather( compare );
	return si_to_int((qword)mask);
#endif
}
...
{
	m_quad = HK_VECTOR4_COMBINE_XYZ_W(v.m_quad, hkQuadReal(0));
}
