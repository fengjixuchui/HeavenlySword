...
{	
	static HK_ALIGN16( const hkUint32 mask[4] ) = { 0x00000001, 0x00000001, 0x00000001, 0x00000001 };

	hkQuadReal bitSelect = {{ 8.0f, 4.0f, 2.0f, 1.0f }};

	hkQuadReal result = __vcmpgefp( m_quad, a.m_quad );
	result = __vand( result, *(hkQuadReal*)&mask );
	result = __vxor( result, *(hkQuadReal*)&mask );
...
inline int hkVector4::compareLessThanEqual4(const hkVector4 &a) const
{
	...
	hkQuadReal result = __vcmpgtfp( m_quad, a.m_quad );
	result = __vand( result, *(hkQuadReal*)&mask );
	result = __vxor( result, *(hkQuadReal*)&mask );
	result = __vcfsx( result, 0 );
	result = __vmsum4fp( result, bitSelect );
...
{
	static HK_ALIGN16( const hkUint32 negativeMask[4] ) = { 0x80000000, 0x80000000, 0x80000000, 0x80000000 };
	static HK_ALIGN16( const hkUint32 shift[4] ) = { 0x0000001f, 0x0000001f, 0x0000001f, 0x0000001f };
	
	hkQuadReal mask = {{ 8.0f, 4.0f, 2.0f, 1.0f }};

	hkQuadReal result = __vand( m_quad, *(const hkQuadReal*)&negativeMask );
	result = __vsrw( result, *(const hkQuadReal*)&shift );
	result = __vcfsx( result, 0 );
	result = __vmsum4fp( result, *(const hkQuadReal*)&mask );
	result = __vctsxs( result, 0 );
...
inline void hkVector4::_setTransformedPos(const hkTransform& t, const hkVector4& v)
{
	...
	m_quad = __vmaddfp( r.getColumn(2).m_quad, v2, r1 );

	m_quad = __vaddfp( m_quad, t.getTranslation().m_quad );
}
