...
namespace hkCompat_hk300_hk310
{
	...
	static hkVersionUtil::ClassAction UpdateActions[] =
	{
		...
		{ 0x1a8afca6, 0x1718f806, hkVersionUtil::VERSION_INPLACE,	"hkWorldObject", HK_NULL },
		...
		{ 0x846fc690, 0x846fc690, hkVersionUtil::VERSION_INPLACE, "hkAnimationTrack", HK_NULL },
		...
		hkObjectUpdateTracker& tracker )
	{
		...
		for( int i = 0; i < objectsInOut.getSize(); ++i )
		{
			...
			if( hkString::strCmp( v.m_class->getName(), "hkConstraintInstance" ) == 0 )
			{
				const hkClassMember& mem = v.m_class->getDeclaredMember(2);
				HK_ASSERT( 0, hkString::strCmp("entities", mem.getName()) == 0);
