...
template < typename T >
static HK_FORCE_INLINE T pointerHash(T addr, int mod)
{
	...
	//return T(addr) & mod; // debug hash function.
}
...
static int isPowerOf2(int i) { return (i&(i-1)) == 0; }
...
void hkPointerMapBase<T>::clear()
{
	...
	hkString::memSet( m_elem, 0, hkSizeOf(T) * initialCapacity * 2 );
...
void hkPointerMapBase<T>::insert( T key, T val )
{
	...
	for( i = pointerHash(key, m_hashMod);
		(m_elem[i] != 0) && (m_elem[i] != key);
		i = (i+1) & m_hashMod )
	{
		// find free slot
	}

	// dont increment nelems if overwriting.
...
void hkPointerMapBase<T>::resizeTable(int newcap)
{
	...
	hkString::memSet( m_elem, 0, hkSizeOf(T) * newcap ); // dont bother to zero values, only keys
	m_numElems = 0;
	m_hashMod = newcap - 1;
...
hkBool hkPointerMapBase<T>::isOk() const
{
	...
	for( i = 0; i <= m_hashMod; ++i )
	{
		if( m_elem[i] != 0 )
		{
			...
			{
				j = (j + 1) & m_hashMod;
				HK_ASSERT(0x4f6528df,  m_elem[j] != 0 );
			}
		} 
	}
	return true;
