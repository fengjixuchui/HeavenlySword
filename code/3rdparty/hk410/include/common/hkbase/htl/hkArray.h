...
struct hkArrayUtil
{
	...
	static void HK_CALL _reduce(void* array, int sizeElem, char* inplaceMem, int numElemInplaceMem);
};
...
class hkArray
{
	...
	public:
		...
		HK_FORCE_INLINE T* expandAt( int index, int numToInsert );
			...
			/// See also getSubarray() and the constructor, which uses an existing
			/// C style array in place.
		void insertAt(int i, const hkArray<T>& a);
...
class hkInplaceArrayAligned16 : public hkArray<T>
{
	...
	protected:
		...
		// This strange declaration is made due avoid Vector4 iterating constructors
		HK_ALIGN16( hkReal m_storage[sizeof(T)/sizeof(hkReal) * N] );
...
#include <hkbase/htl/hkArray.inl>
