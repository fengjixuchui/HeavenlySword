...
HK_FORCE_INLINE hkArray<T>::hkArray(int n)
	:	m_data( static_cast<T*>(hkThreadMemory::getInstance().allocateChunk(n * hkSizeOf(T), HK_MEMORY_CLASS_ARRAY))),
		m_size(n),
...
{
	m_data = static_cast<T*>( hkThreadMemory::getInstance().allocateChunk( a.getSize() * hkSizeOf(T), HK_MEMORY_CLASS_ARRAY));
	m_size = a.m_size;
...
HK_FORCE_INLINE void hkArray<T>::releaseMemory()
{
	...
	{
		hkThreadMemory::getInstance().deallocateChunk( m_data, getCapacity() * hkSizeOf(T), HK_MEMORY_CLASS_ARRAY);
	}
}
...
#endif
		releaseMemory();
		m_data = static_cast<T*>( hkThreadMemory::getInstance().allocateChunk( a.m_size * hkSizeOf(T), HK_MEMORY_CLASS_ARRAY));
...
template <typename T>
HK_FORCE_INLINE void hkArray<T>::optimizeCapacity( int numFreeElemsLeft )
{
	int size = m_size + numFreeElemsLeft;
...
#endif
		hkArrayUtil::_reduce( this, hkSizeOf(T), HK_NULL, 0 );
	}
}
...
template <typename T>
void hkArray<T>::insertAt(int index, const hkArray<T>& array)
{
	HK_ASSERT(0x4cbc67c5,  index >= 0 && index <= getSize() );
	const int numtoinsert= array.m_size;
	...
	copyBackwards(m_data + index,               array.begin(),  numtoinsert);
	m_size = newsize;
}
...
{
	insertAt(i, hkArray( const_cast<T*>(&t), 1, 1) );
}
...
template <typename T, unsigned N>
HK_FORCE_INLINE void hkInplaceArray<T,N>::optimizeCapacity( int numFreeElemsLeft )
{
	if( (this->m_capacityAndFlags & hkArray<T>::DONT_DEALLOCATE_FLAG) == 0)
	{
		int size = this->m_size + numFreeElemsLeft;
		if ( (size < int(N)) || (size*2 < this->getCapacity() ) )
		{
#ifdef HK_DEBUG
			if ( this->isLocked() )
