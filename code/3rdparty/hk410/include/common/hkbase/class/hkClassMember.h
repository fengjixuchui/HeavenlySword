...
extern const class hkClass hkClassMemberClass;
...
class hkClassMember
{
	public:
		...
		enum Type
		{
			...
			/// By default pointers are required to be resolved
			/// and pointers are assumed to be hkSerializables.
		enum Flags
		{
				/// Pointer is not essential to construction. e.g. userdata pointer.
			POINTER_OPTIONAL = 1,
				/// Pointer to a non serializable object.
			POINTER_VOIDSTAR = 2,
			...
			ENUM_32 = 32,
				...
				/// Whether an array of hkUint8 can be treated as hkUint8 or whether it is just raw data
			ARRAY_RAWDATA = 64
		};

			/// Domain flags in a bitfield.
		enum Range
		{
			INVALID = 0,
			DEFAULT = 1,
		...
		inline int getFlags() const;
			...
			/// Get the range of this member. See the Range enumeration.
			/// If the member has a range value 'r', write the value into 'out'
			/// (which must be large enough) and return HK_SUCCESS. Otherwise
			/// return HK_FAILURE.
		hkResult getRangeValue( Range r, void* out ) const;
	...
	private:
		...
		hkUint16 m_flags; // Pointers:optional, voidstar, rawdata. Enums:sizeinbytes.
		hkUint16 m_offset; // Address offset from start of struct.
};

#include <hkbase/class/hkClassMember.inl>
