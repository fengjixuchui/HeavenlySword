...
namespace
{
	...
	{
		int type; /* const enum hkClassMember::Type */
		const char* name;
		short size;
		short align;
	};
...
static const ClassMemberProperty ClassMemberProperties[] =
{
	...
	{ hkClassMember::TYPE_POINTER, "hkFunctionPointer", sizeof(void*), HK_ALIGN_OF(void*) },
...
HK_COMPILE_TIME_ASSERT( sizeof(ClassMemberProperties)/sizeof(ClassMemberProperty) == hkClassMember::TYPE_MAX + 1 );
...
{
	int type = (getType() != TYPE_ZERO) ? getType() : getSubType();
	int nbytes = -1;
	switch( type )
	{
		case TYPE_BOOL:
		...
		case TYPE_CSTRING:
		{
			...
			nbytes = ClassMemberProperties[ type ].size * nelem;
			break;
		}
		case TYPE_ENUM:
		{
			HK_ASSERT2( 0x1fbcb6bf, (m_flags!=0) && ((m_flags & (m_flags-1)) == 0), "bad m_flags " << m_flags);
			int nelem = getCstyleArraySize() ? getCstyleArraySize() : 1;
			nbytes = nelem * m_flags / 8;
			break;
		}
		case TYPE_STRUCT:
		{
...
int hkClassMember::getAlignment() const
{
	...
	{
		align = ClassMemberProperties[ type ].align;
	}
	HK_ASSERT(0, align != -1);
	return align;
}
...
static void getSimpleTypeName( hkClassMember::Type type, int nelem,  hkString& ret )
{
	...
	if( nelem == 0 )
	{
		ret = ClassMemberProperties[ type ].name;
	}
	else // c array
	{
		ret.printf("%s[%i]", ClassMemberProperties[ type ].name, nelem );
	}
}
...
int hkClassMember::getTypeName(char* buf, int bufLen) const
{
	...
	else if( ( type == TYPE_ARRAY ) || ( type == TYPE_SIMPLEARRAY ))
	{
		...
		{
			ret.printf("%s&lt;%s&gt;", arrayContainer, ClassMemberProperties[ atype ].name );
		}
		else if( atype == TYPE_POINTER )
		{
			if( m_class )
			{
				ret.printf("%s&lt;%s*&gt;", arrayContainer, m_class->getName() );
			}
			else
			{
		...
		{
			ret.printf("%s&lt;struct %s&gt;", arrayContainer, m_class->getName() );
		}
		else
		{
	...
	{
		ret.printf("enum %s", m_enum->getName() );
	}
	else if( type == TYPE_STRUCT )
	{
		if( m_cArraySize == 0 )
		...
		{
			ret.printf("struct %s[%i]", m_class->getName(), m_cArraySize);
		}
	}
...
int hkClassMember::getArrayMemberSize() const
{
	...
	// arrays of enums?
	if ( getArrayType() == TYPE_ENUM )
	{
		HK_ASSERT2(0x198765e3,0,"Arrays of enums are not supported yet...");
		return -1;
	}
	else if( getArrayType() != TYPE_STRUCT )
	{
		int sz = ClassMemberProperties[ getArrayType() ].size;
...
{
	HK_ASSERT( 0x709bd5aa, getType() == TYPE_ENUM );
	return *m_enum;
}
...
// domain values are stored as a char bitfield indicating which
// values are present default, min, max, etc. The actual values
// directly follow the bitfield with no alignment padding so be
// sure to memcpy them out.
hkResult hkClassMember::getRangeValue( Range range, void* p ) const
{
	const char* extrap = (const char*)m_enum;
	if( extrap && (extrap[0] & range ) )
	{
		int sz = getSizeInBytes();
		int offset = 1;
		...
		hkString::memCpy( p, ((const char*)m_enum)+offset, sz );
		return HK_SUCCESS;
	}
...
hkClassMember::Type hkClassMember::getTypeOf( const char* name )
{
	...
	{
		if( sname == ClassMemberProperties[i].name )
		{
