...
#include <hkbase/stream/impl/hkPspSocket.h>
...
#define DEVKIT_PATH "host0:/usr/local/psp/devkit/"
...
namespace hkPspSocketContext
{
	...
	static int loadDialogDummyModule()
	{
		...
		ret = ::sceKernelLoadModule(PSPNET_AP_DIALOG_DUMMY_PRX, 0, NULL);
	...
	int loadPspNetworkModules()
	{
		...
		//HK_REPORT("Load SCE_UTILITY_NET_MODULE_COMMON.");
		ret = sceUtilityLoadNetModule(SCE_UTILITY_NET_MODULE_COMMON);
		if( SCE_ERROR_ISFAILED(ret) )
		{
			HK_PSP_WARN(0, "sceUtilityLoadNetModule(SCE_UTILITY_NET_MODULE_COMMON) failed, ", ret);
			return ret;
		}

		//HK_REPORT("Load SCE_UTILITY_NET_MODULE_INET.");
		ret = sceUtilityLoadNetModule(SCE_UTILITY_NET_MODULE_INET);
		if( SCE_ERROR_ISFAILED(ret) )
		{
			HK_PSP_WARN(0, "sceUtilityLoadNetModule(SCE_UTILITY_NET_MODULE_INET) failed, ", ret);
			return ret;
		}
		...
		if( SCE_ERROR_ISFAILED(ret) )
		{
			HK_PSP_WARN(0, "sceKernelLoadModule(PSPNET_AP_DIALOG_DUMMY_PRX) failed, ", ret);
		}
	...
	int unloadPspNetworkModules()
	{
		...
		if( SCE_ERROR_ISFAILED(ret) )
		{
			HK_PSP_WARN(0, "sceKernelUnloadModule(PSPNET_AP_DIALOG_DUMMY_PRX) failed, ", ret);
			return ret;
		}

		ret = sceUtilityUnloadNetModule(SCE_UTILITY_NET_MODULE_INET);
		if( SCE_ERROR_ISFAILED(ret) )
		{
			HK_PSP_WARN(0, "sceUtilityUnloadNetModule(SCE_UTILITY_NET_MODULE_INET) failed, ", ret);
			return ret;
		}

		ret = sceUtilityUnloadNetModule(SCE_UTILITY_NET_MODULE_COMMON);
		if( SCE_ERROR_ISFAILED(ret) )
		{
			HK_PSP_WARN(0, "sceUtilityUnloadNetModule(SCE_UTILITY_NET_MODULE_COMMON) failed, ", ret);
		}
...
#	define HK_PSP_LOAD_CONF(ID, CODE, ENTRY) \
	...
	static void displayPspNetworkStatus()
	{
		...
		/* tcpcb */
		struct SceNetInetTcpcbstat thead[MAX_TCPCB_NUM];
		ts = thead;
		num = MAX_TCPCB_NUM * sizeof(struct SceNetInetTcpcbstat);
...
hkResult hkPspSocket::connect(const char* servername, int portNumber)
{
	...
	if(::sceNetInetConnect(m_socket, (const struct SceNetInetSockaddr*)(&server), sizeof(server)) == SOCKET_ERROR)
	{
