...
static hkStreamStatisticsCollector::Node* _newNode( hkStreamStatisticsCollector::Node* parent, const char* name, int size, int allocSize, hkObjectArray<hkString>* stringTable)
{
	if ( allocSize == 0 )
	{
		allocSize = size;
	...
	if (!node)
	{
		if (stringTable && name)
		{
			*(stringTable->expandBy(1)) = name;
			name = (*stringTable)[stringTable->getSize()-1].cString();
		}
...
void _sumNodes( hkStreamStatisticsCollector::Node* n, hkObjectArray<hkString>* stringTable )
{
...
void hkStreamStatisticsCollector::makeTree(hkArray<Node*>& m_nodesOut, hkObjectArray<hkString>* stringTable)
{
...
//	hkMonitorStreamAnalyzer::writeStatistics( outstream, nodes, "hkWorld", reportLevel);
