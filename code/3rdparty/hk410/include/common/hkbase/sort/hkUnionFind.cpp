...
int hkUnionFind::moveBiggestGroupToIndexZero( hkArray<int>& elementsPerGroup )
{
	...
	hkLocalBuffer<int> reindex(ngroups);
	{
		for (int i = 0; i < ngroups;i++)
		{
			reindex[i] = i;
		}
		reindex[0] = biggestIndex;
		reindex[biggestIndex] = 0;
	...
	for (int i = 0; i < m_numNodes;i++)
	{
		...
		int newGroup = reindex[oldGroup];
		m_parents[i] = newGroup;
	}
	return biggestIndex;
}

void hkUnionFind::reindex( const hkFixedArray<int>& reindex, int numNewGroups, hkArray<int>& elementsPerGroup )
{
	HK_ASSERT( 0xf0322345, reindex.getSizeDebug() == elementsPerGroup.getSize() );
	for (int i = 0; i < m_numNodes;i++)
	{
		int oldGroup = m_parents[i];
		int newGroup = reindex[oldGroup];
