...
void hkSpuDmaCache<ElemType, numCacheRows, overflowBufferSize>::exit()
{
	...
	for (int r = 0; r < numCacheRows; r++)
	{
		...
		{
			hkSpuDmaManager::performFinalChecks( lines[0].m_originalAddress.val(), &lines[0].m_object, sizeof(ElemType));
		}
		if ( lines[1].m_originalAddress)
		{
			hkSpuDmaManager::performFinalChecks( lines[1].m_originalAddress.val(), &lines[1].m_object, sizeof(ElemType));
		}
	}
	for (int i = 0; i < overflowBufferSize; i++)
	{
		...
		{
			hkSpuDmaManager::performFinalChecks( line->m_originalAddress.val(), &line->m_object, sizeof(ElemType));
		}
	}
#endif
}
...
uploadToCacheLine:
	...
	{
		hkSpuDmaManager::performFinalChecks( freeCacheLine->m_originalAddress, &freeCacheLine->m_object, sizeof(ElemType) );
	}
	{
