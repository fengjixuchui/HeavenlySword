...
#define HK_DECLARE_SYSTEM_ALLOCATOR() \
	...
	///		 calls the appropiate hkMemory::getInstance() functions
...
class hkThreadMemory
{
	public:
		...
		HK_DECLARE_SYSTEM_ALLOCATOR();
			...
			/// Creates a new instance of a thread memoryInitialise the reference count to 1.
			/// \param maxNumElemsOnFreeList: specifies how many objects can be stored on the free list
			///                               before those blocks are given to the main memory manager.
			///								  If you set maxNumElemsOnFreeList to 0, you practically disable this
			///								  memory manager.
		hkThreadMemory( hkMemory* mainMemoryManager, int maxNumElemsOnFreeList = 16);

			/// initialized the thread local storage
		static void init();

			/// frees the thread local storage
		static void quit();
		...
		HK_FORCE_INLINE void* allocateChunkConstSize( int nbytes, HK_MEMORY_CLASS cl );
			/// A very fast inline version of deallocateChunk() which requires that the size is known at compile time and is less than 8k
		...
		virtual void  alignedDeallocate(void* ptr);
			...
			/// You can call this method any time the stack is empty, e.g. outside of hkWorld::stepDeltaTime callbacks.
	...
	protected:
			...
			//
		HK_FORCE_INLINE void putOnFreeList(void* p, int row);
		...
		void* onRowEmpty(int row);
			// Called when an deallocation from a full row is requested.
		void onRowFull(int row);
	...
	public:
		...
		static HK_THREAD_LOCAL( hkThreadMemory* ) s_threadMemoryInstance;
		...
		MemoryElem* m_free_list[MEMORY_MAX_ALL_ROW];

			// statistics for blocks of each size in the free list
		int m_num_free_blocks[MEMORY_MAX_ALL_ROW];
...
template <typename TYPE>	HK_FORCE_INLINE TYPE* HK_CALL hkAllocateStack(int n);
...
#define HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR_BY_SIZE_UNCHECKED(TYPE,CLASS_SIZE) \
	...
	HK_FORCE_INLINE void  HK_CALL operator delete(void* p)			{ hkThreadMemory::getInstance().deallocateChunkConstSize(p, CLASS_SIZE, TYPE); }								\
...
#   define HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(TYPE,THIS_CLASS) \
        HK_FORCE_INLINE void iShouldNotHaveVtable() const { int m_memSizeAndFlags = 0; m_memSizeAndFlags--; } \
