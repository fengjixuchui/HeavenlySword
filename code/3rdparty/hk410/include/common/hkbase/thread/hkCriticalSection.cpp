...
#		if defined HK_TIME_CRITICAL_SECTION_LOCKS
			HK_THREAD_LOCAL_IMPL( int ) hkCriticalSection::m_timeLocks; // init to 0 (false) per thread
#		endif
#	elif defined(HK_PLATFORM_PS3)
		HK_COMPILE_TIME_ASSERT( sizeof(sys_mutex_t) == sizeof(hkUint32) );
#	endif
	hkCriticalSectionBase* hkCriticalSectionBase::sHead = HK_NULL;
	hkSemaphoreBusyWait hkCriticalSectionBase::sListSemaphore(1,1);
#endif
