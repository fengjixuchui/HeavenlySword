...
class hkDebugDisplayProcess : public hkReferencedObject, public hkProcess, public hkDebugDisplayHandler
{
	public:
		...
		static inline const char* HK_CALL getName() { return "DebugDisplay"; }

		//
		// Display handler functions
		//
		virtual hkResult addGeometry(const hkArray<hkDisplayGeometry*>& geometries, const hkTransform& transform, hkUlong id, int tag);
		...
		virtual hkResult removeGeometry(hkUlong id, int tag);
