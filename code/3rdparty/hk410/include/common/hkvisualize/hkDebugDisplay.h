...
class hkDebugDisplay : public hkSingleton<hkDebugDisplay>
{
	...
	public:
			...
			/// for supplying an id is to take the address of the hkCollidable from which the geometries were
			/// created.  
			/// Please refer to the hkDisplayHandler class description for the meaning of id and tag.
		void addGeometry(const hkArray<hkDisplayGeometry*>& geometries, const hkTransform& transform, hkUlong id, int tag);
		...
		void updateGeometry(const hkTransform& transform, hkUlong id, int tag);
			...
			/// Please refer to the hkDisplayHandler class description for the meaning of id and tag.
		void removeGeometry(hkUlong id, int tag);
...
#	define HK_ADD_GEOMETRY(geometries, transform, id) hkDebugDisplay::getInstance().addGeometry(geometries, transform, id, 0);
...
#	define HK_REMOVE_GEOMETRY(id) hkDebugDisplay::getInstance().removeGeometry(id, 0);
