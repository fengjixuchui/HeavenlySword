...
/// address of the hkCollidable where the geometries came from.
/// Geometries are built using the hkShapeDisplayBuilder class in
/// hkvisualize/hkShapeDisplayBuilder.h/cpp.
///
...
class hkDebugDisplayHandler
{
	public:
		...
		virtual hkResult addGeometry(const hkArray<hkDisplayGeometry*>& geometries, const hkTransform& transform, hkUlong id, int tag) = 0;
		...
		virtual hkResult updateGeometry(const hkTransform& transform, hkUlong id, int tag) = 0;
			...
			/// Please refer to the class description for the meaning of id and tag.
		virtual hkResult removeGeometry(hkUlong id, int tag) = 0;
