...
//
void hkDebugDisplay::addGeometry(const hkArray<hkDisplayGeometry*>& geometries, const hkTransform& transform, hkUlong id, int tag)
{
	m_arrayLock->enter();
	for(int i = 0; i < m_debugDisplayHandlers.getSize(); i++)
	{
		m_debugDisplayHandlers[i]->addGeometry(geometries, transform, id, tag);
	}
	m_arrayLock->leave();
}
...
void hkDebugDisplay::removeGeometry(hkUlong id, int tag)
{
	m_arrayLock->enter();
	for(int i = 0; i < m_debugDisplayHandlers.getSize(); i++)
	{
		m_debugDisplayHandlers[i]->removeGeometry(id, tag);
	}
	m_arrayLock->leave();
}
