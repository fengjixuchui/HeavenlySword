		...
		m_frameTimer("Frame Timer")
{
	...
	addDefaultProcess("DebugDisplay");	
...
void hkVisualDebugger::pollForNewClients()
{
	...
	if(m_server)
	{
		...
		if(socket)
		{
			HK_REPORT("A new network client has been received (host name not availibe at present)");
...
void hkVisualDebugger::step(hkReal frameTimeInMs)
{
	...
	for(int i = (nc - 1); i >= 0; --i) // backwards as they may be deleted.
	{
		...
		if( client.m_processHandler->isOk() )
		{
			client.m_processHandler->step(frameTimeInMs);

			writeStep(i, frameTimeInMs); // a 0x00 command followed by the float32 for time
		}
