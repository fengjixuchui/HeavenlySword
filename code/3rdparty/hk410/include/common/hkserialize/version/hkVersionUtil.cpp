	...
	hkObjectUpdateTracker& tracker )
{
	...
	if( buffer.getSize() ) // some versioning was done
	{
		...
		tracker.addAllocation(versioned);
...
void hkVersionUtil::recomputeClassMemberOffsets( hkClass*const* classes )
{
...
hkResult HK_CALL hkVersionUtil::updateToCurrentVersion(
	...
	const hkVersionRegistry& reg )
{
	const char* originalVersion = reader.getOriginalContentsVersion();
	if( hkString::strCmp( originalVersion, getCurrentVersion() ) != 0 )
	{
		...
		{
			return hkVersionUtil::updateBetweenVersions(
				loadedObjects,
		...
		return HK_FAILURE; // getContents failed
	}
