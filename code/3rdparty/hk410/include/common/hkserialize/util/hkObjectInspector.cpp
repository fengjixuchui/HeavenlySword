...
typedef hkArray<hkObjectInspector::Pointer> PointerArray;
...
static inline hkResult scanArrayOfStructs(const void* arrayStart, int arraySize, const hkClass* klass, PointerArray& ioPointers);
...
inline hkResult scanArrayOfStructs(const void* arrayStart, int arraySize, const hkClass* klass, PointerArray& ioPointers)
{
	...
	for( int memberIdx = 0; memberIdx < klass.getNumMembers(); ++memberIdx )
	{
		...
		switch( member.getType() )
		{
			...
			case hkClassMember::TYPE_INPLACEARRAY:
			{
				...
				const DummyArray<char>* carray = (const DummyArray<char>*)memberStart;
				...
				if (carray->data != HK_NULL)
				{
					...
					{
						const DummyArray<hkVariant>* varray = (const DummyArray<hkVariant>*)memberStart;
			...
			case hkClassMember::TYPE_HOMOGENEOUSARRAY:
			{
				...
				const DummyHomogeneousArray* darray = (const DummyHomogeneousArray*)memberStart;
