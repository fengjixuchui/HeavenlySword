...
static int isBasicPointerType( hkClassMember::Type t )
{
	return t == hkClassMember::TYPE_POINTER
		|| t == hkClassMember::TYPE_ARRAY
		|| t == hkClassMember::TYPE_SIMPLEARRAY
		|| t == hkClassMember::TYPE_HOMOGENEOUSARRAY
		|| t == hkClassMember::TYPE_VARIANT
		|| t == hkClassMember::TYPE_CSTRING;
}

static int isPointerType( const hkClassMember& member )
{
	return isBasicPointerType( member.getType() )
		|| ((member.getType() == hkClassMember::TYPE_ZERO) && isBasicPointerType( member.getSubType()) );
}
	...
	hkPointerMap<const hkClass*,int>& classesDone )
{
	...
	for( int hierIdx = 0; hierIdx < hier.getSize(); ++hierIdx)
	{
		...
		for( int memIdx = 0; memIdx < klass.getNumDeclaredMembers(); ++memIdx)
		{
			...
			if( isPointerType(member) )
			{
				memberAlign = rules.m_bytesInPointer;
			}
