...
class hkLoader 
{
	public:
		...
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_SERIALIZE, hkLoader);
		...
		class hkRootLevelContainer* load( const char* filename, hkFinishLoadedObjectRegistry* finish, hkVersionRegistry* version );

			/// Loads from a stream, Explicitly allocates and keeps a handle to the memory allocated 
			/// This method is usually called multiple times to load several different assets.
			/// The hkVersionRegistry may be null which disables versioning.
			/// Uses the registries passed explicitly.
		class hkRootLevelContainer* load( class hkStreamReader* reader, hkFinishLoadedObjectRegistry* finish, hkVersionRegistry* version );
		...
		void* load( class hkStreamReader* reader, const hkClass& expectedTopLevelClass );
			...
			/// The hkVersionRegistry may be null which disables versioning.
			/// Uses the registries passed explicitly.
		void* load( const char* filename, const hkClass& expectedTopLevelClass, hkFinishLoadedObjectRegistry* finish, hkVersionRegistry* version );
			...
			/// The hkVersionRegistry may be null which disables versioning.
			/// Uses the registries passed explicitly.
		void* load( class hkStreamReader* reader, const hkClass& expectedTopLevelClass, hkFinishLoadedObjectRegistry* finish, hkVersionRegistry* version );
