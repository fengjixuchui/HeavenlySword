...
hkResult HK_CALL hkVersionCheckingUtils::verifyUpdateDescription(
	...
	bool verbose )
{
	hkRenamedClassNameRegistry newClassFromOldName(updateDescription.renames, updateDescription.classes);

	hkResult result = HK_SUCCESS;

	for( int i = 0; oldClasses[i] != HK_NULL; ++i )
	{
		...
		if( containsVariants(*newClass) )
		{
			...
			{
				report.printf("%s has variants, but does not set VERSION_VARIANT\n", newClass->getName() );
				result = HK_FAILURE;
				continue;
			}
		}
		...
		if( oldGlobalSig == newGlobalSig )
		{
			if( action && hkString::strCmp( action->oldClassName, oldClass->getName()) == 0 )
			{
				if( action->oldSignature != oldGlobalSig || action->newSignature != newGlobalSig )
				{
					report.printf("%s old(0x%x, 0x%x) actual(0x%x, 0x%x) signature mismatch\n", oldClass->getName(),
		...
		if( oldLocalSig == newLocalSig
			...
			&& hkString::strCmp(oldParentName, newParentName) == 0)
		{
			continue;
		}
		...
		if( action->versionFunc == HK_NULL )
		{
			const hkClass* c = newClassFromOldName.getClassByName(oldClass->getName());
			if( c )
			{
				for( int memberIndex = 0; memberIndex < c->getNumMembers(); ++memberIndex )
				{
					const hkClassMember& oldMem = c->getMember(memberIndex);
					const hkClassMember* newMemPtr = oldClass->getMemberByName( oldMem.getName() );
					if( newMemPtr )
					{
						...
						{
							report.printf("%s::%s type changed but no version func\n",
