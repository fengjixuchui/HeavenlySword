...
class hkXmlPackfileReader : public hkPackfileReader
{
	public:
		...
		virtual void* getContentsWithRegistry( const char* className, const hkFinishLoadedObjectRegistry* finish );
		...
		virtual hkObjectUpdateTracker& getUpdateTracker();
			...
			// Inherited from hkPackfileReader
		virtual const char* getOriginalContentsVersion();

	protected:

		const hkClass* getClassByName( const char* className, hkClassNameRegistry& reg, hkStringMap<hkClass*>& partiallyLoadedClasses, hkPointerMap<const hkClass*, int>& offsetsRecomputed, int classVersion );
