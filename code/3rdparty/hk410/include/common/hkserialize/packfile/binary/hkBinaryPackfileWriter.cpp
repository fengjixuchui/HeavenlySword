...
HK_COMPILE_TIME_ASSERT( sizeof(hkPackfileSectionHeader) == 12*sizeof(hkInt32));
...
static const int BINARY_FILE_VERSION = 4;
...
struct hkBinaryPackfileWriter::SaveContext
{
	...
	hkArray<Location> m_locations; // eventual locations of objects - matches m_pendingwrites
	hkStreamWriter* m_stream;
	hkPlatformObjectWriter m_writer;
	hkStringMap<int> m_usedClassNames;
	hkObjectArray<hkRelocationInfo> m_relocsBySect;
...
HK_COMPILE_TIME_ASSERT( sizeof(hkPackfileSectionHeader)==48 );

static void padUp( hkStreamWriter* w, int pad=16 )
{
	int o = w->tell();
	hkInplaceArray<char, 32> buf;
	buf.setSize(pad, (unsigned char)(0xff));
	if( o & (pad-1) )
	{
...
static void saveFileHeader(hkPlatformObjectWriter& writer, hkStreamWriter* stream, const hkPackfileWriter::Options& options, int numSections )
{
	...
	header.m_fileVersion = BINARY_FILE_VERSION;
...
void hkBinaryPackfileWriter::writeAllObjects( SaveContext& context )
{
	...
	if(1)
	{
		...
		hkStringMap<const hkClass*>::Iterator it = m_knownClasses.getIterator();
		for( ; m_knownClasses.isValid(it); it = m_knownClasses.getNext(it) )
		{
			writeClassName( context, *m_knownClasses.getValue(it), section.m_absoluteDataStart );
		}
	...
	for( int sectionIndex = INDEX_FIRST_NORMAL; sectionIndex < context.m_sectionHeaders.getSize(); ++sectionIndex)
	{
		...
		if( context.m_options.m_writeMetaInfo || sectionIndex != classSectionIndex )
		{
			for( int i = 0; i < m_pendingWrites.getSize(); ++i )
			{
				...
				HK_ASSERT(0,w.m_sectionIndex != 0);
		...
		// some global fixups are changed into imports
		{
			for( int i = fixups.m_global.getSize() - 1; i >= 0; --i )
			{
				if( const char* id = m_imports.getWithDefault( fixups.m_global[i].m_toAddress, HK_NULL ) )
				{
					...
					HK_ASSERT2(0x68eb4f33, m_knownObjects.getWithDefault(fixups.m_global[i].m_toAddress, -1) < 0,
