...
#include <hkanimation/playback/multithreaded/spu/hkSpuConfig.h>
...
static HK_ALIGN16(char stackBuffer[HK_SPU_TOTAL_BUFFER_SIZE]);
...
{
	HK_SPU_DEBUG_PRINTF(("SPU(%d) animation thread started\n", (int)spuId));

	hkSpuStack::getInstance().initMemory(stackBuffer, HK_SPU_TOTAL_BUFFER_SIZE);
	...
	while (1)
	{
		hkSpuProcessNextAnimationJob( *queue, hkJobQueue::WAIT_FOR_NEXT_JOB, int(spuId) );
