...
inline hkBool hkDefaultChunkCache::hashKey( struct hashKeyInfo& info ) 
{
	//
	// run the key through the hashing function ( each pool has it's own modulus value == bucket size )
	//
...
// add this request to the list of unavailable chunks
		...
		// warn that chunk size is unavailable
