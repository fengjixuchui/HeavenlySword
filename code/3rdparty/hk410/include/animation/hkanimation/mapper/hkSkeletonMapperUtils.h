...
/// and, through the "createMapping" static method, creates the mapping data that can be used at run-time (by an hkSkeletonMapper) to
...
class hkSkeletonMapperUtils
{
	public:
		...
		struct Params
		{
			...
			const hkSkeleton* m_skeletonA;
			const hkSkeleton* m_skeletonB;
			...
			/// Creates the mapping data (used by hkSkeletonMapper) used to map poses from skeleton A to skeleton B
			/// and vice-versa.
		static void HK_CALL createMapping (const Params& params, hkSkeletonMapperData& aToB, hkSkeletonMapperData& bToA);
		...
		static const struct hkSkeletonMapperData::SimpleMapping* HK_CALL findSimpleMapping(hkInt16 boneA, const hkSkeletonMapperData& mapperdata );
		...
		static hkResult HK_CALL lockTranslationsAutomatically (const hkSkeletonMapperData& mapperData);
};
