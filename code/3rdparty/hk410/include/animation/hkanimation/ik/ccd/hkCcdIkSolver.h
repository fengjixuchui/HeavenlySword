...
class hkCcdIkSolver : public hkReferencedObject
{
	public:
		...
		hkCcdIkSolver( hkInt32 iterations = 8, hkReal gain = 0.5f );

			/// The main method in the class, it performs the IK on the given pose.
		hkBool solve ( const hkArray<IkConstraint>& constraints, hkPose& poseInOut );
...
#include <hkanimation/ik/ccd/hkCcdIkSolver.inl>
