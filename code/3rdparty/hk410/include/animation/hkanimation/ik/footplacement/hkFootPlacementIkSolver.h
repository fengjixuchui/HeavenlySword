...
class hkRaycastInterface
{
	public:

		virtual ~hkRaycastInterface() { }
...
class hkFootPlacementIkSolver : public hkReferencedObject
{
	public:
		...
		struct Input
		{
			...
			hkRaycastInterface* m_raycastInterface;
		...
		hkFootPlacementIkSolver(const Setup& setup);
		...
		void doFootPlacement (const Input& input, Output& output, hkPose& poseInOut);
	...
	private:
		...
		*/
		...
		hkBool castFoot( hkRaycastInterface* raycastInterface, const hkVector4& footWS, const hkVector4& footEndWS, hkVector4& projectedFootWSOut, hkVector4& groundNormalWSOut ) const;
};
