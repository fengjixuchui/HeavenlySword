...
class hkSkeletonUtils
{
	public:
		...
		static void HK_CALL enforceSkeletonConstraintsLocalSpace(const hkSkeleton& skeleton, hkQsTransform* poseLocalInOut);

			/// Given a model space pose, it will ensure that any bone constraint in the skeleton are enforced
			/// Currently the only bone constraint is "lockTranslation"
			/// This ensures the pose translations match the translations in the skeleton reference pose.
		static void HK_CALL enforceSkeletonConstraintsModelSpace(const hkSkeleton& skeleton, hkQsTransform* poseModelInOut);

			/// Given a model space pose, it will ensure that any bone constraint in the skeleton are enforced
			/// Currently the only bone constraint is "lockTranslation"
			/// This ensures the pose translations match the translations in the constraint pose (given in local space).
		static void HK_CALL enforcePoseConstraintsLocalSpace (const hkSkeleton& skeleton, const hkQsTransform* constraintsLocal, hkQsTransform* poseInOut);

			/// Given a model space pose, it will ensure that any bone constraint in the skeleton are enforced
			/// Currently the only bone constraint is "lockTranslation"
			/// This ensures the pose translations match the translations in the constraint pose (given in local space).
		static void HK_CALL enforcePoseConstraintsModelSpace (const hkSkeleton& skeleton, const hkQsTransform* constraintsLocal, hkQsTransform* poseModelInOut);

			/// Looks for a bone with the specified name (possibly filtering it). Returns -1 if not found, or the bone ID if found
		static hkInt16 HK_CALL findBoneWithName (const hkSkeleton& skeleton, const char* name, hkStringCompareFunc compare = HK_NULL);

			/// Locks the translation of all bones in the given skeleton (except roots)
		static void HK_CALL lockTranslations (const hkSkeleton& skeleton, hkBool exceptRoots = true);

			/// Unlocks all the translations for the given skeleton
		static void HK_CALL unlockTranslations (const hkSkeleton& skeleton);
		...
		static hkBool HK_CALL getBoneChain(const hkSkeleton& skeleton, hkInt16 startBone, hkInt16 endBone, hkArray<hkInt16>& bonesOut);

			/// Fills bonesOut with the id of all bones descending from the given bone
			/// This methods possibly allocates space in the array (since its length is unknown by the caller)
		static void HK_CALL getDescendants(const hkSkeleton& skeleton, hkInt16 startBone, hkArray<hkInt16>& bonesOut);

			/// This method checks the mapping in the binding and returns true if all bone indices are valid for this skeleton
		static hkBool HK_CALL isBindingOk( const hkSkeleton& skeleton, const class hkAnimationBinding& binding );

			/// Given a local pose, gets the scale in model space of the associated bone
		static void HK_CALL getModelSpaceScale (const hkSkeleton& skeleton, const hkQsTransform* poseLocal, int boneId, hkVector4& scaleOut);
		...
		static void HK_CALL normalizeRotations(hkQsTransform* transformsInOut, int numTransforms );
			...
			/// The m_animationTrackToBoneIndices array must be appropriately initialized 
			/// (remember to set m_numAnimationTrackToBoneIndices and allocate memory)
		static hkResult HK_CALL bindByName(const hkSkeleton& skeleton, const class hkSkeletalAnimation& anim, hkStringCompareFunc compare, class hkAnimationBinding& bindingOut);
};
