...
class hkSimdSkinningDeformer : public hkSkinningDeformer
{
	public:
		...
		static void HK_CALL deform( const hkTransform* m_worldCompositeMatrices, const hkSimdBinding& binding  );

			/// Input and Output are both 16 byte aligned
		static void HK_CALL deformAligned( const hkTransform* m_worldCompositeMatrices, const hkSimdBinding& binding  );
			
			/// Only Input is 16 byte aligned, requires internal loop copy to unaligned float output buffers so slower
		static void HK_CALL deformAlignedInput( const hkTransform* m_worldCompositeMatrices, const hkSimdBinding& binding );
...
#endif // HK_SIMD_SKINNING_DEFORMER_H
