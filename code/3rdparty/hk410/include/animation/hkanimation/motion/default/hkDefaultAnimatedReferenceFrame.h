...
/// class by extracting the motion from an hkInterleavedSkeletalAnimation.
class hkDefaultAnimatedReferenceFrame : public hkAnimatedReferenceFrame
{
	public:
		...
		struct MotionExtractionOptions
		{
			...
			/// motion of an animation and associate the motion to it, you can use the hkAnimatedReferenceFrameUtils class.
		hkDefaultAnimatedReferenceFrame( const MotionExtractionOptions& options );


			/// Destructor; it deallocates the memory used to store the motion track
		~hkDefaultAnimatedReferenceFrame();

			/// Overridden from hkDefaultAnimatedReferenceFrame, this method returns the motion extracted from the
			/// from the animation up to the given time. See hkAnimatedReferenceFrame::getReferenceFrame() for details.
	...
	public:
		...
		static void project(const MotionExtractionOptions& options, const hkQsTransform& rootMotion, const hkVector4& previousProjection, hkVector4& projectionOut);
	...
	public:
		...
		HK_FORCE_INLINE hkDefaultAnimatedReferenceFrame( hkFinishLoadedObjectFlag flag ) : hkAnimatedReferenceFrame(flag) {}
