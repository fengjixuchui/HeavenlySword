...
/// The construction info for the hsMultithreadingUtil
struct hsMultithreadingUtilCinfo
{
...
class hsMultithreadingUtil
{
	public:
		...
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_UTILITIES, hsMultithreadingUtil);

		enum { MAX_NUM_PHYSICS_THREADS = 6 };

			/// Initialize multithreading state and create threads.
		hsMultithreadingUtil( const hsMultithreadingUtilCinfo& cinfo );

			/// Destroy threads and delete state.
		~hsMultithreadingUtil();
		...
		void stepWorld( const hkArray<hkReal>& physicsDeltaTimes );
			...
			/// physics threads.
		void startStepWorld( const hkArray<hkReal>& physicsDeltaTimes );
