...
#include "hsMultithreadingUtil.h"
...
hsMultithreadingUtilCinfo::hsMultithreadingUtilCinfo()
:	m_world(HK_NULL), 
	m_numThreads(1), 
...
hsMultithreadingUtil::State::State() 
:	m_workerThreadFinished( 0, hsMultithreadingUtil::MAX_NUM_PHYSICS_THREADS ), 
	m_stepInitSemaphore( 0, hsMultithreadingUtil::MAX_NUM_PHYSICS_THREADS ), 
	m_stepEndSemaphore( 0, hsMultithreadingUtil::MAX_NUM_PHYSICS_THREADS ), 
	m_asyncSetupSemaphore( 0, hsMultithreadingUtil::MAX_NUM_PHYSICS_THREADS ),
	m_asyncFinishSemaphore( 0, hsMultithreadingUtil::MAX_NUM_PHYSICS_THREADS ),
	m_workerThreads()
{
	m_numThreads = 0;
	...
	m_world = HK_NULL;
	m_deltaTimes.pushBack( 1.0f / 60.0f);

	// Asychronous stepping is disabled by default
	m_frameDeltaTime = -1;
}

hsMultithreadingUtil::WorkerThreadData::WorkerThreadData() : m_semaphore(0,1) 
{
...
hsMultithreadingUtil::hsMultithreadingUtil(const hsMultithreadingUtilCinfo& ci)
{
...
hsMultithreadingUtil::~hsMultithreadingUtil()
{
	if (m_state.m_world)
	{
...
void hsMultithreadingUtil::initMultithreading(hkWorld* world, int numThreads)
{
...
void hsMultithreadingUtil::attachWorld(hkWorld* world)
{
	HK_ASSERT2(0xaf33ede1, !m_state.m_world, "There is already a world attached to this hsMultithreadingUtil. Please detach the old world first.");
	m_state.m_world = world;
	m_state.m_world->addReference();
}

void hsMultithreadingUtil::detachWorld(hkWorld* world)
{
...
void hsMultithreadingUtil::setFrameTimeMarker( hkReal frameDeltaTime )
{	
	HK_ASSERT2(0xad56ddf7, m_state.m_physicsRunning == false, "Calling hsMultithreadingUtil::setFrameTimeMarker(), without having called hsMultithreadingUtil::waitForStepWorldFinished().");

	HK_WARN_ONCE(0x09afce65, "Multithreaded asynchrouous stepping with hsMultithreadingUtil is not recommended, as it will lead to poor performance \
...
void hsMultithreadingUtil::startStepWorld( const hkArray<hkReal>& physicsDeltaTimes )
{
	HK_ASSERT2(0xad56dd77, m_state.m_physicsRunning == false, "Calling hsMultithreadingUtil::startStepWorld() for the second time, without having called hsMultithreadingUtil::waitForStepWorldFinished().");
	m_state.m_physicsRunning = true;

	m_state.m_deltaTimes = physicsDeltaTimes;
...
void hsMultithreadingUtil::waitForStepWorldFinished()
{
...
void hsMultithreadingUtil::stepWorld( const hkArray<hkReal>& physicsDeltaTimes )
{
	HK_ASSERT2( 0xf0213454, m_state.m_world->isLocked() == false, "You cannot lock the world when calling this function." );
	startStepWorld( physicsDeltaTimes );
	waitForStepWorldFinished();
}

void hsMultithreadingUtil::quitMultithreading()
{
	waitForStepWorldFinished();
...
void hsMultithreadingUtil::reassignThreadTokens()
{
...
hkResult hsMultithreadingUtil::addThread()
{
...
hkResult hsMultithreadingUtil::removeThread()
{
...
static void* HK_CALL WorkerThreadFunc(void *v)
{
	hsMultithreadingUtil::WorkerThreadData& data = *static_cast<hsMultithreadingUtil::WorkerThreadData*>(v);
	hsMultithreadingUtil::State& state = *data.m_state;
	...
	while (data.m_killThread == false)
	{
		...
		if ( state.m_frameDeltaTime == -1.f )
		{
...

			{
				...
				if ( token == HK_THREAD_TOKEN_PRIMARY )
				{
					...
					//
					state.m_world->stepBeginSt( state.m_deltaTimes[0] );
		...
		else
		{
			...
			for(int i = 0; i < state.m_deltaTimes.getSize(); i++)
			{
				if (token == HK_THREAD_TOKEN_PRIMARY)
				{
				...
				if ( token == HK_THREAD_TOKEN_PRIMARY )
				{
					...
					state.m_world->stepBeginSt( state.m_deltaTimes[i] );
		...
		ntAssert(state.m_world->isSimulationAtMarker());
...
/*
* Havok SDK - PUBLIC RELEASE, BUILD(#20060802)
*
